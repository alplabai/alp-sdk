 ...
 *
 * v2n-secure-element-sign -- exercise the OPTIGA Trust M secure
 * element by initialising it, reading the product info object, and
 * issuing an ECDSA-P256 sign APDU against a fixed message hash.
 *
 * The Trust M sits on BRD_I2C at 7-bit address 0x30.  On a fresh
 * chip the host has to (1) issue OPEN_APPLICATION at the data-link
 * layer -- the driver's `optiga_trust_m_init` does this -- and (2)
 * issue a CALC_SIGN APDU referencing the key slot the production
 * line provisioned with an ECC private key.
 *
 * The example targets **key OID 0xE0F0** which is the Trust M's
 * canonical "Device Endpoint" device-identity slot per Infineon's
 * Solution Reference Manual; chips that aren't provisioned (e.g.
 * a factory-fresh part on an early-bring-up board) reply with
 * status `0x01 0x02` ("data-object not present") and the example
 ...
 * SHA-256("alp") = 2a517c2f6d3a4ad0c25137d44829ccec2785334123ef37e8
 *                   81abb91eed1d4659  (32 bytes).
...
static const uint8_t MESSAGE_DIGEST[32] = {
	0x2a, 0x51, 0x7c, 0x2f, 0x6d, 0x3a, 0x4a, 0xd0, 0xc2, 0x51, 0x37, 0xd4, 0x48, 0x29, 0xcc, 0xec,
	0x27, 0x85, 0x33, 0x41, 0x23, 0xef, 0x37, 0xe8, 0x81, 0xab, 0xb9, 0x1e, 0xed, 0x1d, 0x46, 0x59,
};

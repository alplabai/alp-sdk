...
#ifdef CONFIG_MDIO
static int zephyr_mdio_read_shim(uint8_t phy_addr, uint8_t reg,
                                 uint16_t *val, void *user) {
    const struct device *mdio = (const struct device *)user;
    return mdio_read(mdio, phy_addr, reg, val);
}

static int zephyr_mdio_write_shim(uint8_t phy_addr, uint8_t reg,
                                  uint16_t val, void *user) {
    const struct device *mdio = (const struct device *)user;
...
static int mock_mdio_read(uint8_t phy_addr, uint8_t reg,
                          uint16_t *val, void *user) {
    (void)phy_addr; (void)reg; (void)user;
    *val = 0u;
    return 0;
}
static int mock_mdio_write(uint8_t phy_addr, uint8_t reg,
                           uint16_t val, void *user) {
    (void)phy_addr; (void)reg; (void)val; (void)user;
    return 0;
}
#endif
...
static void exercise_phy(const char *label,
    ...
    printf("[%s] link  -> status=%d up=%d speed=%d full_duplex=%d\n",
           label, (int)s, (int)up, (int)speed, (int)full_duplex);
}

...
int main(void)
{
	...
	rv = disk_access_ioctl(EMMC_DISK_NAME, DISK_IOCTL_GET_SECTOR_SIZE, &sector_size);
	if (rv != 0) {
		printf("[emmc] GET_SECTOR_SIZE -> %d\n", rv);
		return 0;
	}
	uint32_t sector_count = 0u;
	rv = disk_access_ioctl(EMMC_DISK_NAME, DISK_IOCTL_GET_SECTOR_COUNT, &sector_count);
	if (rv != 0) {
	...
	rv = disk_access_ioctl(EMMC_DISK_NAME, DISK_IOCTL_GET_ERASE_BLOCK_SZ, &erase_block_sz);
	if (rv == 0) {

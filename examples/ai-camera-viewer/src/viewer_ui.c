...
void viewer_ui_build(void)
{
    ...
    for (int i = 0; i < VIEWER_MAX_BOXES; i++) {
        ...
        lv_obj_set_style_border_color(s_box_widgets[i],
                                      lv_color_hex(0x00FF80), 0);
...
void viewer_ui_apply(const viewer_state_t *s)
{
    ...
    /* Stats strip. */
    snprintf(buf, sizeof(buf), "%2u.%u fps  %5u µs/inv",
             (unsigned)(s->fps_x10 / 10),
             (unsigned)(s->fps_x10 % 10),
             (unsigned)s->last_invoke_us);
    lv_label_set_text(s_lbl_stats, buf);

    snprintf(buf, sizeof(buf), "cam: %s  inf: %s  boxes: %u",
             s->camera_ok    ? "OK" : "!!",
             s->inference_ok ? "OK" : "!!",
             (unsigned)s->n_boxes);

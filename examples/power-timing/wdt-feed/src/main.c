...
int main(void)
{
     ...
     * than not offering the mode at all.  Reopening ALP_E1M_WDT0 here
     * (same wdt_id, right after the close() above) is safe: the
     * dispatcher's per-wdt_id exclusivity (#1650) already guarantees
     * no other handle owns it, and the Zephyr backend reclaims a
     * still-armed device from the prior handle automatically. */
	int        cookie  = 42;

...
void autopilot_attitude_loop(autopilot_state_t *s)
{
	...
	while (1) {
         ...
         * (GPS-HOLD/RTH). */
		if (s->mode == AP_MODE_STABILISE || s->mode == AP_MODE_GPS_HOLD || s->mode == AP_MODE_RTH) {
			const float roll_sp =
...
void autopilot_nav_loop(autopilot_state_t *s)
{
	...
	while (1) {
		...
		if (s_gps_uart) {
			uint8_t line[128];
			size_t  got;
			(void)ublox_neo_m9n_read_nmea_line(&s_gps, line, sizeof(line), &got, /*timeout_ms=*/20);
		}
...
void autopilot_emit_motors(const autopilot_state_t *s)
{
	for (int i = 0; i < AUTOPILOT_N_MOTORS; i++) {
		...
		const uint32_t pulse_ns = (uint32_t)(1000000.f + s->motor_cmd[i] * 1000000.f);

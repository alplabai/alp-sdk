...
with its parallel `E1M_GPIO_<class><N>` index, and the board DTS muxes
the pad to GPIO.

The E1M-EVK has no dedicated GPIO LED; its only user LEDs are the RGB
cluster on the PWM pads.  So this demo:

- reads the **encoder push switch** on `E1M_GPIO_IO4` (active-low) as
  the button, and
- drives the **RGB-red pad** (default function PWM3) as a digital GPIO
  via `E1M_GPIO_PWM3` (index 29) as the LED.

The portable rule (`e1m_pinout.h` "Pin-as-GPIO fallback"): open a pad's
analog/timer function with its peripheral id (`E1M_PWM3` ->
`alp_pwm_open`), or claim it as a GPIO with its `E1M_GPIO_<class><N>`
index (`E1M_GPIO_PWM3` -> `alp_gpio_open`).  Never hold both against
...
- [`<alp/e1m_pinout.h>`](../../../include/alp/e1m_pinout.h) -- the
  `E1M_GPIO_<class><N>` pin-as-GPIO indices + the positional
  pin-array invariant.

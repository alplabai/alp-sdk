 ...
 * (boards/alp_e1m_aen801_m55_he_ae822fa0e5597ls0_rtss_he.overlay). Was index
 * [3] until the overlay's unused READY entry (index [2]) was removed -- see
 * that overlay's P2_6 comment. A native Alif GPIO, NOT a CC3501E-proxied
 * one -- unlike the mux ENABLE above, this pin is reached through the
 * ordinary Alif GPIO backend (gpio14), so it needs no bridge and no
 * route-table entry. Raw index, not an ALP_E1M_GPIO_* macro, for the same
 * reason CC3501E_BRIDGE_PIN_* are raw indices: this pin is SoM-internal,
 * not an E1M edge pad. */

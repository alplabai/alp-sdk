 ...
 *
 * An earlier version of this header said the link then stayed dead through a
 ...
 * Flow C image was loaded over an already-running app, which double-faults
 * the host core into lockup before a single console byte is emitted
 * ("clearing lockup after double fault", pc 0xeffffffe).  The host core was
 * dead, not the link.  A later run from a verifiably cleared buffer had a
 * warm nRESET pulse revive the link on the first attempt.  Cold-cycle
 * between Flow C runs, or that trap reads exactly like a dead bridge -- it
 * has now done so twice in this campaign.
 *
...
int main(void)
{
	 ...
	 *    on the R2 module this bench currently holds: Alif P2_6 there is

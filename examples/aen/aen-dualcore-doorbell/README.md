...
- `RESULT SKIP: dualcore-doorbell -- ...` — the peer never showed within a
  bounded window: no `mproc_boot` backend was selected (`ALP_ERR_NOSUPPORT`)
  and HE was never released, HE released but never rang, HE's own MHU-1
...
- `RESULT FAIL: alp_mproc_boot_core rc=%d` — HP only: a real local error,
  ...
  `ALP_ERR_NOSUPPORT`); a real SE refusal comes back as `ALP_ERR_IO` and lands
  here, not in the skip above.

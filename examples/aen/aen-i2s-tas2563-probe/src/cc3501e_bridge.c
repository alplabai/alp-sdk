 ...
 *
 * E1M-AEN SoM CC3501E bridge bring-up helper -- copied verbatim from
 * examples/aen/aen-cc3501e-gpio/src/cc3501e_bridge.c. See cc3501e_bridge.h.
 */
...
/*
 * AEN LP-pad mux (Alif Ensemble E8, M55-HE). WIFI_EN (P15_5) and nRESET
 * (P15_1) are on the Alif LP-GPIO island, bound by the generic
 * snps,designware-gpio driver which does NOT apply Alif pinctrl -- so the LP
 * pads stay un-muxed and their output drivers OFF unless poked directly.
 * 0x23 = the Alif GPIO-output pad config (driver + read-enable + drive
 * strength).
 */
...
alp_status_t cc3501e_bridge_bringup(cc3501e_t *fw)
{
	...
	/* 1. Control pins: WIFI_EN (supply gate) + nRESET. These are Alif
	 *    LP-GPIO pads, not E1M edge pads -- the SoM owns them. */
	 ...
	 *    muxed on P14_7; the app passes ALP_SPI_NO_CS so no software GPIO
	 *    CS is installed and the SPI controller drives SS0 per transfer.
	 *    Mode 0 matches the CC3501E vendor image frameFormat. */
	...
	/* Raise the SCLK past 1 MHz -- see cc3501e_bridge.h for the derivation. */
	{
	...
	/* 3. Bind the bus + control pins; attach the GPIO proxy (proxied E1M
	 *    IOs then route over the bridge); run the power + reset sequence.
	 *    Leaves WIFI_EN HIGH. */
	(void)cc3501e_init(fw, spi);
	fw->enable_pin = wifi_en;
	fw->reset_pin  = nrst;
	/* Optional host-IRQ/READY line -- not wired by this app's overlay
	 * (2-entry alp_pins array), so this returns NULL and the driver keeps
	 * the fixed-gap fallback. */

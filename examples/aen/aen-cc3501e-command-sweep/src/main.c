 ...
 * ORDER
 * -----
 * Bring-up first (STEP 1), then META, then the remaining families in
 * ascending opcode order (WIFI 0x1x, SOCKETS 0x2x, BLE 0x3x, OTA 0x4x,
 * STREAM_WRITE 0x45, GPIO 0x5x, SPI1 0x55-0x57, CAMERA/POWER 0x6x,
 * DIAGNOSTICS 0x7x last, so its frame counters tally the WHOLE sweep). Where
 * a family has an enable/disable pair the disable half runs before the next
 * family starts, so no family inherits radio/session state a sibling left
 * armed -- e.g. BLE_GATT_REGISTER runs BEFORE BLE_ADV_START (NimBLE refuses
 * a register while advertising) and BLE_DISABLE is the family's last call.
 *
...
/*
 * ======================================================================
 * PART 0 -- shared reporting + coverage-tracking plumbing
 * ======================================================================
 *
...
/** Print one INVOKED opcode's result. @p verdict is a short caller-chosen
 *  label -- "OK", "REFUSED" (an expected refusal given this bench's actual
 *  hardware/credentials), or "FAIL" (anything else) -- decided by the call
 *  site from the ACTUAL @p rc against what that specific opcode can
 *  legitimately return here, never inferred generically: a -2
 *  (ALP_ERR_NOT_READY) means something different on WIFI_GET_RSSI (not
 *  associated -- expected) than it would on PING (link not initialised --
 *  a real fault), so only the call site that knows which opcode it is can
 *  tell the two apart. */
...
{
	sweep_mark(op);
	printk("  0x%02X %-19s rc=%-3d  %-7s  %s\n", (unsigned)op, name, (int)rc, verdict, note);
}

/** Print one deliberately-SKIPPED opcode + why. */
static void sweep_skip(alp_cc3501e_cmd_t op, const char *name, const char *reason)
{
	sweep_mark(op);
	printk("  0x%02X %-19s SKIPPED   %s\n", (unsigned)op, name, reason);
}
...
int main(void)
{
	...
	printk("\n--- META (0x00-0x06) ---\n");
	{
		...
		sweep_report(ALP_CC3501E_CMD_PING, "PING", r, (r == ALP_OK) ? "OK" : "FAIL", "");
		...
		sweep_report(ALP_CC3501E_CMD_GET_MAC, "GET_MAC", r, (r == ALP_OK) ? "OK" : "FAIL", "");

		alp_cc3501e_diag_info_t diag = { 0 };
		r                            = cc3501e_diag_info(&fw, &diag);
		sweep_report(
		    ALP_CC3501E_CMD_GET_DIAG_INFO, "GET_DIAG_INFO", r, (r == ALP_OK) ? "OK" : "FAIL", "");
	 ...
	 * anywhere. This is also why Part 2 cannot measure SOCK_SEND/SOCK_RECV
	 * throughput -- see that section. */
	printk("\n--- SOCKETS (0x20-0x26) ---\n");
	{
		uint16_t     srv = 0u;
		...
		r = (open_client == ALP_OK) ? cc3501e_sock_connect(&fw, cli, k_test_peer_ip, 9u, 2000u)
		                            : open_client;
		sweep_report(ALP_CC3501E_CMD_SOCK_CONNECT,
		             ...
		             " here is the expected shape, not a link fault");
	...
	printk("\n--- BLE (0x30-0x3B) ---\n");
	{
		...
		sweep_report(
		    ALP_CC3501E_CMD_BLE_ENABLE, "BLE_ENABLE", r, (r == ALP_OK) ? "OK" : "FAIL", "");
	 ...
	 * across sizes runs in Part 2 below. */
	printk("\n--- STREAM (0x45), coverage ---\n");
	{
		...
		sweep_report(ALP_CC3501E_CMD_STREAM_WRITE,
		             ...
		             r,
		             (r == ALP_OK) ? "OK" : "FAIL",
		             "64 B coverage probe -- see Part 2 for the full size sweep");
	}
	 ...
	 * full size sweep runs in Part 2. */
	printk("\n--- SPI1 host passthrough (0x55-0x57), coverage ---\n");
	{
		...
		sweep_report(ALP_CC3501E_CMD_SPI1_TRANSFER,
		             ...
		             " connector -- see Part 2 for the full size sweep");
	 ...
	 * WHOLE sweep's frame counters -- not just this one family's traffic. */
	printk("\n--- Diagnostics (0x70-0x71), final tally ---\n");
	{
	 ...
	 * frame sizes. SOCK_SEND/SOCK_RECV are NOT measured here -- see the
	 * note printed below for why.
	 * =================================================================
	 */
	printk("\n=== PART 2: THROUGHPUT ===\n");
	...
	printk("\n--- SPI1_TRANSFER throughput (0x56) ---\n");
	{
		...
		if (cfg != ALP_OK) {
			printk("  SPI1_CONFIGURE -> %d -- cannot sweep SPI1_TRANSFER without a"
			       ...
			       " for as a coverage FAIL/REFUSAL above)\n",
			       (int)cfg);
		} else {
	...
	printk("  No Wi-Fi association exists on this run (WIFI_CONNECT_STA / WIFI_AP_START"
	       ...
	       " route to a real peer. Part 1's SOCKETS coverage above already shows"

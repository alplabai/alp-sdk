...
void hud_ui_apply_telemetry(const drone_telemetry_t *t)
{
    ...
    snprintf(buf, sizeof(buf), "R:%+6.1f P:%+6.1f Y:%6.1f",
             (double)t->roll_deg, (double)t->pitch_deg, (double)t->yaw_deg);
    lv_label_set_text(s_lbl_attitude, buf);

    /* GPS. */
    if (t->gps_fix) {
        snprintf(buf, sizeof(buf), "GPS %d sats\n%.5f, %.5f",
                 (int)t->sat_count, (double)t->lat_deg, (double)t->lon_deg);
    } else {
        snprintf(buf, sizeof(buf), "GPS: %d sats (no fix)",
                 (int)t->sat_count);
    }
    ...
    snprintf(buf, sizeof(buf), "BAT %5.2fV %+5.2fA\nremain %3ld min",
             (double)t->battery_v, (double)t->battery_a,
    ...
    snprintf(buf, sizeof(buf), "MODE: %s",
             names[t->mode <= DRONE_MODE_GPS_HOLD ? t->mode : 0]);

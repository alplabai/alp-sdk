/*
 * u8g2 font glyph table -- u8g2_font_6x10_tr, extracted verbatim from
 * u8g2's csrc/u8g2_fonts.c (upstream: https://github.com/olikraus/u8g2,
 * public domain per the font's own header comment below).
 *
 * u8g2_fonts.c bundles EVERY font u8g2 ships (tens of MB, ~700 tables)
 * in one translation unit; compiling it wholesale to reach a single 6x10
 * ASCII font is impractical for a teaching example. `u8g2_font_6x10_tr`
 * is already `extern`-declared in u8g2.h (every font symbol is, whether
 * or not u8g2_fonts.c is linked), so defining just this one array here
 * satisfies the link without touching the rest of the font table.
 */

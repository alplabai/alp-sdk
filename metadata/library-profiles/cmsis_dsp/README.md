...
[`metadata/libraries/cmsis-dsp.yaml`](../../libraries/cmsis-dsp.yaml)
pins the per-capability accelerator bindings: the SIMD axis
(Helium MVE / NEON), TMU CORDIC + FFT, and the ADC stream-DMA path,
with scalar libm as the required SW fallback.  The loader resolves
each axis against the SoM's `capabilities:` block and emits the
matching `CONFIG_ALP_CMSIS_DSP_*=y`.  CMSIS-DSP's

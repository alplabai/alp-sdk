...
alp_status_t alp_button_led_init(alp_button_led_t *bl, const alp_button_led_config_t *cfg)
{
	...
	alp_status_t s = alp_gpio_configure(
	    bl->button, ALP_GPIO_INPUT, cfg->active_low_button ? ALP_GPIO_PULL_UP : ALP_GPIO_PULL_DOWN);
...
alp_button_led_set_press_callback(alp_button_led_t *bl, alp_button_led_press_cb_t cb, void *user)
{
	...
	const alp_gpio_edge_t edge =
	    bl->active_low_button ? ALP_GPIO_EDGE_FALLING : ALP_GPIO_EDGE_RISING;

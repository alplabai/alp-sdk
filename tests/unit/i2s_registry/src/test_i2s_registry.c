...
ZTEST(alp_i2s_registry, test_selector_prefers_zephyr_drv_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("i2s", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_i2s_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("i2s", "fictional:soc:zz");
    zassert_not_null(be);
...
{
    for (const alp_backend_t *be = __start_alp_backends_i2s;
         be < __stop_alp_backends_i2s; ++be) {
...
ZTEST(alp_i2s_registry, test_sw_fallback_round_trip)
{
    ...
    uint8_t rx[8] = { 0u };
    size_t got = 0xFFu;
    zassert_equal(ops->read(st, rx, sizeof(rx), &got, 10u),
                  ALP_ERR_NOSUPPORT);

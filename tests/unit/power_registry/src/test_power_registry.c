...
{
	const alp_backend_t *be = alp_backend_select("power", "fictional:soc:zz");
	zassert_not_null(be);
	if (IS_ENABLED(CONFIG_ALP_SDK_POWER_PM_POLICY)) {
		zassert_equal(strcmp(be->vendor, "zephyr"), 0);
		zassert_equal(be->priority, 100);
...
ZTEST(alp_power_registry, test_power_request_sleep_inval_guards)
{
	...
	zassert_equal(alp_power_request_sleep(h, ALP_POWER_MODE_RUN, 1000u, NULL), ALP_ERR_INVAL);
	zassert_equal(alp_power_request_sleep(h, (alp_power_mode_t)99, 1000u, NULL), ALP_ERR_INVAL);
	zassert_equal(alp_power_request_sleep(h, ALP_POWER_MODE_SLEEP, 0u, NULL), ALP_ERR_INVAL);
...
ZTEST(alp_power_registry, test_renesas_vendor_ext_returns_inval_on_null_handle)
{
	...
	zassert_equal(alp_renesas_power_supervisor_mode_set(NULL, 1u), ALP_ERR_INVAL);
}
...
ZTEST(alp_power_registry, test_renesas_vendor_ext_rejects_non_renesas_backend)
{
	...
	zassert_equal(alp_renesas_power_supervisor_mode_set(h, 1u), ALP_ERR_NOT_PRESENT_ON_THIS_SOC);
	zassert_equal(alp_renesas_power_supervisor_mode_set(h, 0u), ALP_ERR_NOT_PRESENT_ON_THIS_SOC);
	/* Higher mode values are also rejected by the same gate -- the
     * gate runs before any payload validation. */
	zassert_equal(alp_renesas_power_supervisor_mode_set(h, 99u), ALP_ERR_NOT_PRESENT_ON_THIS_SOC);

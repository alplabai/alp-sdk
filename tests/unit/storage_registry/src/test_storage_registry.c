...
ZTEST(alp_storage_registry, test_selector_picks_sw_fallback_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("storage", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_storage_registry, test_configure_inline_aes_rejects_bad_inputs)
{
     ...
     * check fires first). */
    zassert_equal(alp_storage_configure_inline_aes(NULL, NULL),
                  ALP_ERR_INVAL);
    ...
    zassert_equal(alp_storage_configure_inline_aes(NULL, &bad_mode),
                  ALP_ERR_INVAL);
    ...
    zassert_equal(alp_storage_configure_inline_aes(NULL, &null_key),
                  ALP_ERR_INVAL);
    ...
    zassert_equal(alp_storage_configure_inline_aes(NULL, &bad_kb),
                  ALP_ERR_INVAL);
    ...
    zassert_equal(alp_storage_configure_inline_aes(NULL, &off_cfg),
                  ALP_ERR_NOT_READY);
}
...
{
    for (const alp_backend_t *be = __start_alp_backends_storage;
         be < __stop_alp_backends_storage; ++be) {
...
ZTEST(alp_storage_registry, test_vendor_ext_gates_non_matching_backends)
{
    ...
    zassert_equal(alp_alif_storage_secaes_key_provision(NULL, key16, 16u),
                  ALP_ERR_INVAL);
    zassert_equal(alp_nxp_storage_otfad_provision(NULL, 0u, key16, iv16),
                  ALP_ERR_INVAL);

    uint32_t status_out = 0u;
    zassert_equal(alp_alif_storage_secaes_get_status(NULL, &status_out),
                  ALP_ERR_INVAL);
    zassert_equal(alp_nxp_storage_otfad_set_window(NULL, 0u, 0u, 1024u),
                  ALP_ERR_INVAL);
...
ZTEST(alp_storage_registry, test_otfad_window_alignment_validation)
{
    ...
    zassert_equal(alp_nxp_storage_otfad_set_window(&h, 99u, 0u, 1024u),
                  ALP_ERR_INVAL);
    /* end <= start -> INVAL. */
    zassert_equal(alp_nxp_storage_otfad_set_window(&h, 0u, 1024u, 1024u),
                  ALP_ERR_INVAL);
    /* Misaligned start -> INVAL. */
    zassert_equal(alp_nxp_storage_otfad_set_window(&h, 0u, 512u, 1024u),
                  ALP_ERR_INVAL);
    /* Misaligned end -> INVAL. */
    zassert_equal(alp_nxp_storage_otfad_set_window(&h, 0u, 0u, 1500u),
                  ALP_ERR_INVAL);

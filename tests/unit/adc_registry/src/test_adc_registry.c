...
{
	const alp_backend_t *be = alp_backend_select("adc", "alif:ensemble:e7");
	zassert_not_null(be);
...
ZTEST(alp_adc_registry, test_alif_set_trigger_rejects_non_alif_handle)
{
	alp_adc_t *h = _make_fake_handle("fictional:soc:zz");
	zassert_not_null(h);
	alp_status_t rc = alp_alif_adc_set_trigger_source(h, ALP_ALIF_ADC_TRIGGER_TIMER0);
...
ZTEST(alp_adc_registry, test_alif_set_trigger_rejects_out_of_range_enum)
{
	alp_adc_t *h = _make_fake_handle("alif:ensemble:e7");
	zassert_not_null(h);
	alp_status_t rc = alp_alif_adc_set_trigger_source(h, (alp_alif_adc_trigger_t)99);
	zassert_equal(rc, ALP_ERR_INVAL);
}

ZTEST(alp_adc_registry, test_vendor_ext_null_handle_returns_inval)
{
	zassert_equal(alp_alif_adc_set_trigger_source(NULL, ALP_ALIF_ADC_TRIGGER_SOFTWARE),
	              ALP_ERR_INVAL);
}

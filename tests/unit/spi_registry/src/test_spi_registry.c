...
ZTEST(alp_spi_registry, test_selector_prefers_zephyr_drv_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("spi", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_spi_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("spi", "fictional:soc:zz");
    zassert_not_null(be);
...
extern const alp_backend_t __start_alp_backends_spi[];
extern const alp_backend_t __stop_alp_backends_spi[];

static const alp_spi_ops_t *_find_sw_fallback_ops(void)
{
    for (const alp_backend_t *be = __start_alp_backends_spi;
         be < __stop_alp_backends_spi; ++be) {

...
ZTEST(alp_gpio_registry, test_selector_prefers_zephyr_drv_on_alif)
{
	...
	const alp_backend_t *be = alp_backend_select("gpio", "alif:ensemble:e7");
	zassert_not_null(be);
...
ZTEST(alp_gpio_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
	...
	const alp_backend_t *be = alp_backend_select("gpio", "fictional:soc:zz");
	zassert_not_null(be);
...
{
	for (const alp_backend_t *be = __start_alp_backends_gpio; be < __stop_alp_backends_gpio; ++be) {
...
ZTEST(alp_gpio_registry, test_sw_fallback_round_trip)
{
	...
	zassert_equal(ops->enable_irq(st, ALP_GPIO_EDGE_RISING, NULL, NULL), ALP_ERR_NOSUPPORT);

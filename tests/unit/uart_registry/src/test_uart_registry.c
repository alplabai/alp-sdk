...
ZTEST(alp_uart_registry, test_selector_prefers_zephyr_drv_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("uart", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_uart_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("uart", "fictional:soc:zz");
    zassert_not_null(be);
...
extern const alp_backend_t __start_alp_backends_uart[];
extern const alp_backend_t __stop_alp_backends_uart[];

static const alp_uart_ops_t *_find_sw_fallback_ops(void)
{
    for (const alp_backend_t *be = __start_alp_backends_uart;
         be < __stop_alp_backends_uart; ++be) {
...
ZTEST(alp_uart_registry, test_rx_ringbuf_returns_nosupport_on_sw_fallback)
{
    ...
    alp_uart_rx_ringbuf_t *rb =
        alp_uart_rx_ringbuf_attach(&h, backing, sizeof(backing));
    zassert_is_null(rb);

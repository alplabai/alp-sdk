...
ZTEST(alp_pwm_registry, test_selector_prefers_zephyr_drv_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("pwm", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_pwm_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("pwm", "fictional:soc:zz");
    zassert_not_null(be);
...
extern const alp_backend_t __start_alp_backends_pwm[];
extern const alp_backend_t __stop_alp_backends_pwm[];

static const alp_pwm_ops_t *_find_sw_fallback_ops(void)
{
    for (const alp_backend_t *be = __start_alp_backends_pwm;
         be < __stop_alp_backends_pwm; ++be) {
...
ZTEST(alp_pwm_registry, test_sw_fallback_round_trip)
{
    ...
    zassert_equal(ops->configure(st, ALP_PWM_ALIGN_EDGE, 0u, 0u),
                  ALP_ERR_NOSUPPORT);
    ...
    zassert_equal(ops->capture_open(&cap_cfg, &cap_st, &cap_caps),
                  ALP_ERR_NOSUPPORT);

    uint32_t period_ns = 0u, pulse_ns = 0u;
    zassert_equal(ops->capture_read(&cap_st, &period_ns, &pulse_ns),
                  ALP_ERR_NOSUPPORT);

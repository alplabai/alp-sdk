...
ZTEST(alp_can_registry, test_selector_prefers_zephyr_drv_on_alif)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("can", "alif:ensemble:e7");
    zassert_not_null(be);
...
ZTEST(alp_can_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
    ...
    const alp_backend_t *be =
        alp_backend_select("can", "fictional:soc:zz");
    zassert_not_null(be);
...
{
    for (const alp_backend_t *be = __start_alp_backends_can;
         be < __stop_alp_backends_can; ++be) {
...
ZTEST(alp_can_registry, test_sw_fallback_round_trip)
{
    ...
    int32_t fid = -1;
    zassert_equal(ops->add_filter(st, &filter, _rx_cb_noop, NULL, &fid),
                  ALP_ERR_NOSUPPORT);

...
ZTEST(alp_registry, test_probe_refines_caps_per_instance)
{
	...
	zassert_true(alp_capabilities_has(demo_capabilities(&h0), ALP_INSTANCE_CAP_DMA));
...
{
	const alp_backend_t *be = alp_backend_select("demo", "fictional:stub:target");
	zassert_not_null(be);
...
{
	const alp_backend_t *be = alp_backend_select("tb_prio", "vendor:soc:t1");
	zassert_not_null(be);
...
{
	const alp_backend_t *be = alp_backend_select("tb_twoexact", "vendor:soc:t3a");
	zassert_not_null(be);
	zassert_equal(be->priority, 100);
	zassert_equal(strcmp(be->vendor, "alif"), 0, "expected alif (alphabetic), got %s", be->vendor);
}

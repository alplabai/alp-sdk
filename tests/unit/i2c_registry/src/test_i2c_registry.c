...
ZTEST(alp_i2c_registry, test_selector_prefers_zephyr_drv_on_alif)
{
	...
	const alp_backend_t *be = alp_backend_select("i2c", "alif:ensemble:e7");
	zassert_not_null(be);
...
ZTEST(alp_i2c_registry, test_selector_falls_back_to_sw_when_only_fallback_present)
{
	...
	const alp_backend_t *be = alp_backend_select("i2c", "fictional:soc:zz");
	zassert_not_null(be);
...
{
	for (const alp_backend_t *be = __start_alp_backends_i2c; be < __stop_alp_backends_i2c; ++be) {

...
ZTEST(alp_lib_knobs, test_sw_fallback_knobs_defined)
{
	const int reachable =
	    ...
	    _ALP_REQUIRE_KCONFIG(ALP_MADGWICK_LIBM) + _ALP_REQUIRE_KCONFIG(ALP_U8G2_SW_BLIT) +
	    _ALP_REQUIRE_KCONFIG(ALP_GFX_COMPAT_SW) + _ALP_REQUIRE_KCONFIG(ALP_JSMN_SW) +
	    _ALP_REQUIRE_KCONFIG(ALP_CATCH2_SW) + _ALP_REQUIRE_KCONFIG(ALP_NANOPB_SW) +
	    _ALP_REQUIRE_KCONFIG(ALP_COAP_NO_TLS) + _ALP_REQUIRE_KCONFIG(ALP_LWS_NO_TLS) +
	    _ALP_REQUIRE_KCONFIG(ALP_MODBUS_SYNC_IO) + _ALP_REQUIRE_KCONFIG(ALP_PID_INT_MATH);

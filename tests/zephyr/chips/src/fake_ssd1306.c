...
static void log_append(uint8_t *buf, size_t *len, size_t cap, const uint8_t *src, size_t n)
{
	size_t free = cap - *len;
	if (n > free) n = free;
...
static int fake_ssd1306_init(const struct emul *target, const struct device *parent)
{
	(void)parent;
	struct fake_ssd1306_data *d = target->data;
	g_fake_ssd1306              = d;

...
ZTEST(alp_mproc, test_frame_encode_decode_roundtrip)
{
	...
	size_t        framed_len = 0;
	zassert_equal(
	    alp_mproc_frame_encode(42, payload, sizeof(payload), framed, sizeof(framed), &framed_len),
	    ALP_OK);
	...
	zassert_equal(alp_mproc_frame_decode(framed, framed_len, &seq, &payload_out, &payload_len),
	              ALP_OK);
...
ZTEST(alp_mproc, test_frame_encode_zero_payload_succeeds)
{
	...
	size_t  framed_len = 0;
	zassert_equal(alp_mproc_frame_encode(1, NULL, 0, framed, sizeof(framed), &framed_len), ALP_OK);
...
ZTEST(alp_mproc, test_frame_encode_null_out_invalid)
{
	...
	zassert_equal(alp_mproc_frame_encode(1, "abc", 3, NULL, 999, &framed_len), ALP_ERR_INVAL);
...
{
	uint8_t framed[32];
	zassert_equal(alp_mproc_frame_encode(1, NULL, 4, framed, sizeof(framed), NULL), ALP_ERR_INVAL);
}
...
ZTEST(alp_mproc, test_frame_encode_capacity_short)
{
	...
	zassert_equal(alp_mproc_frame_encode(1, payload, sizeof(payload), framed, sizeof(framed), NULL),
	              ALP_ERR_NOMEM);
}
...
ZTEST(alp_mproc, test_frame_decode_short_frame_errors)
{
	...
	zassert_equal(alp_mproc_frame_decode(frame, sizeof(frame), NULL, NULL, NULL), ALP_ERR_IO);
}
...
ZTEST(alp_mproc, test_frame_decode_bad_magic_errors)
{
	...
	size_t        framed_len = 0;
	zassert_equal(
	    alp_mproc_frame_encode(1, payload, sizeof(payload), framed, sizeof(framed), &framed_len),
	    ALP_OK);
	framed[0] ^= 0xFFu;
	zassert_equal(alp_mproc_frame_decode(framed, framed_len, NULL, NULL, NULL), ALP_ERR_IO);
}
...
ZTEST(alp_mproc, test_frame_decode_declared_len_overflow_errors)
{
	...
	framed[14] = 0xBE;
	framed[15] = 0xEF;
	zassert_equal(alp_mproc_frame_decode(framed, sizeof(framed), NULL, NULL, NULL), ALP_ERR_IO);
}
...
ZTEST(alp_mproc, test_frame_decode_null_frame_invalid)
{
	...
	zassert_equal(alp_mproc_frame_decode(NULL, 0, &seq, &payload_out, &payload_len), ALP_ERR_INVAL);

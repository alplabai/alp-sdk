...
ZTEST(alp_iot, test_wifi_open_no_radio_returns_null) {
    ...
    zassert_equal(alp_last_error(), ALP_ERR_NOSUPPORT,
                  "expected NOSUPPORT, got %d", (int)alp_last_error());
}

ZTEST(alp_iot, test_wifi_connect_null_handle_errors) {
    alp_status_t s = alp_wifi_connect(NULL,
        &(alp_wifi_credentials_t){ .ssid = "alp-lab", .psk = NULL },
        1000);
...
ZTEST(alp_iot, test_mqtt_open_null_cfg_invalid) {
    ...
    zassert_equal(alp_last_error(), ALP_ERR_INVAL,
                  "NULL cfg must stamp INVAL, got %d", (int)alp_last_error());
}
...
ZTEST(alp_iot, test_mqtt_publish_null_handle_errors) {
    zassert_equal(alp_mqtt_publish(NULL, "topic",
                                   (uint8_t[]){0xaa}, 1,
                                   ALP_MQTT_QOS_0, false),
                  ALP_ERR_NOT_READY);
}

ZTEST(alp_iot, test_mqtt_subscribe_null_handle_errors) {
    zassert_equal(alp_mqtt_subscribe(NULL, "topic",
                                     ALP_MQTT_QOS_0,
                                     NULL, NULL),
                  ALP_ERR_NOT_READY);
}

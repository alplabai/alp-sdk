...
class TestAlpBoardDefineEmit(unittest.TestCase):
    ...
    def test_cmake_args_header_matches_materialise_twin(self) -> None:
        """The `--emit cmake-args` header must read "-- pass to cmake." --
        tan-cli's `_slice_config_artefact` twin
        (`python/tan/planner/buildplan.py`) writes this exact text to
        `build/<core>-baremetal/cmake-args.txt`, and `docs/board-config-emit.md`
        documents this SDK's `--output` as the way to produce that same
        file by hand; the two producers of one documented path must agree
        on more than just the `-D` lines."""
        rc, out, err = self._cmake_args(
            "E1M-AEN801", "e1m-evk", "a32_cluster", "baremetal")
        self.assertEqual(rc, 0, msg=err)
        self.assertIn(
            "# Auto-generated by scripts/alp_orchestrate.py -- pass to cmake.",
            out)

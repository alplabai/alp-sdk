...
Validates the SoM-swappability promise: given a fixed carrier
...
def test_compose_route_pad_listed_in_both_layers():
    """Carrier role + SoM dispatch combine into one composed route."""
    mod = _load_module()
    carrier_route = {
        "e1m": "E1M_GPIO_IO15",
        "role": "bmi323_int1",
    ...
    composed = mod._compose_route("E1M_GPIO_IO15", carrier_route, pad_routes)
    assert composed["dispatch"] == "cc3501e"
    assert composed["dispatch_pin"] == 14
    assert composed["carrier_role"] == "bmi323_int1"
    assert composed["carrier_macro"] == "EVK_PIN_BMI323_INT1"


def test_compose_route_pad_only_in_carrier_falls_back_to_direct():
    ...
    composed = mod._compose_route("E1M_GPIO_IO5", carrier_route, {})
    assert composed["dispatch"] == "direct"
    assert composed["carrier_role"] == "cam_rst"


def test_compose_route_som_swap_changes_dispatch_path():
    """The SoM-swappability promise in one assertion: a fixed carrier
    ...
    aen_composed = mod._compose_route("E1M_GPIO_IO15", carrier_route, aen_pad_routes)
    n93_composed = mod._compose_route("E1M_GPIO_IO15", carrier_route, n93_pad_routes)
    assert aen_composed["dispatch"] == "cc3501e"
    assert n93_composed["dispatch"] == "direct"
    # Carrier role identical in both -- this is the "swap modules without
    # changing the board" property.
    assert aen_composed["carrier_macro"] == n93_composed["carrier_macro"]
...
def test_compose_route_pad_in_neither_layer():
    ...
    assert composed["dispatch"] == "direct"
    assert composed.get("carrier_role") is None

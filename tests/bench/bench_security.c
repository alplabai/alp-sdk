...
void bench_security_main(void)
{
    ...
    BENCH_RUN("alp_hash_open(SHA256)", 1000000,
              { (void)alp_hash_open(ALP_HASH_SHA256); });

    /* alp_hash_update on a NULL handle -- the wrapper's NULL guard.
     * Same shape as every accessor in the suite. */
    BENCH_RUN("alp_hash_update(NULL)", 1000000,
              { (void)alp_hash_update(NULL, NULL, 0u); });
    ...
    BENCH_RUN("alp_aead_encrypt(NULL handle)", 1000000, {
        uint8_t ct[32]  = { 0 };
        uint8_t tag[16] = { 0 };
        (void)alp_aead_encrypt(NULL,
                               k_iv12, sizeof k_iv12,
                               NULL, 0u,
                               NULL, 0u,
                               ct, tag, sizeof tag);
    });
    ...
    BENCH_RUN("alp_aead_decrypt(NULL handle)", 1000000, {
        uint8_t pt[32] = { 0 };
        (void)alp_aead_decrypt(NULL,
                               k_iv12, sizeof k_iv12,
                               NULL, 0u,
                               NULL, 0u,
                               k_tag16, sizeof k_tag16,
                               pt);
    });
     ...
     * feature audit. */
    BENCH_RUN("alp_random_bytes(NULL out)", 1000000,
              { (void)alp_random_bytes(NULL, 16u); });
}

...
```text
  ...
  │               │    │     per-backend blobs:  Vela (Ethos-U) · DRP-AI · dxcom (DEEPX) · CPU/TFLM     │
  │               │    │                                                                                │
  │               │    │  Model families:  classification · detection (YOLO v5/v8) · segmentation ·     │
  │               │    │                   keyword-spotting · pose                                      │
  │               │    │                                                                                │
  │               │    │  →  the .alpmodel is loaded & dispatched at RUNTIME by the SDK (Inference ↓)   │
  └───────────────┘    └────────────────────────────────────────────────────────────────────────────────┘
          │
  ┌───────────────┐    ┌────────────────────────────────────────────────────────────────────────────────┐
  │ Dev Tooling   │ ─► │  board.yaml · alp_project.py (per-core emit) · alp_orchestrate.py              │
  │ (v0.6)        │    │  west alp-build / alp-image / alp-flash / alp-clean · VS Code extension        │
  │               │    │  validate_board_yaml.py · alp model build (.alpmodel) · program_eeprom.py      │
  └───────────────┘    └────────────────────────────────────────────────────────────────────────────────┘
          │
  ┌───────────────┐    ┌────────────────────────────────────────────────────────────────────────────────┐
  ...
  │               │    │                                                                                │
  │               │    │  Inference  ──  the .alpmodel runtime: this is where on-device AI runs         │
  │               │    │  ─ alp_inference_open_alpmodel()  loads the fat .alpmodel package              │
  │               │    │  ─ selects the matching blob:  silicon-ref + SRAM-fit + preferred_backend      │
  ...
  │               │    │  ─ LVGL · GPU2D/Dave2D      ─ BOARD_ID ADC            ─ <alp/tmu.h>, alp/power │
  │               │    │                                                                                │
  │               │    │  Heterogeneous IPC (v0.6):  <alp/rpc.h> · <alp/system_ipc.h> · <alp/mproc.h>   │
  │               │    │     framed RPMsg / OpenAMP · auto endpoint IDs · mailbox / shmem / hwsem       │
  │               │    │  Vendor escape hatches:  <alp/ext/{alif, renesas, nxp, deepx}>                 │
  │               │    │                                                                                │
  │               │    │  ── 80+ Tier-1 chip drivers: lsm6dso, bmi323, bmp581, icm42670, ina236,        │
  │               │    │        tmp112, tcal9538, rv3028c7, 24c128, cc3501e, ssd13xx, … + community     │
  │               │    │  ── User libraries (board.yaml libraries:):  ETL · fmt · nlohmann_json ·       │
  │               │    │        doctest · LVGL · MbedTLS · CMSIS-DSP · LittleFS                         │
  └───────────────┘    └────────────────────────────────────────────────────────────────────────────────┘
          │
  ┌───────────────┐    ┌────────────────────────────────────────────────────────────────────────────────┐
  │ OS            │ ─► │  Zephyr (M-class cores) · Yocto (A-class cores) · Bare-metal (no-RTOS)         │
  │ (per-core     │    │  heterogeneous = peers on the same SoM (selected per-core in cores:)           │
  │  slice)       │    │                                                                                │
  └───────────────┘    └────────────────────────────────────────────────────────────────────────────────┘
          │
  ┌───────────────┐    ┌────────────────────────────────────────────────────────────────────────────────┐
  │ Vendor SDK    │ ─► │  Alif Ensemble (AEN) · Renesas RZ/V2N · NXP i.MX 93 · DEEPX DX-M1              │
  │               │    │  NPU runtimes the SDK dispatches into:  Ethos-U/Vela · DRP-AI · DEEPX dx_rt    │
  └───────────────┘    └────────────────────────────────────────────────────────────────────────────────┘
          │
  ┌───────────────┐    ┌────────────────────────────────────────────────────────────────────────────────┐

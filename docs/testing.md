...
| Portable-class lifecycle contract — every `alp_<class>_open/close/capabilities` (13 classes) | `tests/zephyr/conformance/` — the data-driven conformance gate a new backend must pass; see the matrix in its `src/main.c` + `docs/porting-new-som.md` §11 | n/a (native_sim; real-SoM builds upgrade degrade rows to positive paths) |

...
- **Ethernet DMA buffers must live in global SRAM0, not the M55 DTCM.** The long
  ...
  Build-verified narrower follow-up (not yet re-benched on silicon): pin only
  the Ethernet-owned buffers into SRAM0 instead of moving all of main RAM,
  which is slower than DTCM for everything that is not Ethernet-DMA traffic
  and intermittently broke the ISP — the descriptor rings via the ethernet
  node's `memory-region` DT property, and the net_buf pool via
  `CONFIG_ETH_DWMAC_ALIF_NET_BUF_IN_DMA_REGION` relocating
  `subsys/net/ip/net_pkt.c` — while main RAM stays on DTCM. The same placement
  idea was prototyped on silicon in scratch bench run 200, but that scratch
  used a different app-level relocation, `CONFIG_NOCACHE_MEMORY=y`, and
  different ring addresses than this mechanism — it has not itself run on
  silicon yet.
...
| Ethernet links but never gets a lease / no traffic | GMAC DMA descriptor rings + net_buf pool are in the M55 **DTCM** (`zephyr,sram = &dtcm`), off the DMA bus. Bench-proven fix: whole-RAM `zephyr,sram = &sram0` + `CONFIG_DCACHE=n` (§3). Narrower, build-verified but not yet re-benched: give the ethernet node a `memory-region` + enable `CONFIG_ETH_DWMAC_ALIF_NET_BUF_IN_DMA_REGION` + `CONFIG_DCACHE=n` (§3) so only the Ethernet-owned buffers move and main RAM stays on DTCM. Applies to any DMA-master block (GMAC/NPU/SDHC). |

...
| **SD card** (DWC SDHC) | 🔴 DISABLED *(HW defect, #2051)* | The EVK SDIO 74LVC157 mux (EN=IO20 / SEL=IO21, both CC3501E-side, U38/U39) has no high-impedance state -- its SoC-facing outputs are held low whenever the mux is powered, regardless of ENABLE, a confirmed hardware defect pending a component change on the 2626-R2 EVK. `sdhc0` stays `status = "disabled"` in the shared SoC dtsi rather than fight those pads. Not a code/Flow-D bug; not fixed by routing the mux or inserting a card. See `docs/boards/e1m-evk.md` and `examples/aen/aen-sdhc-probe`. |

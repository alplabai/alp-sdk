...
| **I2S TX** (`i2s3`) | ✅ PASS | Clocks the tone out with the 76.8 MHz audio clock. Audible amp output verified separately on `e1m-aen-evk-03` with U46 replaced by a 3257-type bus switch powered from `+3V3` (the as-built 74LVC157 is a one-way mux, never passes SoC→amp I2S at any VCC) + TAS2563 config; an open TDM clock-error latch and open issue #2146 (amp auto-shutdown after I2S stop) remain. |
...
| **SD card** (DWC SDHC) | ✅ PASS | SD works in 4-bit mode on `e1m-aen-evk-03` with U38/U39 replaced by 3257-type bus switches powered from `+3V3` (the standard fit going forward). EN=IO20, CC3501E-side on both hw revisions; SEL=IO21, CC3501E-side on r1 only -- unrouted/hardware-strapped on r2. |

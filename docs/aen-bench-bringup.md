...
| **PDM mics** | 🟡 PARTIAL *(gain re-verification pending)* | Round 4d (issue #2133, commit `68a169977`): 48 kHz mode 7 rate confirmed exact (`measured_rate_hz=48000`, `slab_missed=0`, `overrun=0`), and a 30 s attended clap test confirmed live audio on all 4 channels (peak p2p ch0=34 ch1=46 ch2=48 ch3=48). Gain confirmed ~40 dB too low at the old default (`CH0 GAIN=0x0000000D`); fixed by a new `channel-gain` DT property defaulting to `0x800` (issue #2143) -- silicon re-run with the corrected default pending. |

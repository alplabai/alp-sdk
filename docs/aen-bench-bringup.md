...
| **PDM mics** | 🟡 PARTIAL *(full-scale headroom unverified)* | Register-level configuration and 48 kHz mode 7 rate verified exact (`measured_rate_hz=48000`, `slab_missed=0`, `overrun=0`, commit `68a169977`). Acoustic capture at 48 kHz VERIFIED by a speaker-to-mic loopback (issue #2133 round 4f, `e1m-aen-evk-03`, 2026-09-15 14:49Z, TAS2563 speakers, gain `0x200` readback-confirmed): the 1 kHz Goertzel bin rose from ~9/0 dB in silence to ~58 dB at volume 48, a 500 Hz bin lit up only during the 500 Hz stimulus, p2p rose from 128 in silence to ~651 at 1 kHz/volume 48. Gain was `0x0D` (~40 dB too quiet); a round 4d fix to `0x800` clipped on silicon at the estimated full scale; now a provisional `0x200` default (issue #2143) -- full-scale headroom remains unverified (the loopback signal was only ~20 LSB pre-gain). |

...
| **Camera stack** (`cam`/`csi`/`dphy`/`arx3a0`) | ✅ PASS *(bind)* | All four nodes BIND + the v4.4-ported drivers load. No camera module was fitted during this pass; the J5 shields are now described and build-tested. **Update (2026-09-21, e1m-aen-evk-02): the `innomaker_cam_ov9281` shield's live capture is bench-verified** -- real 640x400 GREY8 frames, once a P/N-crossing camera-connector adapter is fitted (see `docs/boards/e1m-evk.md`'s Camera section) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` is set. IMX219/OV5647/IMX296 live capture remains BENCH-UNVERIFIED. |

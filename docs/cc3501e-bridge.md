...
| `wifi connect` (STA associate) | **~15 s association**; worst case, a never-associating AP blocks `companion_conn_thread` for the console's full `ALP_COMPANION_WIFI_CONN_MS = 75000u` (75 s) before it reports the failure — accounting-derived, not bench-measured; it was ~16.7 s before the #1481 poll-budget fix | **survives** — async connect, shell stays live; `ver` after the connect still returns (no desync, no power-cycle) |

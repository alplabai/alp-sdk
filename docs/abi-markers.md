...
| `ext/alif/storage.h`  | `[ABI-EXPERIMENTAL]` | v0.5 header+stub.  Issue #224: `alp_alif_storage_secaes_key_provision()` gained a real body over the hal_alif SE-service transport (`se_service_send_request`, `SERVICE_APPLICATION_OSPI_WRITE_KEY_ID`), gated on `CONFIG_ALP_SDK_STORAGE_ALIF_SECAES` -- UNVERIFIED ON SILICON (no OSPI SecAES-relevant part on any bench unit reachable at implementation time; see `src/backends/ext/alif/storage.c`).  `key_bytes` narrowed to 16-only (AES-128 is the only width the SE service accepts; the header previously and incorrectly documented 16/24/32).  `alp_alif_storage_secaes_get_status()` still unconditionally `ALP_ERR_NOSUPPORT` -- no vendor-published SE service reads the engine's status back. |

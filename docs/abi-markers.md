...
| `jpeg.h`              | `[ABI-EXPERIMENTAL]` | v0.13 new -- portable JPEG-encoder surface (`alp_jpeg_open/encode/capabilities/close`).  Task 1 shipped the class dispatcher + NOT_IMPLEMENTED stub; Task 2 adds a portable software baseline-JPEG backend (4:2:0 + mono, priority 50) that now encodes on every SoM without JPEG hardware -- the Alif E8 Hantro VC9000E hardware backend (Tasks 3-4) is still pending. |

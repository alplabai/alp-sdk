...
| `peripherals_unverified`        | Array of `peripherals` keys whose count has no datasheet/DFP/HWRM citation in this file (#936) — e.g. a value copied from a sibling part and never independently confirmed. `scripts/gen_soc_caps.py` prints an `UNVERIFIED` comment above the SoC's block in `soc_caps.h`; `validate_metadata.py` warns if a listed key doesn't exist in `peripherals`. A file whose WHOLE `peripherals` block is unaudited for this part (inherited wholesale, e.g. no datasheet exists yet) should use `pending_reference_manual_ingestion: true` instead of listing every key here. |

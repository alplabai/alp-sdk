...
| `helper_firmware[]`    | SoM extension                     | Yes (`TBD` per field)     | One entry per on-module helper MCU image (CC3511E firmware, GD32 bridge firmware, …).  Three INDEPENDENT axes: `flash_method`/`flash_args` (how it is written locally), `update_channel` (how it is updated in the field), `flash_policy` (who may invoke the flash method — `customer`/`factory`/`recovery_only`; **required** once an entry declares both of the other two).  A `swd_probe` entry naming `flash_args.target` must also name `flash_args.jlink_device` — see `metadata/e1m_modules/README.md`. |

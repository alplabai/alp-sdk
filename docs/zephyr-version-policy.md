...
| `hal_alif` Zephyr module             | **v2.3.0** (our own pin) | A **top-level project** in [`west.yml`](../west.yml) pinned from Alif's GitHub (`alifsemi` remote) -- Zephyr's own west.yml does **not** carry it, so we fetch and re-pin it ourselves on every `west update`.  Bumped deliberately with the Zephyr cycle. |

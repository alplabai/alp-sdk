...
| [0022](0022-python-executor-renode-retirement.md) | `tan` ships as Python (PyInstaller freeze of `alp-tan`), not Rust, from v0.5.0; Renode is retired from the command surface, no replacement simulator; narrowly supersedes 0020's Rust-executor language clause + its `renode` command-surface entries only | Accepted in part — amended 2026-08-07: the Python-executor clause shipped and stands; the Renode-retirement clause is **withdrawn** (`tan renode` is still registered at tan-cli v0.5.1, and tan-cli#448 was re-scoped from removal to retain-with-a-support-paused-warning), so 0020's `renode` command-surface entries are NOT superseded |

...
| [0014](0014-build-plan-emit-cli-contract.md) | The `alp` CLI consumes the orchestrator's emitted build plan (`--emit build-plan`), not a re-implemented planner | Accepted — partially superseded by 0020 (mechanism clause + 84-87 consequence only; contract properties still live) |
...
| [0020](0020-sdk-owns-build-execution.md) | The SDK plans; a standalone `tan` CLI (new repo, extracted from vscode) is the whole build command surface + sole executor — three repos; the six `west alp-{build,flash,image,size,renode,clean}` extensions + `fan_out` retired (the `alp` script and read-only `west alp-{emit,migrate,lock,quality}` remain); supersedes 0014 mechanism + 84-87; pairs with #837 | Accepted — direction + alp-sdk Phase 1/4 on `dev`, co-signed by Hakan; remediation met except the cross-repo `repository_dispatch` trigger, which stays outstanding as a maintainer action (see ADR Amendment 7) |

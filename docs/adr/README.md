...
| [0020](0020-sdk-owns-build-execution.md) | The SDK owns build execution (one Python executor, stays standalone-buildable, `west alp-build` native); a thin Rust manager above dispatches + renders. Phase-1-first/reversible; supersedes 0014's execute/schedule half only. Awaiting Hakan's co-sign | Proposed |

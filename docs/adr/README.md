...
| [0032](0032-licence-gated-vendor-data.md) | Licence-gated vendor data (e.g. the Alif Ensemble SVDs) is carried in a segregated subtree with the vendor's unmodified licence beside it and a `NOTICE` entry naming it -- NOT by extending `library-v1.schema.json` `license.enum`, which gates only what a project selects through `libraries:` and whose ADR 0018 guard would be widened to admit something that never travels that path. The root `NOTICE` currently states Alif/Renesas/NXP/DEEPX vendor material is NOT redistributed, so the carve-out is an amendment to a published representation and lands with the first file it describes, never ahead of it. Extends 0018 | Proposed |

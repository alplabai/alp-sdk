...
| [0020](0020-sdk-owns-build-execution.md) | A standalone `tan` CLI owns the command surface and execution; Python Tan now also carries the relocated in-process planner, while alp-sdk retains the reference emitters | Accepted — amended 2026-08-03 for the Python port; only `migrate`, `lock`, and `quality` still forward to west |

...
1. **E1M reservation** — `E1M_<CLASS>_COUNT` macros in
   `<alp/e1m_pinout.h>`.  This is the portability bound.  Apps that
   use `E1M_<CLASS><N>` for `N < E1M_<CLASS>_COUNT` are
...
**Good:**
- Apps choose: portable (E1M_*) or vendor-extended (raw integers
...
- Adding a new peripheral class to E1M v1.x is a one-line
  `#define E1M_<CLASS>_COUNT N` addition plus an ADR pointer to
  the spec change.
...
- `include/alp/e1m_pinout.h` — the `E1M_*_COUNT` definitions.

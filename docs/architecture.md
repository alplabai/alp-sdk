...
| Camera           | `alp/camera.h`       | Zephyr `video_*` API.  V2N MIPI CSI-2 wrapper in v0.2.          | v0.2 video stack (CPI / CSI-2 / D-PHY / ARX3A0) ported to Zephyr v4.4 + binds on E8; ISP-Pico ext + portable ISP config vendor-gated. A board-agnostic Raspberry-Pi-style CSI-2 OV9281 camera-module sensor driver + matching Zephyr shield ships alongside — bench-verified on an E1M-AEN803 on the E1M-EVK (2026-09-21) — see [camera-shields.md](camera-shields.md) |

...
Zephyr board files for Alp modules are generated by `scripts/alp_project.py
--input <board.yaml> --core <core_id> --emit zephyr-board --output <dir>`
(`scripts/gen_zephyr_board.py`, issue #523).  The Alif Ensemble (`aen`)
family is fully generated -- `board.yml`, `Kconfig.alp_<board>`, the
twister board `.yaml`, `_defconfig`, pinctrl `.dtsi`, `Kconfig.defconfig`,
...
| Camera           | `alp/camera.h`       | Zephyr `video_*` API.  V2N MIPI CSI-2 wrapper in v0.2.          | v0.2 video stack (CPI / CSI-2 / D-PHY / ARX3A0) ported to Zephyr v4.4 + binds on E8; ISP-Pico ext + portable ISP config vendor-gated. Three board-agnostic Raspberry-Pi-style CSI-2 camera-module sensor drivers (OV5647, OV9281, IMX296) + matching Zephyr shields ship alongside — OV9281 is bench-verified on an E1M-AEN803 on the E1M-EVK (2026-09-21); OV5647 is bench-attempted and BLOCKED at D-PHY Stop-state (issue #2248); IMX296 live capture remains bench-pending (no module seated) — see [camera-shields.md](camera-shields.md) |

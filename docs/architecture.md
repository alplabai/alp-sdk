...
| Camera           | `alp/camera.h`       | Zephyr `video_*` API.  V2N MIPI CSI-2 wrapper in v0.2.          | v0.2 video stack (CPI / CSI-2 / D-PHY / ARX3A0) ported to Zephyr v4.4 + binds on E8; live capture bench-pending (sensor wiring); ISP-Pico ext + portable ISP config vendor-gated. Two board-agnostic Raspberry-Pi-style CSI-2 camera-module sensor drivers (OV5647, OV9281) + matching Zephyr shields ship alongside — see [camera-shields.md](camera-shields.md) |

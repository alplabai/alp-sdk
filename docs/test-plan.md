...
| AEN801 peripheral matrix — PDM mics | `examples/aen/aen-pdm-mic-alif` | 🟡 partial | Register-level configuration verified. 48 kHz mode 7 capture rate verified exact (`measured_rate_hz=48000`) with no drops (`slab_missed=0`, `overrun=0`, commit `68a169977`). Acoustic capture UNVERIFIED -- a believed round 4d "clap test" was not attended (nobody clapped), so its recorded bursts are not evidence of live mics (issue #2133 round 4e correction). Gain was `0x0D` (~40 dB too quiet); a round 4d fix to `0x800` clipped on silicon; now a provisional `0x200` default (issue #2143) pending calibration and a controlled-stimulus re-run | same | v0.8 |

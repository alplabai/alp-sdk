...
| AEN801 peripheral matrix — SD card | DWC SDHC | 🟡 partial | DWC SDHC controller initialises but the card is unreachable -- **hardware-gated**: the EVK's SDIO mux (EN=IO20, CC3501E-side on both hw revisions; SEL=IO21, CC3501E-side on r1 only -- unrouted/hardware-strapped on r2) is not routed with a card inserted on this bench unit | same | v0.8 |

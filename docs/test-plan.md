...
| AEN801 peripheral matrix — I2S TX | `examples/aen/aen-i2s-amp-alif` (i2s3) | ✅ verified | i2s3 clocks a tone out at the 76.8 MHz audio clock (this example does not wire the amps). Separately, amp playback audibility is verified on `e1m-aen-evk-03` with U46 replaced by a 3257-type bus switch powered from `+3V3` (the as-built 74LVC157 can never pass this direction at any VCC) -- an open TDM clock-error latch and open issue #2146 (amp auto-shutdown after I2S stop) remain | same | v0.8 |
...
| AEN801 peripheral matrix — SD card | DWC SDHC | ✅ verified | SD works in 4-bit mode on `e1m-aen-evk-03` with U38/U39 replaced by 3257-type bus switches powered from `+3V3` (the standard fit going forward; the stock 74LVC157 mux never passes SD at any VCC, same directional failure as the I2S mux). EN=IO20, CC3501E-side on both hw revisions; SEL=IO21, CC3501E-side on r1 only -- unrouted/hardware-strapped on r2 | same | v0.8 |

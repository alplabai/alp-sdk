...
| AEN801 peripheral matrix — RPi-style CSI-2 camera first light (E1M-AEN803) | `examples/aen/aen-camera-firstlight` (cam/csi/dphy + IMX219/OV5647/OV9281/IMX296 on the E1M-EVK J5) | 🟡 partial (OV9281 verified) | OV9281: real GREY8 frames land in memory on an E1M-AEN803 on the E1M-EVK (2026-09-21), in all three modes (640x400, 1280x720, 1280x800), each at its configured frame rate (measured 60-frame bursts: ~100/~50/~100 fps), with the sensor test pattern also verified in all three -- needs the SoM's P/N-crossing camera-connector adapter (`docs/boards/e1m-evk.md`) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` (default on for the E8). OV5647: bench-attempted (2026-09-22, E1M-AEN803 serial 2026W36-0001 on E1M-EVK hw_rev 2626-r2, with the J5 pin-11 pull-up rework fitted) and BLOCKED -- `alp_camera_open` fails `ALP_ERR_TIMEOUT` at D-PHY Stop-state (`CSI_PHY_STOPSTATE` reaches only bit0; `CSI_PHY_RX` bit17 `RXCLKACTIVEHS` never sets), tracked as issue #2248; see `docs/camera-shields.md`'s OV5647 driver section. NOT bench-verified. IMX219/IMX296 compile + link against the real board target (`build_only` twister scenarios) but have not yet run on real silicon (no module seated) | same | v0.8 |

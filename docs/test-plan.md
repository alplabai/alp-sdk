...
| AEN801 peripheral matrix — PDM mics | `examples/aen/aen-pdm-mic-alif` | 🟡 partial | Register-level configuration verified. 48 kHz mode 7 capture rate verified exact (`measured_rate_hz=48000`) with no drops (`slab_missed=0`, `overrun=0`, commit `68a169977`). Acoustic capture at 48 kHz VERIFIED by a speaker-to-mic loopback (issue #2133 round 4f, `e1m-aen-evk-03`, 2026-09-15 14:49Z, TAS2563 speakers, gain `0x200` readback-confirmed): Goertzel-bin analysis found the 1 kHz bin rising from ~9/0 dB in silence to ~58 dB at volume 48, a 500 Hz bin only lighting up during the 500 Hz stimulus, and p2p rising from 128 in silence to ~651 at 1 kHz/volume 48. Gain was `0x0D` (~40 dB too quiet); a round 4d fix to `0x800` clipped on silicon; now a provisional `0x200` default (issue #2143) -- full-scale headroom at that gain is still UNVERIFIED (the loopback signal was only ~20 LSB pre-gain against an estimated ~900-1000 LSB pre-gain full scale) | same | v0.8 |

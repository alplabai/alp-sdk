...
| AEN801 peripheral matrix — RPi-style CSI-2 camera first light | `examples/aen/aen-camera-firstlight` (cam/csi/dphy + IMX219/OV5647/OV9281/IMX296 on the E1M-EVK J5) | 🟡 partial (OV9281 verified) | OV9281: real 640x400 GREY8 frames land in memory on e1m-aen-evk-02 (2026-09-21) -- needs the SoM's P/N-crossing camera-connector adapter (`docs/boards/e1m-evk.md`) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` (default on for the E8). IMX219/OV5647/IMX296 compile + link against the real board target (four `build_only` twister scenarios) but have not yet run on real silicon | same | v0.8 |

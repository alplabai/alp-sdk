...
| ISP (VeriSilicon ISP Pico (vsi,isp-pico)) vendor-ext surface | `src/backends/ext/alif/camera.c` + `<alp/ext/alif/camera.h>` (ISP Pico backend, priority 100 on E4/E6/E8) | ⏳ untested | 3A window / per-channel gain LUT / LSC MESH LUT take effect on real ISP Pico silicon (E4/E6/E8 — **not E7**); non-Alif handle returns `ALP_ERR_NOT_PRESENT_ON_THIS_SOC` | NULL/INVAL + vendor-handle-gate ZTESTs green in `pr-twister`; **HAL-backed ISP statistics readback** on ISP Pico silicon — AE/AF/LSC are gated on a hal_alif `isp_wrapper` version bump (see `zephyr/drivers/video/isp_pico.c`'s HAL_ALIF VERSION MISMATCH note); the gain path is contract-absent, not merely unwired (see `src/backends/ext/alif/camera.c`) | v0.5 (header+stub), real bodies gate v0.x |

...
| DesignWare I2S channel-enable model on the Alif E8 (issue #2205) | `zephyr/drivers/i2s/i2s_dw.c` + `zephyr/drivers/i2s/i2s_dw.h` | 🟡 partial | The register fact the fix rests on is measured on silicon: on the E8's `i2s3` (`0x49017000`) `TER` bit 0 (`TXCHENX`) and `RER` bit 0 (`RXCHENX`) cannot be cleared, even with the block idle, so `i2s_tx_channel_disable()` / `i2s_rx_channel_disable()` are no-ops there. The driver changes built on it (START-time pre-emption of the other direction, `dev_data->dir` following START, the full `IMR` mask at init, the widened ISR guard, read-modify-write channel enables) are host-unit-tested only and have not been re-run on silicon | `e1m-aen-evk-03`, Flow C RAM load of the archived #2179 fix-arm image (`e1ed4881a`, ELF md5 `d9aad40ed6d57addb1b1f06d646de6ee`), no MRAM writes (bookend at `0x80010000` byte-identical); hardware breakpoint at `0x9F72`, the first instruction after the init-time `i2s_tx_channel_disable()`, in `POST_KERNEL`: `IER 0x00000F00` (the SVD reset value), `IRER`/`ITER`/`CER` `0`, `RER` = `TER` = `0x00FFFF01` (the E8 SVD reset value, `LPI2S` `I2S_TER0`/`I2S_RER0`, bit 0 read-only). Straight after a RESETPIN reset every one of those read `0x00000000`, `IER` included -- an unclocked block, not a reset value. Host: `tests/unit/i2s_dw_underrun` on native_sim/native/64 | v0.17 |

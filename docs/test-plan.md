...
| `alp_wdt_close()` no longer disarms the whole device; `ALP_WDT_INTERRUPT_ONLY` gains `on_expire`/`user` | `include/alp/wdt.h` + `src/backends/wdt/{zephyr_drv,yocto_drv,sw_fallback}.c` + `src/wdt_dispatch.c` | 🟡 partial | On the Zephyr backend: `z_close()` no longer calls `wdt_disable(dev)`, so closing one handle no longer disarms a device a non-SDK Zephyr consumer (e.g. `CONFIG_TASK_WDT`) might share; a reopen of the same `wdt_id` still succeeds (`z_open()` reclaims a stale `wdt_setup()` with one `wdt_disable()` + retry on `-EBUSY`); an `ALP_WDT_INTERRUPT_ONLY` open with `on_expire` set actually invokes the callback in ISR context when the deadline is missed | `native_sim` has no `alp-wdt0` DT alias, so `zephyr_drv`'s `z_open()` always returns `NOT_READY` before reaching `wdt_install_timeout()` -- the reclaim path and the ISR trampoline are compiled/linked but never executed by any of this: local `twister -p native_sim/native/64` (host toolchain) run of `alp.unit.wdt_exclusivity` (3/3), the wdt cases in `tests/zephyr/peripheral/src/wdt.c` + `config_defaults.c` (9/9), `alp_sdk.conformance.portable_api` (11/11), and the `wdt-feed` example, all green; **needs-silicon**: whether the reclaim actually reopens a still-armed device, whether the ISR trampoline actually fires, and the close-vs-latched-interrupt race window all need an E1M-AEN801 or E1M-V2N101 bench run | v0.17 |

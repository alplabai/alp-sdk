...
| Feature | Module / file | Status | What "verified" means | Evidence | Gates |
|---|---|---|---|---|---|
| RPC link liveness (`alp_rpc_set_link_callback` / `alp_rpc_link_state`, `alp_rpc_send` rejects into a `LOST` link) | `include/alp/rpc.h` + `src/rpc_dispatch.c` + `src/backends/rpc/{zephyr_drv,yocto_drv,sw_fallback}.c` + `src/common/stub/stub_rpc.c` | ⏳ untested | A real far-core reset (V2N CM33 reset, or an AEN dual-core peer crash) is observed as `ALP_RPC_LINK_LOST` within a bounded time, without a false-positive LOST on a merely-busy link; `alp_rpc_send()` then fails `ALP_ERR_NOT_READY` instead of silently accepting | Dispatcher-level contract (`_rpc_op_enter`/`_rpc_op_leave` gating, `alp_rpc_notify_link()` updating state + invoking the registered callback, `alp_rpc_send()`'s LOST-rejects-early path) covered on native_sim in `tests/unit/rpc_registry/src/test_rpc_registry.c`.  The Zephyr backend's `rpc_ept_bound`/`rpc_ept_unbound`/`rpc_ept_error` wiring, including the `closing`-gate reuse via `rpc_recv_enter()`/`rpc_worker_leave()`, covered on native_sim in `tests/unit/rpc_zephyr_backend/src/test_rpc_zephyr_backend.c` (scenario 5) against a hand-driven `struct rpc_be`, not a real `ipc_service` transport.  `needs-silicon`: no real far-core reset has been driven against real V2N/AEN hardware in this change; which of `ipc_service`'s `unbound` vs `error` callback actually fires for a genuine far-core reset is unconfirmed | v0.17 |

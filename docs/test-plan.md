...
| AEN801 peripheral matrix — PDM mics | `examples/aen/aen-pdm-mic-alif` | 🟡 partial | Round 4d (issue #2133, commit `68a169977`): 48 kHz mode 7 capture rate confirmed exact (`measured_rate_hz=48000`) with no drops (`slab_missed=0`, `overrun=0`), and a separate attended clap test confirmed the mics are live on all 4 channels. Gain is confirmed ~40 dB too low at the old default (`CH0 GAIN=0x0000000D`); fixed by a new `channel-gain` DT property defaulting to `0x800` (issue #2143), but a silicon re-run with the corrected default is still pending | same | v0.8 |

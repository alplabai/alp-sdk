...
| AEN801 peripheral matrix — RPi-style CSI-2 camera first light (E1M-AEN803) | `examples/aen/aen-camera-firstlight` (cam/csi/dphy + OV5647/OV9281 on the E1M-EVK J5) | ✅ verified | OV9281: real GREY8 frames land in memory on an E1M-AEN803 on the E1M-EVK (2026-09-21), in all three modes (640x400, 1280x720, 1280x800), each at its configured frame rate (measured 60-frame bursts: ~100/~50/~100 fps), with the sensor test pattern also verified in all three -- needs the SoM's P/N-crossing camera-connector adapter (`docs/boards/e1m-evk.md`) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` (default on for the E8). CPI starvation-pause behaviour changed by issue #2287 after this bench (now a CPI-only pause instead of cycling the CSI-2 endpoint/sensor) -- re-bench pending. OV5647: bench-verified (2026-09-22, E1M-AEN803 serial 2026W36-0001 on E1M-EVK hw_rev 2626-r2, with the J5 pin-11 pull-up rework fitted), RAW10 640x480, issue #2248 -- see `docs/camera-shields.md`'s OV5647 driver section. CPI starvation-pause behaviour changed by issue #2287 after this bench (now a CPI-only pause instead of cycling the CSI-2 endpoint/sensor) -- re-bench pending. IMX296: Stage A bench-verified (issue #2287, E1M-AEN803 2026W36-0001), single-lane RAW10 1456x1088 -- I2C identity, CSI-2 streaming, and ONE full frame captured through `<alp/camera.h>` and correlated against a diag control capture (0.98, bench run 292 vs 293). Run 292's build had CONFIG_LOG unset (no log output at all), so INT_IPI_PIXEL_IF_HLINE_ERR/_FIFO_OVERFLOW status on the product build is UNKNOWN, not "not seen" -- the HLINE_ERR counts cited elsewhere were measured in earlier diag runs (logging enabled), before the fixes; frame rate not measured, and continuous streaming, ISP-Pico, AE and fast-trigger mode are not yet bench-verified for this sensor (later stage) | same | v0.8 |

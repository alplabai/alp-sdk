...
- AEN-Zephyr rows have no automated CI verifier -- there is no
  self-hosted HIL runner for the AEN bench (see
  `docs/ci/HW-IN-LOOP.md`).  Every AEN row flips to ✅ on an
  explicitly-invoked, labgrid-reservation-gated bench run against a
  real E1M EVK, with the result attached to the PR or release that
  needs it.
...
## Families not yet silicon-validated

You should **NOT** ship production firmware against a family with no
`✅ verified` row anywhere below unless you've done your own HIL.  As of
...
| `<alp/ble.h>` real impl | `src/backends/ble/zephyr_drv.c` + `src/backends/ble/cc3501e.c` | 🟡 partial | GATT server register + characteristic read/write, the client read/write context-pool's CAS-loss branch and its `ALP_ERR_BUSY` pool-exhaustion refusal (`z_gatt_read()`/`z_gatt_write()`, issue #1939) all host-tested under native_sim; the pool's timeout-abandon branch itself (`zephyr_drv.c` `z_gatt_read()`/`z_gatt_write()` "Deadline passed" `alp_lifecycle_cas()`, issue #1939) plus advertise + connect + client GATT read/write from a second BLE device over the air still need a real two-device bench proof | native_sim (server register/read/write + client-read callback + GATT ctx-pool CAS-loss + BUSY refusal, `tests/zephyr/ble_gatt_server`); HIL still pending for the timeout-abandon branch, OTA connect/read/write/scan, and the CC3501E provider | v0.3 |
...
| `validate_board_yaml.py` v0.3 capability cross-check | `scripts/validate_board_yaml.py` | 🟡 partial | Peripheral-vs-SoC-caps mismatch emits an `ALP-B010` warning; a warning-only run exits 0 (E1M-AEN801 + `peripherals: [emmc]`) -- the in-tree `ALP-B010` fixture (`tests/fixtures/board_yaml_bad/ALP-B010-peripheral-not-on-soc.yaml`) instead exits 5 on the unrelated hw_rev-buildable gate (#1025), not on this check | `tests/scripts/test_validate_board_yaml_entrypoints.py`'s `test_reports_per_core_peripheral_warning` | v0.3 |
...
| GD32 bridge firmware (`gd32-bridge-firmware:`) scaffold | `gd32-bridge-firmware:` | ⏳ untested | Build green with `BRIDGE_HAL_BACKEND=stub`; protocol round-trip via the host driver against the stub firmware on the GD32 silicon | HIL + smoke test | v0.3.x |
...
| Yocto I²S backend (ALSA) | `src/backends/i2s/yocto_drv.c` (+ `src/i2s_dispatch.c`) | 🟡 code-complete, link-unverified | An unsupported sample rate refuses at `alp_i2s_open()` with `ALP_ERR_NOSUPPORT` instead of streaming at ALSA's negotiated rate (#1648 tier 1); TX/RX round-trips at a supported rate against a real I²S DAI. Known gap: `alp_i2s_write()`'s documented `ALP_ERR_OUT_OF_RANGE` bound on an oversize `bytes` argument is not enforced here — `y_write()` loops across periods instead of refusing | `tests/yocto/i2s_alsa.c` (NULL/INVAL + unreachable device failure paths, mirroring `audio_alsa.c`); backend also compiles `-fsyntax-only`-clean against real `<alsa/asoundlib.h>` (gcc, zero warnings) and every PR via `pr-plain-cmake.yml`'s `yocto` job (`libasound2-dev` installed); **full ALSA link + on-target rate-refusal / TX-RX round-trip need a real I²S DAI (`hil-yocto`)** | v0.4 |
...
| `meta-alp-sdk` BSP for V2N / V2N-M1 / i.MX 93 | `meta-alp-sdk/` | 🟡 partial | `bitbake alp-image-edge` succeeds for each `e1m-<sku>-a55` MACHINE; produced image boots on the matching SoM | V2N leg: full `core-image-minimal` bake clean on BSP v6.30 (8313 tasks, 2026-05-26, kernel 6.1.141-cip43 + carrier dtb + FIP) and the produced kernel/dtb run the live V2N bench board from eMMC. A separate `drpai`-OFF `alp-image-edge` bake under the `alp` distro completed (12118 tasks, 716 MB `.wic.gz`), but that image has not booted on the bench. `drpai`-enabled `alp-image-edge`, `alp-image-prod`, V2N-M1, and i.MX 93 bakes remain unverified; see [`bring-up-drpai-v2n.md`](bring-up-drpai-v2n.md)'s status banner for the authoritative V2N bake state. | v0.7 |
| Authoritative Zephyr board files | in-tree (`zephyr/boards/alp/e1m_aen801_m55_he/`, `.../e1m_aen801_m55_hp/`) | ⏳ untested | `alp_e1m_aen801_m55_he/ae822fa0e5597ls0/rtss_he` board boots and binds the same DT aliases the SDK overlays expect | HIL | v0.4 |
| MCUboot secure-boot on AEN-Zephyr (single-slot boot + verify) | `zephyr/sysbuild/aen/sysbuild.conf` + `keys/generate_dev_key.sh` + `docs/secure-boot.md` | 🟡 partial | Signed image boots + ECDSA-P256 signature verified on real silicon (single-slot, `CONFIG_SINGLE_APPLICATION_SLOT=y`); a tampered signature is caught live (a flipped TLV byte produces `bootutil_verify_sig` failure -> `E: Unable to find bootable image`, halt).  Swap-using-scratch's two-slot rollback-to-previous-slot and mid-swap power-loss recovery are a SEPARATE, still-untested claim -- the tested config has no previous slot to fall back to | Bench-proven on real `AE822FA0E5597LS0` Rev A0, alp-sdk `0da1f1b4` (`docs/secure-boot.md` status block: `PC=80012FBC`, `VTOR=80010800`, `CFSR=00000000`); a separate `swap-using-scratch` build boots (`I: Bootloader chainload address offset: 0x10000`) but its swap/rollback path itself was not exercised | v0.4 (boot+verify); swap/rollback still HIL-gated, see "Failure modes + rollback" in `docs/secure-boot.md` |
| MCUboot customer-path slot0 flash (plain J-Link, no SETOOLS/SE-UART) | `docs/aen-provisioning.md` §0.5 Option B | ✅ verified | An `imgtool`-signed image `loadbin`'d straight to slot0 (`0x80010000`) is verified by MCUboot and chainloaded, and survives repeated cold power-cycles; both a tampered signature and a non-MCUboot image are refused while the debug port stays alive (no brick) | Bench-proven on real E1M-AEN801 silicon -- `docs/secure-boot.md` ("The customer path is now proven too") + `docs/aen-provisioning.md` §0.5 (three cold power-cycles, byte-verified against the staged file).  Proven at `0x80010000` (slot0) only -- writing the ATOC region or erasing MCUboot itself was not tested | v0.4.x |
| Secure OTA on AEN-Zephyr (MCUboot + Mender) | `docs/ota.md` (Zephyr-client decision pending) | ⏳ untested | Signed update delivered, swap-using-scratch completes, next boot validates | HIL + OTA bench (gates on Zephyr-side Mender client choice: `mender-mcu-client` vs Hawkbit, plus the in-tree `alp_e1m_aen801_m55_he` board file) | v0.4 |
...
| Feature | Module / file | Status | What "verified" means | Evidence | Gates |
|---|---|---|---|---|---|
| GD32 bridge link (SPI 25 MHz + I2C), full command set | `gd32-bridge-firmware:` + `chips/gd32g553/` | ✅ verified | Functional suite asserts real values (ADC/DAC/PWM/math/TRNG fault-recover contract) on silicon | Evidence vintage: fw v0.2.9 / protocol v0.7 -- 26/26 on fw v0.2.4 (merge `ed1daf2`); re-smoked through v0.2.9; 20-row HIL soak scored 253/253 at this vintage.  The bridge has since moved to fw v0.2.11 / protocol v0.9 (ADC oversample + resolution, PWM center-align, ADC DSP dispatch, OTA Path-A hardening); that delta has not been re-soaked | v0.6 |
...
| Protocol v0.7 STATUS_SEQ stale-reply kill | `gd32-bridge-firmware:src/transport_spi.c` + host driver | ✅ verified | Negotiated stamp advances per fresh decode; stale replies detected + safely re-sent on silicon | CHANGELOG v0.2.9 entry; `seq_forensics` telemetry in the soak | v0.6 |
| GD32 supervisor A/B OTA (Path A, opcodes 0xF0..) | `gd32-bridge-firmware:src/ota.c` + bootloader | ✅ verified | Full image stream → CRC verify → slot swap → boot-back, e2e on silicon | OTA bench e2e (7 silicon bugs found + fixed; memory ledger `project-gd32-ota-path-a-validated`).  Boot-select, dual-bank FMC-from-RAM erase, and the background-erase path are separately silicon-validated on the GD32 (bootloader silicon-validated 2026-06-04, `gd32-bridge-firmware:src/bootloader/DESIGN.md`; background-erase entry, CHANGELOG "Fixed — GD32 bridge OTA Path-A hardening") | v0.6 |
...
| AEN801 peripheral matrix — I²C + EEPROM | `examples/aen/aen-i2c2-eeprom-regcheck` | ✅ verified | `i2c2` enumerates a 24C128 EEPROM at `0x50`; 12 devices detected total on the bus (the same part also answers a second device-select header at `0x58`, not an additional device) | same | v0.8 |
...
| AEN801 peripheral matrix — RPi-style CSI-2 camera first light (E1M-AEN803) | `examples/aen/aen-camera-firstlight` (cam/csi/dphy + OV5647/OV9281 on the E1M-EVK J5) | ✅ verified | OV9281: real GREY8 frames land in memory on an E1M-AEN803 on the E1M-EVK (2026-09-21), in all three modes (640x400, 1280x720, 1280x800), each at its configured frame rate (measured 60-frame bursts: ~100/~50/~100 fps), with the sensor test pattern also verified in all three -- needs the SoM's P/N-crossing camera-connector adapter (`docs/boards/e1m-evk.md`) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` (default on for the E8). CPI re-arm behaviour changed TWICE by issue #2287 (CPI-only pause, then Stage B's ISP-driven re-arm after a mid-frame-corruption bug, runs 304-310) since this bench -- **changed since bench, re-bench pending**. OV5647: bench-verified (2026-09-22, E1M-AEN803 serial 2026W36-0001 on E1M-EVK hw_rev 2626-r2, with the J5 pin-11 pull-up rework fitted), RAW10 640x480, issue #2248 -- see `docs/camera-shields.md`'s OV5647 driver section. Same CPI re-arm changes since this bench -- **re-bench pending**. IMX296: Stage A+B bench-verified (issue #2287, E1M-AEN803 2026W36-0001) -- single-lane RAW10 1456x1088 full-frame AND 1280x960 ROI crop (runs 294/295), I2C identity, CSI-2 streaming, the ISP-Pico data path through `examples/aen/aen-isp-capture`'s IMX296 variant (run 297), auto-exposure (runs 309/310), and continuous MJPEG streaming through `examples/connectivity/camera-mjpeg-stream`'s IMX296 scenario (runs 312-314: 61 fps encode, up to 28.3 fps delivered). Not verified: IMX296 colour (AWB/CCM, no calibration table exists), fast-trigger mode, and `SHS` < 14 in ROI mode (datasheet VTR vs. this driver's fixed VMAX). See `changelog.d/2287.md` and `docs/camera-shields.md` for the full bench history | same | v0.8 |
...
| AEN801 peripheral matrix — PDM mics | `examples/aen/aen-pdm-mic-alif` | 🟡 partial | Register-level configuration verified by this example. 48 kHz mode 7 capture rate verified exact (`measured_rate_hz=48000`) with no drops (`slab_missed=0`, `overrun=0`, commit `68a169977`). Acoustic capture at 48 kHz on mic ch0/ch1 (PDM controller 0) ONLY is separately VERIFIED, NOT by this example (which has no controlled stimulus of its own), but by a speaker-to-mic loopback -- the `PROBE_LOOPBACK` mode of `examples/aen/aen-i2s-tas2563-probe` on branch `test/u46-i2s-tas2563-on-reworked-mux` (commit `56631094d`, issue #2143), E1M-AEN803 serial 2026W36-0002, 2026-09-15 14:49Z, TAS2563 speakers, gain `0x200` readback-confirmed: Goertzel-bin analysis found the 1 kHz bin at 9.3/0.1 dB (ch0/ch1) in silence rising to 43.4/48.8 dB at volume 16 and 57.8/57.6 dB at volume 48, a 500 Hz bin at 54.7/56.6 dB only lighting up during the 500 Hz stimulus, and peak-to-peak (per channel, 960 ms window, after `zephyr_drv.c`'s `dc_block_s16()` high-pass, hence not a multiple of 32) rising from 128/128 in silence to 266/292 at 1 kHz/volume 16 and 651/652 at 1 kHz/volume 48. The D2 pair (HW 4/5) is register-level verified only, never acoustically tested. Gain was `0x0D` (0.8125x, -1.8 dB vs unity, 31.9 dB below the provisional `0x200`); `0x800` clipped on silicon; now a provisional `0x200` default (issue #2143) -- full-scale headroom at that gain is UNMEASURED | same | v0.8 |
| AEN801 peripheral matrix — I2S TX | `examples/aen/aen-i2s-amp-alif` (i2s3) | ✅ verified | i2s3 clocks a tone out at the 76.8 MHz audio clock (this example does not wire the amps). Separately, amp playback audibility through TAS2563 amps + U46 is 🟡 partial: verified with U46 replaced by a 3257-type bus switch powered from `+3V3` (the as-built 74LVC157 can never pass this direction at any VCC) -- an open TDM clock-error latch and open issue #2146 (amp auto-shutdown after I2S stop) remain | same for i2s3; E1M-AEN803 serial 2026W36-0002 bench session, 2026-09-15, maintainer, for amp playback | v0.8 |
...
| Portable monotonic clock (`alp_uptime_ms`) + CC3501E `poll_by_repeat()` deadline bound (issue #1953) | `include/alp/peripheral.h` + `src/zephyr/delay_zephyr.c` + `src/common/stub/stub_core.c` + `chips/cc3501e/cc3501e_core.c` (`poll_by_repeat`) | ⏳ untested | Every worker-routed CC3501E op (Wi-Fi, BLE, sockets, GPIO proxy, OTA) returns `ALP_ERR_TIMEOUT` no later than its declared `timeout_ms` plus one attempt's own cost, against a companion that never publishes a result -- not `timeout_ms` plus the accumulated cost of every attempt made, as before | native_sim, `tests/zephyr/cc3501e_poll_deadline/src/test_poll_deadline.c`: a stub clock (`alp_uptime_ms`) that advances only via the test's own `alp_delay_ms`/`alp_spi_transceive` seams proves the deadline-bound loop against an always-`RESP_ERR_BUSY` companion, and is mutation-proven against the pre-fix sleep-only budget: reverting `poll_by_repeat()` to that old form reddens BOTH suite cases -- `test_timeout_bounds_wall_time_1953`'s wall-time-bound assertion (500 ms observed against a 200 ms budget, vs. 211 ms post-fix) and `test_zero_timeout_still_makes_one_attempt_1953`'s exact-attempt-count assertion (6 transceive calls observed vs. the required 3; this second case was NOT actually locked before round 18 -- its assertion was `>= 3u`, which the pre-fix form also satisfies).  **No HIL** -- `chips/**` is bench-held per #1937 and the AEN bench is down (#1883, 0-of-10 boots); needs a bench run against a genuinely wedged/stalled companion | v0.17 |
| EEPROM Secure Data Page write + permanent lock (`eeprom_24c128_secure_page_write()` / `eeprom_24c128_secure_page_lock()`, issue #2151) | `chips/eeprom_24c128/eeprom_24c128.c` + `include/alp/hw_info.h` (`alp_secure_page_mirror_t` / `alp_secure_page_mirror_classify`) | ✅ bench-verified | A real onsemi N24S128 Secure Data Page write survives a cold power cycle, reads back byte-exact, and a subsequent lock permanently NAKs every further write to that page -- confirmed by a genuine bench run, not by unit tests alone | Wire-frame + call-order unit-tested only, `tests/common/eeprom_24c128_secure_page.c`: `test_write_frame_is_exact` / `test_lock_frame_is_exact_and_never_polls_the_page`, mutation-proven against three real defect classes review found on this branch -- a selector rotated into the wrong pointer byte, an ACK poll issued against the page the lock had just made NAK-only, and a deleted/underrun post-lock wait (`EEPROM_LOCK_WAIT_MS`).  Of `examples/aen/aen-eeprom-provision` mode 3's three application-level lock gates, only the array-manifest-vs-mirror field cross-check has any test: its ALGORITHM is covered by a duplicate in `tests/common/aen_eeprom_provision_lock_gates.c`, which is not wired to `main.c`'s copy, so a defect in `main.c` alone still passes. The `ALP_SECURE_PAGE_COLD_CYCLED` attestation (separate from `ALP_LOCK_SECURE_PAGE`) and the post-lock re-read -- which re-confirms both the Lock Status bit and the payload before printing PASS -- have NO test; that they are present, and ahead of the lock, rests on code review.  **BENCH-VERIFIED 2026-09-16 on three E1M-AEN modules** -- 2026W36-0005 (E4, `AE402FA0E5597LE0`) and 2026W36-0006 / 2026W36-0007 (E8, `AE822FA0E5597LS0`). On each, the 64-byte page read back byte-identical to the staged blob after a cold power cycle, the lock frame `{0x04, 0x00, 0xFF}` took, and Lock Status moved `0xFD` -> `0xFF`, setting exactly bit 1 and surviving a further cold cycle; Device Config stayed `0x1D` and was never written. That is the first silicon evidence for the `(byte & 0x02) != 0` polarity, which was previously paper-only. Note what is NOT covered: no automated HIL job runs this -- the evidence is three attended provisioning runs recorded in `alp-sdk-internal/ledger/`, and the operation stays one-shot and irreversible | v0.17 |
| OSPI0 pinctrl applied by the flash driver (issue #2041) | `zephyr/drivers/flash/flash_ospi_alif.c` (`ospi_alif_init()`) + `examples/aen/aen-ospi-regcheck`'s board overlay | 🟡 partial | `flash_ospi_alif.c` applies its DT `pinctrl-0` state on real silicon without error, so an OSPI0 consumer no longer has to mux the pads itself or silently drive an unmuxed bus -- and the example's pinctrl group names the E1M-AEN module's own 13-pad OSPI0 route (ports 0/2/6/12, all function 1, per `metadata/e1m_modules/aen/alif-ospi.tsv`) rather than the Alif DevKit's | **Bench-verified 2026-09-13 on two E1M-AEN803 modules** (serial 2026W36-0001 and serial 2026W36-0002), from a clean checkout of merged `dev` (`bdd421d2c`) built for `alp_e1m_aen801_m55_he/ae822fa0e5597ls0/rtss_he`, Flow C ITCM RAM-run: `alif_hal_ospi_initialize() rc=0 handle=1`, `CTRLR0=0x00c00407` (the documented reset value, `OSPI_CTRLR0_RESET_VALUE` in the example), and `RESULT PASS: OSPI/HexSPI node BINDS` on both.  `pinctrl_apply_state()` is fail-closed and runs before the clock-enable, so the device reaching READY is itself the proof that the call returned 0.  Module resolution was confirmed before trusting the result -- `zephyr_modules.txt` carried the bench-verify checkout, with 0 hits on the shared one -- because west/twister can silently resolve `alp-sdk` to a shared checkout.  **Bind-level only, and deliberately not more**: the pad-mux registers were NOT read back, so "pinctrl applied without error" is measured while "the pads carry the intended function" is not; and `aen-ospi-regcheck` performs zero device-level transfers (controller-register reads only), so nothing in tree exercises the corrected chip selects end to end.  That probe, and a `flash_driver_api` to build it on, are #915's work | v0.17 |
| DesignWare I2S channel-enable model on the Alif E8 (issue #2205) | `zephyr/drivers/i2s/i2s_dw.c` + `zephyr/drivers/i2s/i2s_dw.h` | 🟡 partial | The register fact the fix rests on is measured on silicon: on the E8's `i2s3` (`0x49017000`) `TER` bit 0 (`TXCHENX`) and `RER` bit 0 (`RXCHENX`) cannot be cleared, even with the block idle, so `i2s_tx_channel_disable()` / `i2s_rx_channel_disable()` are no-ops there. The driver built on it is bench-verified on the RX-teardown path at `4efc10692`: an RX START on `i2s3` pre-empts the RUNNING TX into ERROR while `CER.CLKEN` and both TAS2563 amps stay up, and `i2s_dw_initialize()` writes the full `IMR` mask over an inherited partial one. NOT exercised on silicon: a TX restart after an RX session, a blocked reader/writer being released, the TDM slot enables, the ISR guard masking `TXFU`, and the DRAIN/STOP log-level change made after that bench run | E1M-AEN803 serial 2026W36-0002, Flow C RAM load of the archived #2179 fix-arm image (`e1ed4881a`, ELF md5 `d9aad40ed6d57addb1b1f06d646de6ee`), no MRAM writes (bookend at `0x80010000` byte-identical); hardware breakpoint at `0x9F72`, the first instruction after the init-time `i2s_tx_channel_disable()`, in `POST_KERNEL`: `IER 0x00000F00` (the SVD reset value), `IRER`/`ITER`/`CER` `0`, `RER` = `TER` = `0x00FFFF01` (the E8 SVD reset value, `LPI2S` `I2S_TER0`/`I2S_RER0`, bit 0 read-only). Straight after a RESETPIN reset every one of those read `0x00000000`, `IER` included -- an unclocked block, not a reset value. Driver bench run: E1M-AEN803 serial 2026W36-0002, `4efc10692` plus the `PROBE_RX_TEARDOWN` mode of `examples/aen/aen-i2s-tas2563-probe` cherry-picked from `test/2179-isr-capture`, 4/4 PASS: step 4 `PWR_CTL=0x0c/0x0c` and `CER=0x00000001`, teardown `CER=0`, 0 write failures, `IMR` `0x00000073` after the run. Init mask: with `IMR` forced to `0x00000033` over SWD just before the init-time store, it read `0x00000073` after it. Every run also printed `E: DRAIN trigger: invalid state` at teardown (the backend's DRAIN refused on the parked TX before its DROP fallback), fixed afterwards at debug level. Host: `tests/unit/i2s_dw_underrun` on native_sim/native/64 | v0.17 |
...
| Portable-API conformance suite (14 classes × 8 cases, `alp_sdk.conformance.portable_api`) | `pr-twister.yml` (`tests/zephyr/conformance/`) | ✅ verified |

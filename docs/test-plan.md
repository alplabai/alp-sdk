...
| 5L35023B clock generator (partial) | `chips/clk_5l35023b/` | 🟡 partial | Register-0 ACK on V2N's BRD_I2C at 7-bit `0x68` (= 8-bit `0xD0`); OTP defaults + the `Audio_CLKB_OE` GPIO carry runtime operation.  PLL / divider helpers gated on the datasheet drop. | NULL-arg + post-init ZTESTs green in `pr-twister`; **HIL exercise** for register dump on real silicon | v0.3.x |
...
| TPS628640 multi-instance buck (partial) | `chips/tps628640/` | 🟡 partial | Probe + raw register R/W across the four V2N-M1 instances (`0x44`, `0x48`, `0x4D`, `0x4F`); VSET decode landed against the datasheet drop | `examples/v2n-pmic-rail-monitor` exercises probe on the EVK; **HIL VSET-readback** gates the next bump | v0.3.x |
...
| Secure OTA on V2N-Yocto (`meta-mender`) | `yocto/meta-alp/conf/distro/include/mender.inc` + machine .conf opt-in lines + `docs/ota.md` | ⏳ untested | A/B partition swap survives an interrupted-update simulation; rollback on failed commit health-check | HIL via `hil-yocto` | v0.4 |

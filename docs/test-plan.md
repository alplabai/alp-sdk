...
| AEN801 peripheral matrix — RPi-style CSI-2 camera first light (E1M-AEN803) | `examples/aen/aen-camera-firstlight` (cam/csi/dphy + OV9281 on the E1M-EVK J5) | ✅ verified | OV9281: real GREY8 frames land in memory on an E1M-AEN803 on the E1M-EVK (2026-09-21), in all three modes (640x400, 1280x720, 1280x800), each at its configured frame rate (measured 60-frame bursts: ~100/~50/~100 fps), with the sensor test pattern also verified in all three -- needs the SoM's P/N-crossing camera-connector adapter (`docs/boards/e1m-evk.md`) and `CONFIG_VIDEO_ALIF_CAM_EXTENDED=y` (default on for the E8) | same | v0.8 |

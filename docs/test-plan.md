...
| AEN801 peripheral matrix — SD card | DWC SDHC | 🔴 disabled | **Hardware defect, not gated (#2051)**: the EVK's SDIO 74LVC157 mux (EN=IO20, SEL=IO21, both CC3501E-side) has no high-impedance state, so its SoC-facing outputs are held low whenever the mux is powered, regardless of ENABLE -- `sdhc0` stays disabled in the devicetree on the 2626-R2 EVK rather than fight those pads | `docs/boards/e1m-evk.md`, `examples/aen/aen-sdhc-probe` | v0.8 |

...
| `<alp/ext/alif/storage.h>` (Alif OSPI SecAES key provisioning, issue #224) | Local physical (JTAG/SWD) + local storage-bus attacker | Caller-supplied AES-128 key is marshalled into one on-stack SE request packet (`ospi_write_key_svc_t`) and sent to the Secure Enclave over the RTSS-HE↔SE MHUv2 mailbox (`se_service_send_request`) -- the SE, not the host, writes the key into the OSPI AES hardware slot.  That packet is the only place the SDK ever holds the key in clear; it is zeroised with a compiler barrier immediately after the call returns, on both the success and failure path, and never logged.  No OPTIGA involvement -- this is a separate key-loading path from the portable surface above.  UNVERIFIED ON SILICON: no bench unit reachable at implementation time has an OSPI SecAES-relevant part populated (board rev r1; the targeted Macronix part ships DNP=0 only on the R2 BOM) |

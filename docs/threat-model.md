...
| `<alp/power.h>` wake-source contract (v0.17, #1813) | A caller arms a wake source (or retention footprint) the backend cannot honour, sleeps, and the device never wakes -- a silent permanent hang, not just a DoS on the caller | `alp_power_configure_wake_source()` / `alp_power_configure_retention()` now return `ALP_ERR_NOSUPPORT` for any bit / level the backend cannot arm, discoverable in advance via `alp_power_capabilities()`, instead of the old "unsupported bits silently ignored" contract; enforced today on the AEN-default `zephyr_stub` backend (#1812) |

...
| INA236 rail monitor (`chips/ina236/`) | `[VERIFIED]` on E1M-AEN801 silicon, **read + calibration paths only** | Bus voltage, shunt voltage, current and power read back on all answering EVK monitors over SoC I2C2, and the scaling cross-checked two independent ways on the +5V rail (`U30` @ `0x4A`, 20 mOhm): the POWER register gave 0.492 W where bus-voltage x shunt-derived current gave 0.5125 W, 4 % agreement. That agreement is what pins the eq.-4 power LSB and the `ADCRANGE=1` `SHUNT_CAL`/4 rescaling — each was wrong before (by 625x and 4x respectively) and either error alone would have broken it. Sustained sampling proven at 223 conversions/s for 12000 conversions via the conversion-ready flag (`examples/aen/aen-inference-energy`, see `docs/measuring-inference-energy.md`). **NOT verified: the ALERT pin, the alert-limit comparators, and the triggered (single-shot) conversion modes — no test or bench run exercises them.** `+V_CAM0` does not answer at `0x4B` on the pre-respin bench board (strapped `0x48`, colliding with the TAS2563 broadcast address). |

<!-- AUTO-GENERATED by scripts/gen_verification_status.py — DO NOT EDIT BY HAND — regenerate with scripts/gen_verification_status.py -->
<!--
...
106 silicon/HIL-gated ledger rows parsed across 13 sections.  A row can carry more than one glyph
...
### "CI-only / tooling rows (no HIL gate)" -- counted separately

Tracked here so the page is complete, but never pooled into the Summary above -- see the note there.  `✅` in this table means "the matching GitHub Actions workflow has been green on `main` for at least two consecutive PRs", not silicon.

| Glyph | Meaning (CI-only sense) | Count |
|---|---|---|
| `⏳` | untested | 0 |
| `🟡` | partial | 0 |
| `✅` | verified | 9 |
...
- [`docs/test-plan.md`](test-plan.md) — the ledger this
  page is generated from; edit rows there, not here.
- [`docs/ci/HW-IN-LOOP.md`](ci/HW-IN-LOOP.md) — HIL runner contract referenced throughout the ledger above.
- [`VERSIONS.md`](../VERSIONS.md) — versioned roadmap; test-plan.md is the ledger that gates each version's tag.
- Regenerate: `python3 scripts/gen_verification_status.py`

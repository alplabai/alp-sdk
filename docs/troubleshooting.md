...
* `tan build` executor bug: [`github.com/alplabai/tan-cli`](https://github.com/alplabai/tan-cli) instead -- alp-sdk is plans-only for the multi-slice build/flash/size/image/clean surface (ADR [0020](adr/0020-sdk-owns-build-execution.md)); `tan run` is the one forwarded single-image escape hatch that still shells `west build`/`west flash` itself.

...
every Python text-IO call under `scripts/` and `tests/` now states
its `encoding=` explicitly, so there is nothing left to grandfather.
...
encoding anywhere in the tree, breaks the build.  See

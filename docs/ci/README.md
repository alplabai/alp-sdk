...
| [`merge-queue-changelog-citations.yml`](../../.github/workflows/merge-queue-changelog-citations.yml)| merge queue + PR (paths, self-test) + manual | active | Re-grades changelog citations on the speculative `merge_group` ref against `github.event.merge_group.base_sha`. Its single static-named job is the context to require; the full metadata workflow stays off merge-queue entries. |

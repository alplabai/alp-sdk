...
| [`pr-static-analysis.yml`](../../.github/workflows/pr-static-analysis.yml)        | PR + push        | active     | `clang-format-diff` on changed lines + `cppcheck` informational pass over `src/` + `chips/` + `shellcheck -S warning` over `scripts/bench/**` (issue #1527).  Diff-only format check (`clang-format · diff-only` is one of `dev`'s required contexts). |

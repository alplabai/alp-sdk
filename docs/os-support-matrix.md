...
| Update audit log — HW_ENFORCED tier | `update_log.h` `ALP_UPDATE_LOG_HW_ENFORCED` (`CONFIG_ALP_SDK_UPDATE_LOG_TFM`) | M (TF-M builds only): app-immutable tier — non-secure client -> secure owner; secure owner stores entries in PSA Protected Storage and anchors the high-watermark counter in protected storage | client backend + PSA owner source present; without a linked/provisioned secure owner `ready()` returns NOSUPPORT so `alp_update_log_open()` falls through to the SW tier (native_sim-tested). Alif E4/E8 GA still requires wiring the secure owner into a TF-M/SPE image and placing the storage region behind the Alif SE/firewall policy; the public hal_alif SE service surface does not expose an audit-log or NV monotonic-counter mailbox command — #111 |

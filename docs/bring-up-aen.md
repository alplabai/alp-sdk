...
> -- but the upstream board file (`alp_e1m_evk_aen`, from
> `alplabai/alp-zephyr-modules`) is **not published yet**, so the
> exact `west build`/`west flash` target is **TBD until it lands**
> (see [`docs/porting-new-som.md`](porting-new-som.md)).  The
> commands below show the older Alif EVK naming
> (`alif_e7_dk_rtss_he`) purely as the invocation *template* --
> substitute the AEN801 target once the board file is generated.

...
| In-system upgrade over SPI / I2C      | **Implemented, gated + HIL-pending.** | Application-bootloader path; the `0xF0..0xFF` opcodes route through `src/bootloader/` into the OTA state machine in [`src/ota.c`](../firmware/gd32-bridge/src/ota.c) (FMC backend `hal/fmc_ota.c`). Destructive flashing is armed only with `-DBRIDGE_OTA_PARTITIONED`; default builds reply `STATUS_NOSUPPORT` (can't brick the running image). The partitioned bootloader/app linker + on-silicon validation are the remaining work. See [`docs/gd32-bridge-protocol.md`](gd32-bridge-protocol.md) §10 Path A. |

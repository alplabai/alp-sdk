...
| `ipc-contract-h` | `alp_system_ipc.h` | `#include`d by every slice's C code |
...
4. Emit shared generated artefacts:
     - build/generated/alp_system_ipc.h       (RPMsg endpoint IDs, carve-out names)
...
(`alp_orchestrate.py:_resolve_carve_outs`):
...
allocating. Re-running the build produces byte-identical layouts. CI can
diff the manifest.
...
### 6.3 RPMsg endpoint allocation (`alp_system_ipc.h`)
...
reserved by the SDK vs free for app use. The resolver fails the build if an
`ipc[]` entry asks for `kind: rpmsg` and no channel is
`reserved_for: alp_default_rpmsg`.
...
- Generators in `alp_orchestrate.py`:
  `alp_system_ipc.h`, `dts-reservations.dtsi`, remoteproc bbappend.

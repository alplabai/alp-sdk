...
**Status:** **Implemented.** `alp.lock` is generated and committed in the
tree today (regenerated by `west alp-lock` -- `scripts/west_commands/alp_lock.py`;
`.github/workflows/pr-metadata-validate.yml` gates drift with
`python3 scripts/west_commands/alp_lock.py --workspace . --check`).

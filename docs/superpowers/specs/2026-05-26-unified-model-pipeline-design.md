...
```jsonc
{
  ...
  "targets": [
    { "backend": "ethos_u", "silicon_ref": "alif:ensemble:e8",
      ...
      "requires": { "sram_kib": 512, "op_features": ["transformer"] }, "blob": 0 },
    { "backend": "drpai", "silicon_ref": "renesas:rzv2n:n44",
      "blob_format": "drpai_dir", "compiler_version": "drp-ai_tvm 2.x",
      "arena_bytes": 1048576, "requires": { "sram_kib": 1024 }, "blob": 1 },
...
3. Find `targets[]` where `backend` is available **and** `silicon_ref` is compatible with `ALP_SOC_REF_STR` (or a compiled-in on-module discrete accelerator) **and** the blob fits the device NPU envelope — `requires.sram_kib` ≤ the device budget `ALP_SOC_NPU_ARENA_SRAM_KIB` (0 = unknown → skip). *Stage 1c:* the `accel_config`/op-features fit is implied by the `silicon_ref` match (the resolver only emits a SoC's own accel-configs); the explicit per-variant op-set check is deferred to Stage 2 (the on-device reader doesn't yet parse blob `op_features`). (This is where a model built for the wrong silicon is rejected.)
...
- **Stage 1 — contract + walking skeleton (Vela + CPU).** Lock the `.alpmodel` container/manifest (JSON canonical + on-device reader); front-end skeleton (`CompilerAdapter` + `vela` + `cpu`, `alp model build` CLI, `board.yaml models:` → target derivation); runtime loader + selection wired to the **existing** Ethos-U + CPU/TFLM backends. **Exit:** compile → package → load → run works on Ethos-U (AEN/i.MX93) + CPU.
  ...
  - **STATUS 2026-05-26 — Stage 1b-i (front-end foundation) IMPLEMENTED** on `feat/alpmodel-frontend-1b`: `board.yaml` `models:` schema block, `CompilerAdapter` + CPU passthrough adapter (`scripts/alp_model/adapters/`), SoM→SoC target resolver (`scripts/alp_model/targets.py`), build driver (`scripts/alp_model/build.py`), and the `alp model build` CLI — CPU path end-to-end (pytest 12/12). **Remaining 1b-ii:** real `vela` adapter (Ethos-U compile + `ethos-u-vela` dep + a `.tflite` fixture), model tensor-I/O extraction (currently empty `inputs`/`outputs`), drpai/deepx adapter detection, and `requires`-envelope population. Then **1c** (runtime loader, which owns the `DEEPX_DX`→`DEEPX_DXM1` enum rename + string→backend mapping).
  - **STATUS 2026-05-26 — Stage 1b-ii (compiler back-ends) DONE** on `feat/alpmodel-frontend-1b-ii`: on-module discrete-accelerator target resolution (V2M DEEPX, via `variants[].alp_module_skus` scan), Vela adapter (real compile; skip-gated when `vela` absent) + drpai/deepx detect-and-skip stubs, `Target.compiler_version` end-to-end (additive manifest field; C reader ignores via `zcbor_any_skip`), TFLite tensor-I/O extraction into `Manifest.inputs`/`outputs` (hermetic 712-byte fixture; best-effort when parser absent), adapter injection + empty-blob guard. pytest 40/40 + 1 skipped (real-vela, expected). **Note:** `requires.sram_kib` stays the shipped key (spec text says `peak_sram_kib`) — reconcile in Stage 1c. ONNX tensor-I/O extraction + real drpai/deepx compiles deferred to Stage 2.
  - **STATUS 2026-05-27 — Stage 1c (runtime loader + blob selection) DONE** on `feat/alpmodel-runtime-loader-1c`: `alp_inference_open_alpmodel()` + `alp_model_open_opts_t` in `<alp/inference.h>`; a pure OS-agnostic selection engine (`src/backends/inference/alp_model_select.{c,h}`) — `silicon_ref` availability (host + compiled-in on-module discrete) + SRAM fit (`requires.sram_kib` ≤ `ALP_SOC_NPU_ARENA_SRAM_KIB`) + `preferred_backend` tiebreak + AUTO/explicit/CPU-fallback (explicit NPU requests bypass CPU fallback → `ALP_ERR_NO_FIT`), exhaustively unit-tested (11 native_sim cases, every branch); the loader (`src/common/alp_model_loader.c`, gated `CONFIG_ALP_SDK_MODEL_READER`) parses → selects → delegates to the existing `alp_inference_open` (5 native_sim integration cases). This stage owns the `DEEPX_DX`→`DEEPX_DXM1` enum rename, `ALP_ERR_NO_FIT`/`NO_BACKEND`/`NOT_FOUND`, the `inference_arena_sram_kib`→`ALP_SOC_NPU_ARENA_SRAM_KIB` envelope (TBD/0 per-SoC), and the v0.5 ABI regen. **Follow-ups:** op-features fit + per-variant accel envelope (Stage 2); `preferred_backend` board define via `alp_project.py`; authored per-SoC SRAM budgets; the 1b-ii `build_model` double-read of `source`.
...
- **Capability envelope source** — *resolved (Stage 1c):* the device NPU arena-SRAM budget is the new SoC-spec `inference_arena_sram_kib` field → `ALP_SOC_NPU_ARENA_SRAM_KIB` (0/TBD per-SoC until authored; 0 = skip the SRAM gate). The per-variant op-set/accel envelope + the on-device reader parsing blob `op_features` are deferred to Stage 2.

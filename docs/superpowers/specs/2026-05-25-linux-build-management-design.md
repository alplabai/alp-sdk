...
**Status:** Design approved 2026-05-25. **Phase 1 validated 2026-05-26** — `core-image-minimal` baked on WSL (BSP v6.30, bitbake-layers); kas retired; docs reconciled to "platform 7.1 / BSP v6.30" (see `docs/superpowers/plans/2026-05-25-linux-build-phase1-prove-v2n.md`, marked DONE). **Phase 2 (a unified `alp build` front-end wrapping per-vendor BSP providers) is SUPERSEDED, not "pending."** [ADR 0020](../../adr/0020-sdk-owns-build-execution.md) (accepted 2026-07-18/20) moved ALL build execution out of alp-sdk to the standalone `tan` CLI; the SDK-side executor and every `alp build`/`alp-*` verb this design's Phase 2 would have added were retired instead of built (`scripts/alp_cli/` carries no `build.py` or `bsp/` package today — verified against the tree). Do not implement this design's §3/§4 front-end; it would resurrect the retired SDK-side executor ADR 0020 exists to prevent. A future Yocto-integration need should route through `tan`'s build-plan consumer contract, not a new SDK-side command.

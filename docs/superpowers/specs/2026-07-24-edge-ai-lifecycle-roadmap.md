...
- **4 · Measure + iterate (designed 2026-07-24).** Closes the lifecycle: deploy → run inference → stream telemetry → dashboard + A/B compare. Telemetry: latency, which NPU backend actually ran, accuracy/confidence (top-K vs expected), peak SRAM, **power**. **Self-improving loop (killer feature):** measured latency validates #1's estimate → calibrates the static estimator → the moat data compounds (estimate → build → measure → estimate improves; only we, with bench + silicon, can close it). **Power via ON-BOARD instrumentation (maintainer HW insight, 2026-07-24):** the on-device runner reads the SoM's on-board **shunt resistors + power-monitor IC over I2C** during the inference window → energy-per-inference (∫ V·I dt), **PER-RAIL** (isolate the NPU rail from the SoC — we know the board topology). Benefits: power measurable at the CUSTOMER's desk (no bench PSU), and **field-reportable** (a fielded device reads its own monitor → fleet energy telemetry). Per-SoM power-topology metadata (monitor IC, I2C address, shunt value, which rail feeds the NPU) is manufacturer-authored — same asset class as the pin maps. **Reference measurement platform = the E1M-EVK / E1M-X-EVK carrier**, which carries the shunt resistors + power ICs; a customer carrier with equivalent instrumentation gets the same read. The bench PSU stays as the reference to calibrate the on-board monitor. (Exact per-EVK topology values — monitor IC part / I2C address / shunt Ω / NPU rail — pending the maintainer's authoritative HW.) **A/B compare:** two models / two quantizations / same model on two SoMs on the same input → latency/accuracy/size/**power** side-by-side → pick the best per SoM (flags "fits a smaller/cheaper in-family module" → cross-sell). Surface: `tan model run <model> [--input X]` → flash runner + stream the structured result; transport = local-serial (customer board) / bench-labgrid (us). Extension: run-inference + live-results panel + A/B view; fleet dashboard later (OTA'd devices report home). **Honest gating (material):** the A55 DEEPX/DRP-AI run/observe half needs the **bench-gated Yocto NPU runtimes that are not shipped yet** — Ethos-U/CPU/Zephyr ships first; the on-board power read also needs the per-SoM power-topology metadata authored + the runner reading the monitor. Most infra-heavy sub-project; deferred last.

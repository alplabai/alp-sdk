...
|---|---|---|
| `tan model doctor` | `toolchains: [{backend, tool, available, version, reason}]` — `backend` ∈ `ethos_u` / `deepx_dxm1` / `drpai` / `cpu`; `tool` ∈ `vela` / `dxcom` / `drpai-tvm`; `reason` non-null only when unavailable | a new read-only `probe()` on each adapter (`is_available()` + version + reason; `scripts/alp_model/adapters/`) |
| `tan model build [--model NAME]` | `models: [{name, source, alpmodel_path, total_bytes, targets: [{backend, silicon_ref, blob_format, accel_config, arena, blob_bytes, requires: {sram_kib, op_features}, compiler_version}], skipped: [{backend, accel_config, status, reason}]}]` — `status` ∈ `skipped` / `incompatible` | `alp model build` (`scripts/alp_cli/model.py`, exists). compiled targets vs skipped coverage read back from the written `.alpmodel` via `read_package()` |
...
| `tan model info NAME` | decoded manifest: every target blob (`backend`, `silicon_ref`, `blob_format`, `blob_bytes`, `requires`) + a SoM-declared-backends × has-blob **coverage matrix** (via `resolve_targets(sku)`) | new `info` subcommand decoding the binary container directly with the **already-existing** `read_package()` (`scripts/alp_model/package.py`) — there is no JSON sidecar. `ALP_ERR_NO_FIT` SRAM-fit flagging is deferred to the consumer (needs the device arena budget) |
...
1. **Upgrade the existing `tan model`** — today it is a `Model(WestForwardArgs)` verbatim passthrough (`crates/tan-cli/src/cli.rs:150`) that forwards to `alp model` with no structured output. Repoint it to a real subcommand group dispatching `build | list | info | doctor | run` → resolve the SDK root (existing resolver) → shell `alp model <sub> --format json` → wrap each payload in `Envelope<T>` (`crates/tan-cli/src/envelope.rs`, mirroring `tan validate`). Pure orchestration; no model domain logic enters `tan-core`. Note: `tan-core`'s `BoardModel` (`model.rs`) parses `inference` but not `models[]` — extend only if the run/observe path needs it. Home: `crates/tan-cli`.

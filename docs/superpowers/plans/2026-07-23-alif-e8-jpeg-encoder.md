...
- **Error/status idiom:** ops return `alp_status_t`; values from `include/alp/error.h` — `ALP_OK`, `ALP_ERR_INVAL`, `ALP_ERR_NOSUPPORT`, `ALP_ERR_NOMEM`, `ALP_ERR_IO`, `ALP_ERR_NOT_READY`, `ALP_ERR_TIMEOUT`, `ALP_ERR_NOT_IMPLEMENTED`, `ALP_ERR_NOT_PRESENT_ON_THIS_SOC`. (Confirm exact spellings against `include/alp/error.h` before first use.)

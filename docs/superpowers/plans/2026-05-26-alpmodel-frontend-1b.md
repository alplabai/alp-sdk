...
- [DONE 1b-ii] **`VelaAdapter`** (`scripts/alp_model/adapters/ethos_u.py`): `is_available()` = `vela` importable / on PATH; `accepts("tflite")`; `compile()` shells out to `vela <src> --accelerator-config <accel_config> --output-dir <out>` and reads back the `*_vela.tflite`. Add `ethos-u-vela` as an optional dep (`[project.optional-dependencies] model-compile`). Register it in `build._ADAPTERS`. Unit-test with vela mocked; a real-vela test that `pytest.skip`s when vela is absent (mirrors compile-what's-available). Needs a tiny `.tflite` fixture under `tests/fixtures/models/`.
- [DONE 1b-ii] **Tensor-I/O extraction**: parse the source model (TFLite flatbuffer / ONNX) to populate `manifest.inputs`/`outputs` (`Tensor` dtype/shape/quant) instead of the empty lists used in 1b-i. (ONNX I/O extraction deferred to Stage 2.)
- [DONE 1b-ii] **`requires` envelope**: populate `req_sram_kib` + `op_features` from the vela compile report so the Stage-1c loader's capability fit-check has real data. (`peak_sram_kib` naming reconciliation → Stage 1c.)
- [DONE 1b-ii] **drpai/deepx adapters**: detect-and-skip skeletons (`is_available()` → checks for the proprietary tool, returns False when absent) so a fat package records them in `coverage`. Real compile = Stage 2.
- **Declarative build hook** (orchestration auto-calls `alp model build` from the project build) is Stage 3.

### Review follow-ups (from the 1b-i final review, 2026-05-26) — fold into 1b-ii / Stage 2
- [DONE 1b-ii] **V2M discrete on-module NPU gap (Important):** `resolve_targets` derives targets ONLY from the *host* SoC's `npus[]`. The V2M family is multi-silicon — `E1M-V2M101.yaml` declares `silicon: renesas:rzv2n:n44` (DRP-AI only in its `npus[]`) **plus** a discrete DEEPX DX-M1 via `on_module.npu: deepx_dxm1` / `capabilities.deepx_dx` (its own SoC `deepx:dx:m1`, behind PCIe). So `resolve_targets("E1M-V2M101")` currently yields `[drpai, cpu]` and **silently omits deepx** (not even a coverage skip). 1b-ii must teach the resolver to fold in on-module discrete accelerators (read `on_module.npu` / `capabilities.*` / the SoM `inference.preferred_backend`), otherwise the deepx detect-and-skip can't surface it. Add a V2M101 resolver test at that point.
- [DONE 1b-ii] **`Blob.compiler_version` is dropped:** adapters return it but the Stage-1a `Target` dataclass has no `compiler_version` field, so `build_model` discards it. When the vela adapter lands with a real version string, add a `compiler_version` field to `Target` (additive manifest-contract change) + thread it through writer → on-device reader if the loader needs it.
- [DONE 1b-ii] **Source-format guard:** `_src_format` accepts any suffix; an unsupported source (e.g. `.pt`) yields a package with only coverage rows + no blob, exit 0. Add a "no adapter accepted .<fmt>" warning/error once onnx-only backends exist.

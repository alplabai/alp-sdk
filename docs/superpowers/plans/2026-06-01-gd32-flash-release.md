...
**Flashing reality (HW rev):** the GD32 is flashed by an **external SWD probe (J-Link) connected directly to the GD32's own SWD header** (`SWDIO=PA13`, `SWCLK=PA14`, `NRST`). The Renesas V2N host is **not** involved — the host-driven SWD path (`P70`/`P71`→GD32) is not wired on this revision.
...
| `tests/scripts/test_alp_orchestrate.py` | Update `flash_method` string `swd_v2n_host`→`swd_probe` | 2 |
| `metadata/e1m_modules/E1M-V2N101/102.yaml`, `E1M-V2M101/102.yaml` | `flash_method`→`swd_probe`; fix `gd32_bridge` `firmware_path` (`gd32_bridge.bin`→`gd32-bridge.bin`) | 2 |
...
## Task 2: Rename swd_v2n_host → swd_probe, then add the SEGGER J-Link path (TDD)

**Files:**
- Rename: `scripts/flash_backends/swd_v2n_host.py` → `scripts/flash_backends/swd_probe.py`
- Modify: `scripts/flash_backends/__init__.py`, `tests/scripts/test_flash_backends.py`, `tests/scripts/test_alp_orchestrate.py`, `metadata/e1m_modules/E1M-V2N101.yaml`, `E1M-V2N102.yaml`, `E1M-V2M101.yaml`, `E1M-V2M102.yaml`, `metadata/schemas/som-preset-v1.schema.json`, `scripts/west_commands/alp_flash.py`, `docs/v0.6-tbd-and-assumptions.md`

### Phase A — mechanical rename (no behaviour change)

- [ ] **Step 1: Move the file + rename the class/key**

Run:
```bash
git mv scripts/flash_backends/swd_v2n_host.py scripts/flash_backends/swd_probe.py
```
In `scripts/flash_backends/swd_probe.py`: rename class `SwdV2nHostFlash`→`SwdProbeFlash`, set `name = "swd_probe"`, and replace the module docstring's first lines (which currently say "via the V2N host's SWD pins … the V2N drives SWDIO=P70") with this clean header (the body is unchanged in Phase A):
```python
...
- [ ] **Step 2: Update the package import**

In `scripts/flash_backends/__init__.py`, change `from . import swd_v2n_host` to `from . import swd_probe` (keep the `# noqa: E402,F401`).

- [ ] **Step 3: Update every remaining reference**

Run to find them:
```bash
grep -rn "swd_v2n_host\|SwdV2nHostFlash" -- . ':!docs/superpowers'
```
Update each occurrence:
- `tests/scripts/test_flash_backends.py`: import `from flash_backends.swd_probe import SwdProbeFlash`; the registry-set entry `"swd_v2n_host"`→`"swd_probe"`; every `SwdV2nHostFlash()`→`SwdProbeFlash()`; every `patch("flash_backends.swd_v2n_host...")`→`patch("flash_backends.swd_probe...")`; rename the test functions `test_swd_v2n_host_*`→`test_swd_probe_*`.
- `tests/scripts/test_alp_orchestrate.py`: the expected `flash_method` string `"swd_v2n_host"`→`"swd_probe"`.
- `metadata/schemas/som-preset-v1.schema.json`: the description example `swd_v2n_host`→`swd_probe`.
- `scripts/west_commands/alp_flash.py`: the comment mentioning `swd_v2n_host`→`swd_probe`.
- `docs/v0.6-tbd-and-assumptions.md`: the flash-backend table row `swd_v2n_host`→`swd_probe`.

- [ ] **Step 4: Update the 4 SoM presets (method key + firmware_path fix)**
...
Run:
```bash
grep -rn "swd_v2n_host\|SwdV2nHostFlash" -- . ':!docs/superpowers' ; echo "exit: $?"
python -m pytest tests/scripts/test_flash_backends.py tests/scripts/test_alp_orchestrate.py -q
```
Expected: the grep prints nothing (exit 1 = no matches); both test files pass (registry now has `swd_probe`, presets resolve).

- [ ] **Step 6: Commit the rename**

```bash
git add -A
git commit -m "refactor(flash): rename swd_v2n_host backend to swd_probe (external SWD probe) + fix gd32_bridge firmware_path" \
  -m "Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>"
```

### Phase B — add the SEGGER J-Link path (TDD)

- [ ] **Step 7: Write the failing J-Link tests**

Extend the import in `tests/scripts/test_flash_backends.py`:
```python
...
```
Add these tests in the `SwdProbeFlash` section:
```python
...
```
Also **replace** `test_swd_probe_requires_interface_and_target` so it forces the OpenOCD path (J-Link absent), keeping the guard meaningful:
```python
...
```
Expected: FAIL — `ImportError: cannot import name '_jlink_commander_script'` (and/or J-Link assertions fail; the backend has no J-Link path yet).

- [ ] **Step 9: Implement the J-Link path — full final `scripts/flash_backends/swd_probe.py`**

Replace the entire contents of `scripts/flash_backends/swd_probe.py` with:
```python
# SPDX-License-Identifier: Apache-2.0
"""
swd_probe -- flash a Cortex-M target over SWD with an external probe.
...
Primary path: SEGGER J-Link (`JLinkExe -device GD32G553xE`), which has
built-in GD32G553 flash support. OpenOCD / pyOCD are fallbacks for
CMSIS-DAP / ST-Link users (mainline OpenOCD's GD32G5 flash support is
...
- [ ] **Step 10: Run the J-Link tests, then the full suite**
...
python -m pytest tests/scripts/test_flash_backends.py tests/scripts/test_alp_orchestrate.py -q
```
Expected: the J-Link tests pass; the full suite passes (all `test_swd_probe_*` + openocd/pyocd fallbacks + orchestrate).

- [ ] **Step 11: Commit the J-Link feature**

```bash
git add scripts/flash_backends/swd_probe.py tests/scripts/test_flash_backends.py
git commit -m "feat(flash): add SEGGER J-Link primary path to swd_probe (GD32G553)" \
  -m "Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>"
```
...
> These document the OpenOCD/pyOCD fallback. They are **bench-unverified for GD32G5 flashing**; J-Link is the supported route. `openocd`/`pyocd` are not installed on this workstation, so correctness is confirmed at the bench.

- [ ] **Step 1: Create `scripts/openocd/cmsis-dap.cfg`**
```tcl
# SPDX-License-Identifier: Apache-2.0
# CMSIS-DAP SWD adapter for the GD32G553 (alternative path).
...
- [ ] **Step 2: Create `scripts/openocd/jlink.cfg`**
```tcl
# SPDX-License-Identifier: Apache-2.0
# Drive a SEGGER J-Link THROUGH OpenOCD. NOTE: the supported J-Link path uses
# SEGGER's own JLinkExe (scripts/flash_backends/swd_probe.py); this file is
# only for users who specifically want OpenOCD in the loop.
adapter driver jlink
transport select swd
adapter speed 2000
```

- [ ] **Step 3: Create `scripts/openocd/gd32g553.cfg`**
```tcl
# SPDX-License-Identifier: Apache-2.0
#
...
#
# BENCH-UNVERIFIED FOR FLASH PROGRAMMING. GD32G5 is not a stock OpenOCD flash
# target; the SUPPORTED route is SEGGER J-Link (`JLinkExe -device GD32G553xE`,
# see docs/gd32-flashing.md). This config brings the core up as a generic
# Cortex-M over SWD so connect / halt / IDCODE work for debug; PROGRAMMING via
# OpenOCD needs a GD32G5-aware OpenOCD build (e.g. GigaDevice's GD-Link).
...
# Internal-flash programming is intentionally NOT declared here: no mainline
# OpenOCD flash driver is confirmed for GD32G5. To program with OpenOCD, add a
# `flash bank` line for your GD32G5-aware build and VALIDATE the
# erase/program/verify cycle on a sacrificial part first. Otherwise use J-Link.
```

- [ ] **Step 4: Create `scripts/openocd/README.md`**
```markdown
...
`scripts/flash_backends/swd_probe.py` (`JLinkExe -device GD32G553xE`) — see
[`docs/gd32-flashing.md`](../../docs/gd32-flashing.md).
...
OpenOCD flash target; `gd32g553.cfg` declares no `flash bank`. For OpenOCD-based
*programming* you need a GD32G5-aware OpenOCD build (e.g. GigaDevice's GD-Link)
and must validate the erase/program/verify cycle on silicon. For routine
flashing, use J-Link.
```
...
Create `docs/gd32-flashing.md`. Replace the three `REPLACE_WITH_TASK1_*` tokens with the values captured in Task 1 Step 6 (no other placeholders permitted):
```markdown
# Flashing the GD32 bridge firmware

The GD32G553 supervisor on E1M-X V2N / V2N-M1 SoMs ships **pre-flashed by Alp**
— for normal use you do nothing. This SOP is for **bench bring-up, factory
first-flash, and recovery**: taking a blank or stale GD32 to a running bridge
over SWD.

> **Status (2026-06-01): v0.1.0 candidate — HIL-pending.** The transport
> firmware builds clean but has **not yet run on silicon**. Flashing it is a
> first-bring-up activity; expect to validate, not just install.
...
- A SWD probe. **Primary: SEGGER J-Link** (+ the SEGGER J-Link Software pack,
  which provides `JLinkExe`/`JLink`). Alternative: CMSIS-DAP or ST-Link via
  OpenOCD (see `scripts/openocd/`, bench-unverified for GD32G5).
...
> OTA is **not** armed (`-DBRIDGE_OTA_PARTITIONED` is OFF). Do not enable it for
> bench flashing — the partitioned bootloader is HIL-pending and a bad
> bootloader bricks the part (no host-driven SWD reflash this HW rev).

## 3. Wire the SWD probe

Connect the J-Link **directly to the GD32's own SWD header** — the V2N host is
not involved in flashing on this hardware revision. The header exposes the
GD32's debug port (`metadata/chips/gd32g553.yaml`):
...
Power the module from its normal supply; the probe senses `VTref`, it does not
power the target.
...
**Standalone — SEGGER J-Link** (the orchestrator generates the same script):
```bash
...
1. **Probe-level:** on connect, J-Link reports the SW-DP IDCODE — confirm it
   reads **`0x6BA02477`** (ARM Cortex-M33 DPv2). A mismatch means wrong
   target/wiring, not a firmware problem.
2. **Bridge-level:** run the host round-trip example and confirm `PING` +
   `GET_VERSION` succeed (see `examples/v2n/v2n-gd32-bridge-ping/README.md`):
   ```bash
   west alp-build examples/v2n/v2n-gd32-bridge-ping
   west flash    # onto the Renesas host that talks to the GD32
   ```
   - **Validate the SPI path first** — it is a clean point-to-point link.
   - The **I2C** management path shares **BRD_I2C**; on the V2M101 bench board
     that bus is currently wedged (see the bench handoff), so an I2C `PING`
     failure there is a board issue, not firmware.

## 6. First-silicon validation points

`hal/bridge_board_config.h` calls these out as the spots most likely to need
tuning on first run (none seen on real hardware yet):

- SPI: the master's CS-to-first-SCK setup vs the GD32's EXTI-trigger + DMA-arm
  latency. If the first reply byte is stale, increase the host's
...
re-run §4 with the probe. There is no host-driven SWD reflash on this HW rev, so
keep a probe on the bench. (The host-driven path lives behind `chips/gd32_swd/`
for a future rev; protocol §10 Path B.)
...
- [ ] **Step 2: Cross-link from `docs/gd32-bridge.md`** — replace `## Flashing` with:
```markdown
## Flashing
...
- [ ] **Step 4: Cross-link from `docs/firmware-quickstart.md`** — replace:
```markdown
...
python - <<'PY'
import re, pathlib
doc = pathlib.Path("docs/gd32-flashing.md")
...
**Files:** none modified (verification + integration).
...
be = flash_backends.lookup("swd_probe")
assert be is not None, "swd_probe not registered"
with patch("flash_backends.swd_probe.shutil.which",
...
- [ ] **Step 3: Full flash-backends + orchestrate suites green**
...
Hand off to the `superpowers:finishing-a-development-branch` skill, which presents the merge/PR/cleanup options. Default intent: merge `feat/gd32-flash-release` into `dev` with `--no-ff`. **Do not push** unless the user asks.
...
- §3 Flash tooling (J-Link primary; `swd_probe`) → Task 2 (rename + J-Link path, tested); §3 alternative (`scripts/openocd/`) → Task 3.
...
- §5 Verification → Task 1 Steps 4-6, Task 5 Steps 1-3; bench boundary documented in the SOP §5-§7.
- Deliverables row "rename + firmware_path fix" → Task 2 Phase A.
- Non-goals respected: OTA stays off; no transport-logic edits; no committed prebuilt blob (only a fingerprint table); no `chips/gd32_swd` work.

**Placeholder scan:** the only fill-ins are the three `REPLACE_WITH_TASK1_*` fingerprint values in Task 4 Step 1, sourced from Task 1 Step 6 — flagged explicitly.

**Type/name consistency:** `_jlink_commander_script(artefact, base, do_reset)` and class `SwdProbeFlash` / key `swd_probe` are referenced identically in the implementation (Task 2 Step 9) and the tests (Task 2 Steps 7, 10) and the Task 5 dry-run. `flash_args` keys (`base`, `jlink_device`, `jlink_speed`, `interface`, `target`, `use_pyocd`, `use_openocd`) match across backend, tests, presets, and SOP. No `swd_v2n_host` references remain outside the rename instructions in Task 2.
```
...
| v0.9.0  | **released 2026-07-06** | Portable-surface consistency batch (2026-07, PRs #319..#335): `<alp/version.h>` version + ABI feature-test macros; SE-backed SoC-identity / power-profile / peer-core-boot surfaces (`<alp/hw_info.h>` / `<alp/power.h>` / `<alp/mproc.h>`); I²C/SPI **target (slave) mode** + `alp_init`/`alp_deinit` in `<alp/peripheral.h>`; `<alp/i2c_regfile.h>` register-file I²C target helper over the experimental `alp_i2c_target_*` surface; `<alp/console.h>` `alp_console_companion_set` companion-console binder for the `alp companion` CLI verb; `alp_dac_capabilities`.  **Pre-1.0 breaking renames:** `alp_wdt_open(const alp_wdt_config_t *)` single-arg, ADC read entry points carry unit suffixes (`_mv`), pin/instance macros `E1M_*`→`ALP_E1M_*`.  Tooling (this-era shape, since retired -- see v0.12.0): the `alp` CLI single front door (`build`/`run`/`flash`/`emit`/`doctor`/`monitor`/`new-som`, `bootstrap.ps1`, one validator); som-preset-v1 schema tightened + `metadata/pinmux/v2n.yaml`; the portable-API **conformance suite** (13 classes × 8 cases) + capability-layer example adoption.  See CHANGELOG [v0.9.0]. |
| v0.10.0 | **released 2026-07-14** | Display/LVGL examples migrated onto `<alp/display.h>` / `<alp/gui.h>` (issue #23/#520, dropping direct Zephyr `display_*` calls); `west alp-migrate` board.yaml schema-migration engine (#610 WS6-b); quality-task registry (`metadata/quality-tasks-v1.json`) + `west alp-quality` profile runner (#610 §5); `build-receipt-v1` + deterministic SBOM/tarball (#610 §7 slice 2); GD32 bridge OTA Path-A hardening plus ADC oversample/PWM center-aligned/DSP dispatch; `tinygsm` + `libhelix` dropped from the curated library set (legal review, copyleft). See CHANGELOG [v0.10.0]. |
| v0.10.1 | **released 2026-07-14** | Bootstrap fix: `scripts/bootstrap.sh` now inits the workspace from alp-sdk's own `west.yml` so its extension commands register (#769, the then-live `west alp-build`/`alp-flash`/`alp-image`/`alp-clean`/`alp-renode` — since retired, see v0.12.0); SDK version single-sourced to `metadata/sdk_version.yaml`, README/docs de-versioned (#445). See CHANGELOG [v0.10.1]. |
| v0.11.0 | **released 2026-07-16** | ABI-freeze-gate test-isolation fix so the gate's own tests survive a version bump (#826); chip config setters (`bmi323`/`icm42670`/`lsm6dso`/`lis2dw12`) reject undeclared enum values instead of writing a reserved register encoding (#790); Yocto `packagegroup-alp-display` RDEPENDS fix. See CHANGELOG [v0.11.0]. |
| v0.11.1 | **released 2026-07-17** | E1M-AEN801 customer-path repair (#834): phantom `alp_e1m_evk_*` board name retired repo-wide (~50 files), board-overlay auto-apply fixed onto the fully-qualified board id (~35 examples), AEN `board.cmake` flash-capability claims corrected, VS Code onboarding doc fixed, anti-rot CI gates added (`pr-getting-started-aen801.yml` onboarding fixture, doc-drift board-alive check tied to `zephyr/boards/alp/<name>/board.yml`).  Also: Windows-vs-Linux digest/path-separator gate parity fixes (#829); ABI-snapshot version-literal + stale `\|\| true` gate residue fixed (#826/#795). See CHANGELOG [v0.11.1]. |
| v0.12.0 | **released 2026-07-22** | **ADR [0020](docs/adr/0020-sdk-owns-build-execution.md) end-state:** build execution moves entirely out of alp-sdk to the standalone, public **`tan` CLI** ([`alplabai/tan-cli`](https://github.com/alplabai/tan-cli)) — the `alp` console-script is retired (`alp_cli` survives only as `tan`'s Python backend, invoked as `python -m alp_cli <sub>`), and the `west alp-{build,image,flash,clean,size,renode}` extensions are retired; alp-sdk is plans-only from here on, emitting `--emit build-plan` / `--emit system-manifest` for `tan` to consume and execute.  Build-plan contract growth backing the cutover (all additive under `schemaVersion 1`, ADR 0014): `executionPolicy` (published skip-vs-fail rules), per-slice `envAppendPath`, envelope provenance (`sdkVersion`/`sdkCommit`), and hermetic path tokenization (`planPathMode: tokened` — `${SDK_ROOT}` / `${PROJECT_ROOT}` / `${PYTHON}` tokens a consumer substitutes at materialise time, issue #865).  `--emit scaffold` (issue #864): a SKU-parameterized new-project template contract (`metadata/templates/catalog-v1.json`) deriving per-SKU cores + portable pins across SoM families.  See CHANGELOG [v0.12.0]. |
| v0.13.0 | **released 2026-07-24** | New portable JPEG-**encoder** surface `<alp/jpeg.h>` (`[ABI-EXPERIMENTAL]`, encode-only, no decode API): a software TooJpeg baseline (`src/backends/jpeg/sw_baseline.c`) ships on every SoM, plus a HW-accelerated `alif_hantro` backend driving the Alif Ensemble E8 Hantro VC9000E encoder on **E1M-AEN801**, **silicon-proven** on real hardware (`JPEG_SWREG0` reads back `JPEG_HW_ID` `0x90001000`; a real HW encode round-tripped through libjpeg to a correct image).  `alp_pixfmt_t` (`[ABI-STABLE]`, additive) gains `ALP_PIXFMT_YUV420_PLANAR`/`ALP_PIXFMT_NV12`; the request/caps structs gain `format`/`pixfmt_mask` so a backend declares its accepted buffer layout(s); the HW backend enforces a DMA-reachable-buffer contract (rejects ITCM/DTCM-resident buffers, sourced from the DT `itcm`/`dtcm` nodes).  Also: runtime BLE GATT server/client read/write on the Zephyr backend + dynamic CC3501E GATT registration; `--emit kconfig` board-scoped Kconfig symbol menu for the LSP.  See CHANGELOG [v0.13.0]. |
| v0.14.0 | **released 2026-07-29** | Portable `<alp/i3c.h>` MIPI I3C Basic controller surface (`[ABI-EXPERIMENTAL]`, `alp_i3c_open/write/read/write_read/close/capabilities`) — Zephyr backend over upstream `i3c_dw.c` on the Alif Ensemble E8 (lpi3c0); controller init **bench-proven** on E1M-AEN801 (Flow C RAM-run, 2026-07-25), live transfer still unproven.  `metadata/bootstrap.json` + `metadata/toolchains.json` single-source the cross-platform bootstrap facts + Zephyr-SDK toolchain pin as data (#949); the `hw_rev`/SDK-version compatibility gate reinstated (#1019).  Zephyr v4.4.0 → v4.4.1 patch bump; Alif E8 SoC peripheral coverage extended (I3C, OSPI/HexSPI, managed-MDIO, build-only).  See CHANGELOG [v0.14.0]. |
| v0.15.0 | **tagged as release candidate only (`v0.15.0-rc1`, 2026-08-01); no final `v0.15.0` tag exists** (measured via `git tag --list`) | `alp_mproc_boot_core()`'s image-residency precondition + a direction-specific M55-HP TCM-invalidation erratum documented from E8 bench evidence (#1070); `CONFIG_DCACHE=n` fixed across eight AEN dual-core / SRAM0-beacon examples that were silently exposed to cross-core cache incoherence; five AEN examples stopped mis-reporting `ALP_ERR_NOSUPPORT` as an unreachable-in-practice SKIP (#1071).  The `hw_rev`-exists-but-not-buildable status gate (#1025, the status half) is NOT in this release: it landed on `dev` as `1a91a232` on 2026-08-02, after the 2026-08-01 `v0.15.0-rc1` tag (`git merge-base --is-ancestor 1a91a232 v0.15.0-rc1` exits non-zero), so it belongs to the next version.  `metadata/sdk_version.yaml` currently declares `version: 0.15.0` / `status: released` and `CHANGELOG.md` carries a dated `[v0.15.0] - 2026-07-31` section, but no plain `v0.15.0` git tag exists — see [`docs/abi/README.md`](docs/abi/README.md)'s v0.15.0 note.  See CHANGELOG [v0.15.0]. |
| v0.16.0 | **candidate (Unreleased)** | Ongoing dev-review hardening sweep: five memory-safety defects fixed across the AEN Zephyr drivers (#1119-#1124); a release-blocker where E1M-AEN801 M55-HP and M55-HE both linked to slot0 `0x80010000`, so flashing both cores silently overwrote one image (#1069); two path-traversal bugs in `tan model build` + untrusted template catalogs (#1125/#1126); per-instance RZ/V2N peripheral register data (#1154); several V2N/V2M metadata-declaration and build/boot fixes (#1155, #1158, #1161, #1166-#1170, #1175, #1176).  See CHANGELOG [Unreleased]. |
...
> no `v0.1.0`-`v0.5.0` tag (the first tag in the repo is `v0.6.0`,
> 2026-06-06) -- these five versions were never cut as individual
> releases, so their Status cells can never legitimately flip to
> `released` the way the versions below them did.  Their listed
> statuses (`in-progress`, `surface complete; impl in progress`,
> `wave-2 surface complete; HAL bodies pending`) describe development
> state at the time this file was last edited for that row, not a
> promise of an eventual tag; the work itself shipped folded into
> `v0.6.0` and later, untagged individually.

...
| v0.17.0 | **candidate (Unreleased)** | Current development: recovering an Alif E8 whose Secure Enclave is stuck in Recovery (`debugging-aen.md` §7.4, ROM → MRAM Recovery over SEUART, `0x4C013477` SW-DP ID check), and the fixes recorded under CHANGELOG [Unreleased] -- including the CC3501E wire-protocol 7 to 8 bump (request identity for every worker-routed opcode), which is HELD until cc3501e-bridge-firmware v0.6.0 ships. |

...
| v0.8.0  | planned (2026-09-17) | V2M101 (DEEPX) silicon-verified + Ubuntu backend (`cores.<id>.os: ubuntu`) + OTA E2E.  Concurrent multi-NPU dispatch (DRP-AI3 + DEEPX simultaneously) proven on V2M101.  Ubuntu image flow lands as alternative to Yocto on V2N/V2M A-cluster (apt-based, customer can `apt install alp-sdk` from a PPA or boot a pre-baked image).  Mender E2E demonstrated on V2N101 fleet of N≥3 boards (signed artefact + swap-using-scratch + rollback). |

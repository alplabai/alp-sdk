...
static void _free_in(struct alp_audio_in *h) { h->in_use = false; }
...
static void _free_out(struct alp_audio_out *h) { h->in_use = false; }
...
alp_status_t alp_audio_in_read(alp_audio_in_t *in, void *buf, size_t frames,

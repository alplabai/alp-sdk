                                                 ...
                                                 const alp_inference_config_t *cfg)
{
    ...
    try {
        st->engine = new (std::nothrow) dxrt::InferenceEngine(
            static_cast<const uint8_t *>(cfg->model_data), cfg->model_size,
...
extern "C" alp_status_t alp_inference_deepx_get_input(struct alp_inference *h_, std::size_t index,
...
extern "C" alp_status_t alp_inference_deepx_get_output(struct alp_inference *h_, std::size_t index,
                                                       alp_inference_tensor_t *out)
{
    auto *h  = reinterpret_cast<alp_inference_handle_layout *>(h_);
    auto *st = static_cast<DeepxState *>(h->be_state);

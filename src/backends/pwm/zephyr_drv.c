...
static const struct pwm_dt_spec _specs[] = {
	ALP_PWM_SPEC_0_INIT, ALP_PWM_SPEC_1_INIT, ALP_PWM_SPEC_2_INIT, ALP_PWM_SPEC_3_INIT,
	ALP_PWM_SPEC_4_INIT, ALP_PWM_SPEC_5_INIT, ALP_PWM_SPEC_6_INIT, ALP_PWM_SPEC_7_INIT,
};
...
z_open(const alp_pwm_config_t *cfg, alp_pwm_backend_state_t *st, alp_capabilities_t *caps_out)
{
	...
	h->flags =
	    (cfg->polarity == ALP_PWM_POLARITY_INVERTED) ? PWM_POLARITY_INVERTED : PWM_POLARITY_NORMAL;
...
static alp_status_t z_set_duty(alp_pwm_backend_state_t *st, uint32_t pulse_ns)
{
	...
	return _errno_to_alp(pwm_set(dev, h->channel, h->period_ns, pulse_ns, h->flags));
}
...
static alp_status_t z_single_pulse(alp_pwm_backend_state_t *st, uint32_t pulse_ns)
{
...
static void z_capture_close(alp_pwm_backend_state_t *st)
{
	(void)st;
}

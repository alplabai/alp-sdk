...
static void _isr_thunk(const struct device *port,
...
static alp_status_t z_open(uint32_t pin_id,
...
static alp_status_t z_configure(alp_gpio_backend_state_t *st,
                                alp_gpio_dir_t dir,
...
static alp_status_t z_irq_enable(alp_gpio_backend_state_t *st,
                                 alp_gpio_edge_t edge,
                                 alp_gpio_cb_t cb,
                                 void *user) {
    (void)cb; (void)user;     /* stashed in the portable handle by the dispatcher */
    alp_z_gpio_side_t *s = (alp_z_gpio_side_t *)st->be_data;
    if (s == NULL) return ALP_ERR_NOT_READY;

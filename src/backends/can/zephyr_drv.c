...
static void _rx_trampoline(const struct device *dev,
...
static alp_status_t z_open(const alp_can_config_t *cfg,
    ...
    if (err != 0) { _free_side(s); return _errno_to_alp(err); }
    ...
    if (cfg->mode == ALP_CAN_MODE_FD && cfg->bitrate_data_hz > 0u) {
        ...
        err = can_set_mode(dev, cfg->loopback ? CAN_MODE_LOOPBACK
                                              : CAN_MODE_NORMAL);
    }
    if (err != 0) { _free_side(s); return _errno_to_alp(err); }
...
static alp_status_t z_send(alp_can_backend_state_t *st,
    ...
    return _errno_to_alp(can_send(dev, &zf, K_MSEC(timeout_ms),
                                  NULL, NULL));
}
...
static alp_status_t z_add_filter(alp_can_backend_state_t *st,
    ...
    for (int i = 0; i < MAX_FILTERS; ++i) {
        if (s->cb_table[i].cb == NULL) { slot = i; break; }
    }
...
static alp_status_t z_remove_filter(alp_can_backend_state_t *st,
                                    int32_t filter_id) {
    const struct device *dev = (const struct device *)st->dev;

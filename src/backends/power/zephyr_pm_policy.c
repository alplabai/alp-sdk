 ...
 * Yocto / Linux path
 * ------------------
 * This backend is Zephyr-only (it includes <zephyr/pm/policy.h>) and
 * never compiles into a Yocto/Linux build.  The Linux counterpart is
 * src/backends/power/yocto_drv.c (issue #613): `/sys/power/state`
 * write + `/sys/class/rtc/rtc0/wakealarm`, registered at the same
 * priority 100 but self-gated on __linux__ instead of CONFIG_PM.
 */

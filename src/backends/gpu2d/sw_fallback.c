...
static uint32_t _blend_px(uint32_t src, uint32_t dst, alp_gpu2d_blend_mode_t mode)
{
	...
	case ALP_GPU2D_BLEND_SRC_OVER: {
		...
		/* issue #1117 round-2 dev review: cast each _u8() result to
		 * uint32_t BEFORE shifting -- _u8() returns uint8_t, which
		 * integer-promotes to (signed) int, so `_u8(oa) << 24` is
		 * signed-int UB whenever oa clamps to >= 0x80 (alpha >= 128,

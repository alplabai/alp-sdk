                              ...
                              alp_capabilities_t *caps_out)
{
    ...
    be->dev = dev;
    int err = k_mem_slab_init(&be->slab, be->slab_buf, block_bytes,
                              CONFIG_ALP_SDK_AUDIO_IN_SLAB_BLOCKS);
    if (err != 0) {
        free_in_be(be);
                              ...
                              uint32_t timeout_ms)
{
    ...
    if (state->cfg.format == ALP_AUDIO_FMT_S16_LE) {
        dc_block_s16((int16_t *)buf, want, state->cfg.channels,
                     be->dc_x_prev, be->dc_y_prev);
    }

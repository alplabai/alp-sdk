...
static alp_status_t z_hash_open(alp_hash_alg_t alg,
...
static alp_status_t z_hash_update(alp_hash_backend_state_t *state,
                                   ...
                                   uint8_t *tag_out, size_t tag_len)
{
    ...
    size_t       produced = 0;
    psa_status_t st       = psa_aead_encrypt(be->key_id, psa_alg, iv, iv_len, aad, aad_len,
                                             plain, plain_len, scratch, sizeof(scratch),
                                             &produced);
    if (st != PSA_SUCCESS) return psa_to_alp(st);
                                   ...
                                   uint8_t *plain_out)
{
    ...
    size_t       produced = 0;
    psa_status_t st       = psa_aead_decrypt(be->key_id, psa_alg, iv, iv_len, aad, aad_len,
                                             scratch, cipher_len + tag_len, plain_out,
                                             cipher_len, &produced);
    return psa_to_alp(st);
#else
    (void)state;

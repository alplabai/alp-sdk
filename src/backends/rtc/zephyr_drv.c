...
#define ALP_RTC_DEV_OR_NULL(idx) \
	COND_CODE_1(DT_NODE_HAS_STATUS(DT_ALIAS(_CONCAT(alp_rtc, idx)), okay), \
...
static alp_status_t z_set_time(alp_rtc_backend_state_t *st, const alp_rtc_time_t *t)
{
...
static alp_status_t z_get_time(alp_rtc_backend_state_t *st, alp_rtc_time_t *t)
{
	const struct device *dev = (const struct device *)st->dev;
	struct rtc_time      zt;

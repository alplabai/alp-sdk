 ...
 *                     down on it) should not see one.  isp_close() passes
 *                     true unconditionally -- it needs the real stop. */
static void _release_vbufs(alp_v2n_n44_isp_state_t *st, bool stop_stream)
{
	 ...
	 * returns non-zero cannot hang this close() path forever. */
...
/* through the portable v4.4 video API.  No V2N-specific MMIO yet; */
/* the N44 SoC port wires its MIPI CSI-2 IP up to drivers/video/,  */
/* at which point these calls drive real silicon unchanged.        */
/* ============================================================== */

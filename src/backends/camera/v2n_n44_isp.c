...
static alp_status_t _errno_to_alp(int err)
{
    ...
    case -EAGAIN:     return ALP_ERR_TIMEOUT;
...
{
    switch (fmt) {
    case ALP_PIXFMT_RGB565:   return VIDEO_PIX_FMT_RGB565;
    case ALP_PIXFMT_RGB888:   return VIDEO_PIX_FMT_RGB24;
    case ALP_PIXFMT_ARGB8888: return VIDEO_PIX_FMT_XRGB32;
    default:                  return 0u;
    }
}
                             ...
                             alp_capabilities_t *caps_out)
{
    ...
    for (uint8_t i = 0; i < want; ++i) {
        ...
        if (st->vbufs[i] == NULL) {
            for (uint8_t j = 0; j < i; ++j) st->vbufs[j] = NULL;
            _free_state(st);
...
static alp_status_t isp_capture(alp_camera_backend_state_t *state,
...
static alp_status_t isp_release(alp_camera_backend_state_t *state,

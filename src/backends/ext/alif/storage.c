 ...
 * 2617-0001), and E1M-AEN801 populates NEITHER external OSPI memory on
 * any revision (metadata/e1m_modules/E1M-AEN801.yaml:
 * on_module.ospi_memories.ospi0.assembled: false) -- it runs from the
 * SoC's on-die MRAM instead.  This call hardcodes
 * OSPI_WRITE_EXTERNAL_KEY_OSPI0 because that is the only OSPI bus any
 * AEN SKU wires at all; E1M-AEN803 is the SKU that populates OSPI0 on
 * this shared PCB, and issue #2041 measured that part as an ISSI
 * IS25WX256-JHLE on CS1 -- not the Macronix MX25UM25645GXDI00 this
 * comment previously named.  The SE-transport half of this call --
 * packet marshalling, send/response,

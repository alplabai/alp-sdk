...
static void _free(struct alp_i2s *h) { h->in_use = false; }

alp_i2s_t *alp_i2s_open(const alp_i2s_config_t *cfg) {
    alp_z_clear_last_error();
    if (cfg == NULL || cfg->channels == 0u || cfg->channels > 2u ||
        cfg->block_frames == 0u) {
        alp_z_set_last_error(ALP_ERR_INVAL); return NULL;
    }
    if (cfg->word_bits != 8u && cfg->word_bits != 16u &&
        cfg->word_bits != 24u && cfg->word_bits != 32u) {
        alp_z_set_last_error(ALP_ERR_INVAL); return NULL;
    }

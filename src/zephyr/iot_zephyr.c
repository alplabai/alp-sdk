...
alp_status_t alp_wifi_connect(alp_wifi_t *w,
    ...
    int err = net_mgmt(NET_REQUEST_WIFI_CONNECT, w->iface,
...
static int parse_broker_uri(const char *uri,
    ...
    if (strncmp(cursor, "mqtts://", 8) == 0) { tls = true; cursor += 8; }
    else if (strncmp(cursor, "mqtt://", 7) == 0) { tls = false; cursor += 7; }
    else { return -EINVAL; }
...
static int resolve_broker_addr(const char *host, uint16_t port,
...
static void alp_mqtt_evt_cb(struct mqtt_client *client,
    ...
    case MQTT_EVT_PUBLISH: {
        ...
        if (m->msg_cb == NULL) {
            ...
            while (remaining > 0) {
                int n = mqtt_read_publish_payload(client, scratch,
        ...
        size_t got  = 0;
        while (got < want) {
            int n = mqtt_read_publish_payload(client, m->rx_buf + got,
                                              want - got);
            if (n <= 0) break;
            got += (size_t)n;
        }
...
alp_mqtt_t *alp_mqtt_open(const alp_mqtt_config_t *cfg) {
    ...
    uint16_t port;
    bool tls;
    int err = parse_broker_uri(cfg->broker_uri, host, sizeof(host),
                               &port, &tls);
    if (err != 0) {
        alp_z_set_last_error(ALP_ERR_INVAL);
        mqtt_pool_release(h);
    ...
    h->client.transport.type  = tls ? MQTT_TRANSPORT_SECURE

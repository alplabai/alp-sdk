...
static struct alp_rpc_sub *sub_find(struct alp_rpc_channel *ch,
                                    const char *method, uint32_t hash)
{
...
alp_rpc_channel_t *alp_rpc_open(const alp_rpc_config_t *cfg)
{
    ...
    if (rc < 0) {
        LOG_ERR("rpc: ipc_service_register_endpoint(%s) failed: %d",
                ch->name, rc);
        ch->in_use = false;
        alp_z_set_last_error(errno_to_alp(rc));
        return NULL;
    ...
    LOG_INF("rpc: opened %s src=0x%x dst=0x%x mbox=%u",
            ch->name, ch->src_ept, ch->dst_ept, ch->mbox_ch);
    return ch;
    ...
    if (sub == NULL) {
        ...
        if (sub == NULL) {
            LOG_WRN("rpc: subscribe table full on %s (cap=%d)",
                    ch->name, CONFIG_ALP_SDK_RPC_SUBS_PER_CHANNEL);

...
static int cmd_res(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh,
	            "    -- total stack %u B, free %u B",
	            (unsigned int)acc.total_size,
	            (unsigned int)acc.total_free);
#else
...
SHELL_SUBCMD_ADD((alp), res, NULL, "Live resources: cores, memory, thread stacks", cmd_res, 1, 0);

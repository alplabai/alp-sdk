...
alp_status_t alp_usb_device_write(alp_usb_dev_t *h, const uint8_t *data, size_t len,
                                  uint32_t timeout_ms)
{
	if (h == NULL || !h->in_use) return ALP_ERR_NOT_READY;
...
alp_status_t alp_usb_device_read(alp_usb_dev_t *h, uint8_t *data, size_t len, size_t *out_len,
                                 uint32_t timeout_ms)
{

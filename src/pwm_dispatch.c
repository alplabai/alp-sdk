...
static void _free(struct alp_pwm *h) { h->in_use = false; }
...
static void _free_cap(struct alp_pwm_capture *h) { h->in_use = false; }
...
alp_status_t alp_pwm_configure(alp_pwm_t *pwm, alp_pwm_align_t align_mode,
    ...
    return pwm->state.ops->configure(&pwm->state, align_mode,
...
alp_pwm_capture_t *alp_pwm_capture_open(const alp_pwm_capture_config_t *cfg) {
    ...
    if (cfg->channel_id >= 8u) {
        alp_z_set_last_error(ALP_ERR_OUT_OF_RANGE); return NULL;
    }
    if ((unsigned)cfg->edge > (unsigned)ALP_PWM_CAPTURE_EDGE_BOTH) {
        alp_z_set_last_error(ALP_ERR_INVAL); return NULL;
    }
    const alp_backend_t *be = alp_backend_select("pwm", ALP_SOC_REF_STR);
    if (be == NULL) { alp_z_set_last_error(ALP_ERR_NOT_PRESENT_ON_THIS_SOC); return NULL; }
    const alp_pwm_ops_t *ops = (const alp_pwm_ops_t *)be->ops;
    if (ops == NULL || ops->capture_open == NULL) {
        alp_z_set_last_error(ALP_ERR_NOSUPPORT); return NULL;
    }
    ...
    if (rc != ALP_OK) { _free_cap(h); alp_z_set_last_error(rc); return NULL; }
    h->cached_caps = caps;
    return h;
}

alp_status_t alp_pwm_capture_read(alp_pwm_capture_t *cap,

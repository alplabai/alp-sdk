/*
 * alp-sdk VENDOR OVERRIDE of GigaDevice's system_gd32g5x3.c (GD32G5x3 firmware
 * library v1.5.0).  Compiled IN PLACE OF the submodule copy by
 * vendors/gd32_firmware_library/CMakeLists.txt, so the upstream submodule stays
 * pristine (clean `git submodule update` diffs) -- mirroring how the FSP
 * r_sci_b_spi module is vendored under zephyr/drivers/spi/r_sci_b_spi/.
 *
 * The ONLY change from the stock file is the system-clock selection.  Stock
 * selects 216M-PLL-HXTAL, but on the E1M-V2M101 SoM the GD32 HXTAL input
 * (5L35023B SE2, an external single-pin oscillator) is configured over BRD_I2C
 * -- held low by the DX-M1 -- so it never starts: SystemInit() then hangs
 * forever in system_clock_216m_hxtal() waiting for HXTALSTB, never reaching
 * main() (SPI/I2C never come up).  This override selects 216M-PLL-IRC8M
 * (internal 8 MHz RC -> 216 MHz) instead, and adds RCU_CTL_HXTALBPS in the
 * (currently unused) HXTAL path for when SE2 is later brought up.  Grep
 * "E1M-V2M101" below for the exact edits.  The Arm + GigaDevice copyright and
 * the Apache-2.0 licence below are unmodified.
 */
...
/* E1M-V2M101: HXTAL (5L35023B SE2, 8 MHz) is UNUSED for now -- the 5L35023B
 * is configured over BRD_I2C which the DX-M1 holds low, so SE2 may be inactive.
 * Clock the GD32 from internal IRC8M (216M_PLL_IRC8M above) until I2C is fixed. */
...
/* E1M-V2M101 SoM: HXTAL = 5L35023B SE2, an 8 MHz single-pin clock (needs HXTAL
 * bypass; see system_clock_216m_hxtal).  8/2 * 108 / 2 = 216 MHz.  Currently
 * UNUSED (IRC8M is selected) until the 5L35023B is configurable over BRD_I2C. */
#define PLLPSC              (2U - 1U)
#define PLLN                (108U)
#define PLLP                ((2U >> 1U) - 1U)
#define PLLQ                (2U)
...
static void system_clock_216m_hxtal(void)
{
    ...
    /* E1M-V2M101 SoM: the GD32 HXTAL input is a single-pin EXTERNAL OSCILLATOR
     * (clock fed into OSC_IN), NOT a 2-pin crystal.  Enable HXTAL bypass so the
     * RCU takes the external clock directly; without it HXTALSTB never asserts
     * in crystal-drive mode and SystemInit hangs here forever (never reaching
     * main(), so SPI/I2C never come up).  Must be set while HXTALEN==0. */
    RCU_CTL |= RCU_CTL_HXTALBPS;

 ...
 * usable from the SDK, though: the chips/tas2563 driver has no
 * write-only path at all (tas2563_init() rejects
 * TAS2563_I2C_ADDR_BROADCAST on every board revision, respun or
 * not), so firmware unconditionally has to issue two targeted
 * unit-address writes back-to-back rather than a 0x48 broadcast.
 *
 ...
 * cannot use 0x48 on either revision anyway (the driver has no
 * write-only path), so the two-write workaround is unconditional,
 * not conditional on which revision you support.
 *

...
#define EVK_I2C_ADDR_TAS2563_HIGH  0x4Eu  /**< U28 smart amp (AD0 = 10k to VDD). The TAS2563 broadcast address (0x48) was occupied on PRE-RESPIN boards by U32 INA236B (+V_CAM0 rail); the U32 re-strap to 0x4B from the next batch freed 0x48 at the hardware level. That does not make it usable from the SDK: the chips/tas2563 driver has no write-only path and tas2563_init() rejects 0x48 on every board revision, so firmware must unconditionally issue two targeted unit-address writes rather than a 0x48 broadcast. */

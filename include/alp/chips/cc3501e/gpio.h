...
/*
 * A THIRD internal list, cc3501e_gpio_rev_dependent[] -- the portable
 * pin_ids whose target chip moves across AEN hw_revs (IO8/IO10/IO21 --
 * issue #2144) -- is deliberately NOT declared here.  Unlike the two lists
 * above, it is not board-provided: it is the SAME set on every AEN board,
 * so it has exactly one, SDK-owned definition
 * (src/backends/gpio/cc3501e_rev_dependent_pins.c, generated by
 * scripts/gen_cc3501e_gpio_routes.py) that src/backends/gpio/cc3501e_proxy.c
 * declares and reads internally -- not part of this public surface.
 */

...
/**
 * Board-provided list of portable pin_ids that are REVISION-DEPENDENT --
 * their target chip (CC3501E vs the Alif SoC) metadata/e1m_modules/aen/
 * hw-revisions.yaml `pad_route_overrides:` moves across AEN hw_revs (IO8,
 * IO10, IO21 today).  `scripts/gen_cc3501e_gpio_routes.py` derives this list
 * from that metadata and emits it into every board's generated
 * cc3501e_gpio_routes.c alongside cc3501e_gpio_routes[] itself; a
 * hand-written table follows the same shape.  alp_gpio_open() on a listed
 * pin_id returns @ref ALP_ERR_NOSUPPORT unless a CRC-valid identity-EEPROM
 * manifest confirms the running module's hw_rev equals
 * CONFIG_ALP_SDK_SOM_HW_REV -- the hw_rev this board's route table was
 * built for -- FAILING CLOSED on a missing/unreadable/mismatched manifest,
 * per pin, rather than the old all-or-nothing hw_rev guard (#1859) this
 * replaces (#2144).  WEAK empty default in the proxy backend: an un-mapped
 * build treats every pin as revision-independent (no behaviour change until
 * populated from the SoM pad map). */

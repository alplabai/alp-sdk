 ...
 *
 * @par Allocation
 *      CALLER-allocated, no heap.  Unlike @ref alp_dsp_chain_t
 *      (library-owned, drawn from a static backend pool), this struct is
 *      a plain value type -- declare it on the stack, `static`, or inside
 *      another struct, and pass its address to @ref alp_dsp_decimator_init.
 *      The library never allocates and keeps no hidden registry of
 *      outstanding decimators.
 *
 * @par Do not access fields directly. Layout is exposed only so callers can
 *      allocate storage without a pool; treat it as opaque otherwise.
 */
typedef struct {
 ...
 *
 * @par Supported ratios: 2, 3, 4, 6 -- the set that serves 48000 Hz ->
 *      16000/8000 Hz (ratios 3, 6) and 32000 Hz -> 16000/8000 Hz (ratios
 *      2, 4).  Any other ratio (including 1, and non-integer resampling)
 *      is rejected; this issue is integer-ratio decimation only.
 *
 * @par Filter design (identical method for every ratio -- generated by
 *      `scripts/gen_dsp_decimator_coeffs.py`, Kaiser-windowed-sinc,
 *      @ref ALP_DSP_DECIMATOR_TAPS = 135 taps, Q15 fixed-point, unity DC
 *      gain): the achieved passband edge, passband ripple, stopband
 *      attenuation (above the OUTPUT Nyquist, `fs_in / (2*ratio)`), and
 *      group delay in OUTPUT samples are:
 *        - ratio 2 (32000->16000 Hz): passband edge 7467.7 Hz, ripple
 *          0.17 dB, stopband attenuation 39.7 dB, group delay ~33.5
 *          output samples.
 *        - ratio 3 (48000->16000 Hz): passband edge 7201.6 Hz, ripple
 *          0.18 dB, stopband attenuation 39.9 dB, group delay ~22.3
 *          output samples.
 *        - ratio 4 (32000->8000 Hz): passband edge 3467.7 Hz, ripple
 *          0.17 dB, stopband attenuation 39.4 dB, group delay ~16.8
 *          output samples.
 *        - ratio 6 (48000->8000 Hz): passband edge 3201.6 Hz, ripple
 *          0.17 dB, stopband attenuation 39.3 dB, group delay ~11.2
 *          output samples.
 ...
 *                      be non-NULL.
 *
 * @return @ref ALP_OK, or @ref ALP_ERR_INVAL if @p dec is NULL or was
 *         never initialised.
 */

 ...
 *
 * Every analog/timer-default pad has a parallel `ALP_E1M_GPIO_<class><N>`
 * GPIO index.  An app that doesn't need the ADC/PWM/QENC/DAC
 * function of a pad can claim it as a digital GPIO by opening the
 * matching `ALP_E1M_GPIO_<class><N>` index -- the board's devicetree
...
/* DAC channel IDs: use the pin-granular ALP_E1M_DAC0 / ALP_E1M_DAC1
 * defined above in the peripheral-instance section.  The earlier
 * `ALP_E1M_DAC_CH0` / `_CH1` aliases (same values) were removed
 ...
 * Indices 42..49.
 *
 * Note: the peripheral-instance ID `ALP_E1M_ADC<N>` (0..7) passed to
 * `alp_adc_open()` and the GPIO-index `ALP_E1M_GPIO_ADC<N>` (42..49)
 ...
 * convention. */
#define ALP_E1M_GPIO_ADC0 42u
#define ALP_E1M_GPIO_ADC1 43u
#define ALP_E1M_GPIO_ADC2 44u
#define ALP_E1M_GPIO_ADC3 45u
#define ALP_E1M_GPIO_ADC4 46u
#define ALP_E1M_GPIO_ADC5 47u
#define ALP_E1M_GPIO_ADC6 48u
#define ALP_E1M_GPIO_ADC7 49u

/* DAC output pads as GPIO indices (silkscreen ANA_OUT0 / OUT1).
 * Same pin-as-GPIO fallback as ADC above.  Indices 50..51. */
#define ALP_E1M_GPIO_DAC0 50u
#define ALP_E1M_GPIO_DAC1 51u

/** Total number of GPIO-capable indices in this header.
 *  Boards' `alp,pin-array` arrays must list this many entries. */
#define ALP_E1M_GPIO_COUNT 52u

 ...
 *
 * One-shot ADC reads against studio-resolved channel indices, plus
 * DMA-backed continuous-acquisition streaming on SoMs whose backend
 * supports it.  The channel-config side (resolution, gain, reference,
 * acquisition time) lives in devicetree on the Zephyr backend; this
 * header only exposes the runtime knobs apps actually tune.
 *
 * The DAC surface moved to its own @ref dac.h header.
 *
 * Backends:
 *   - Zephyr   : `adc_*` driver class.  On the V2N family
 *                (V2N + V2N-M1, both of which carry the GD32G553
 *                supervisor MCU) the SDK routes through the GD32

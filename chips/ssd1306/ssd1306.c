...
void ssd1306_draw_pixel(ssd1306_t *dev,
    ...
    if (on) dev->fb[idx] |= mask;
    else    dev->fb[idx] = (uint8_t)(dev->fb[idx] & ~mask);
}
...
alp_status_t ssd1306_display(ssd1306_t *dev) {
    ...
    uint8_t win_cmd[7] = {
        CTRL_CMD_STREAM,
        CMD_SET_COLUMN_ADDR, 0x00u, cols,
        CMD_SET_PAGE_ADDR,   0x00u, (uint8_t)(pages - 1u)
    };
    ...
    while (off < fb_bytes) {
        ...
        if (n > CHUNK) n = CHUNK;
        for (size_t i = 0; i < n; i++) buf[1 + i] = dev->fb[off + i];

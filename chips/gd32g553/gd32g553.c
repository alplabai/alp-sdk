...
static uint32_t
reply_staging_gap_us(uint8_t cmd, const uint8_t *req_payload, size_t req_payload_len)
{
...
alp_status_t gd32g553_init(gd32g553_t *ctx, alp_spi_t *spi, alp_i2c_t *i2c, uint8_t i2c_addr_7bit)
{
	...
	if (ctx->spi != NULL) {
		...
		uint8_t granted = 0u;
		(void)cmd_send(
		    ctx, GD32G553_TRANSPORT_SPI, GD32G553_CMD_LINK_FEATURES, &want, 1u, &granted, 1u);
	}
	return ALP_OK;
}

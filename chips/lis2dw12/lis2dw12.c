...
static alp_status_t reg_read(lis2dw12_t *dev, uint8_t reg, uint8_t *out, size_t len)
{
...
lis2dw12_set_accel(lis2dw12_t *dev, lis2dw12_odr_t odr, lis2dw12_fs_t fs, lis2dw12_mode_t mode)
{
	...
	switch (mode) {
	case LIS2DW12_MODE_LOW_POWER_12BIT:
		mode_bits = 0x0;
		lp_bits   = 0x0;
		break;
	case LIS2DW12_MODE_LOW_POWER_14BIT:
		mode_bits = 0x0;
		lp_bits   = 0x1;
		break;
	case LIS2DW12_MODE_HIGH_PERF_14BIT:
		mode_bits = 0x1;
		lp_bits   = 0x0;
		break;
	case LIS2DW12_MODE_SINGLE_SHOT:
		mode_bits = 0x2;
		lp_bits   = 0x0;
		break;
	default:
	...
	const uint8_t ctrl1 =
	    (uint8_t)((((uint8_t)odr & 0x0Fu) << 4) | ((mode_bits & 0x03u) << 2) | (lp_bits & 0x03u));

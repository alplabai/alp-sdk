...
static alp_status_t reg_read16(bmi323_t *dev, uint8_t reg,
                               uint8_t *out, size_t words) {
    ...
    uint8_t scratch[32];
    alp_status_t s = alp_i2c_write_read(dev->bus, dev->addr,
                                        &reg, 1,
                                        scratch, total);
...
alp_status_t bmi323_set_accel(bmi323_t *dev,
    ...
    uint16_t v = (uint16_t)((((uint16_t)fs  & 0x07u) << 4)
                          |  ((uint16_t)odr & 0x0Fu));

...
alp_status_t deepx_dxm1_shut_down(deepx_dxm1_t *ctx)
{
	...
	alp_status_t s = alp_gpio_write(ctx->m1_reset_pin, asserted_level(ctx->reset_polarity));
	if (s != ALP_OK) return s;
...
void deepx_dxm1_deinit(deepx_dxm1_t *ctx)
{
	...
	if (ctx->initialised) {
		...
		if (ctx->m1_reset_pin != NULL) {
			(void)alp_gpio_write(ctx->m1_reset_pin, asserted_level(ctx->reset_polarity));
		}

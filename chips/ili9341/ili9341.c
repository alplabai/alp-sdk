 ...
 *
 * Ilitek ILI9341 TFT driver.  See <alp/chips/ili9341.h>.
 */
...
static alp_status_t ili9341_send(ili9341_t *dev, bool is_cmd, const uint8_t *buf, size_t len)
{
...
static alp_status_t ili9341_cmd(ili9341_t *dev, uint8_t c)
{
	return ili9341_send(dev, true, &c, 1);
}

static alp_status_t ili9341_data(ili9341_t *dev, const uint8_t *buf, size_t len)
{
	return ili9341_send(dev, false, buf, len);
}
...
alp_status_t ili9341_init(ili9341_t *dev, alp_spi_t *spi, alp_gpio_t *dc, alp_gpio_t *reset)
{
	 ...
	 * thread.  alp_delay_ms keeps the "at least" guarantee the ILI9341
	...
	if (s != ALP_OK) return s;
	s = ili9341_data(dev, &pixfmt, 1);
	if (s != ALP_OK) return s;

	uint8_t madctl = 0x48;
	s              = ili9341_cmd(dev, ILI9341_CMD_MADCTL);
	if (s != ALP_OK) return s;
	s = ili9341_data(dev, &madctl, 1);
	if (s != ALP_OK) return s;

	s = ili9341_cmd(dev, ILI9341_CMD_DISPON);
	if (s != ALP_OK) return s;
...
alp_status_t ili9341_set_window(ili9341_t *dev, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
	...
	if (x1 >= ILI9341_WIDTH || y1 >= ILI9341_HEIGHT) return ALP_ERR_INVAL;
	...
	alp_status_t s = ili9341_cmd(dev, ILI9341_CMD_CASET);
	if (s != ALP_OK) return s;
	s = ili9341_data(dev, buf, sizeof(buf));
	if (s != ALP_OK) return s;
	...
	s      = ili9341_cmd(dev, ILI9341_CMD_PASET);
	if (s != ALP_OK) return s;
	return ili9341_data(dev, buf, sizeof(buf));
}
...
alp_status_t ili9341_write_pixels(ili9341_t *dev, const uint8_t *pixels, size_t n_bytes)
{
	...
	alp_status_t s = ili9341_cmd(dev, ILI9341_CMD_RAMWR);
	if (s != ALP_OK) return s;
	return ili9341_data(dev, pixels, n_bytes);
}

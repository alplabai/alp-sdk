                                 ...
                                 bool *full_duplex)
{
    ...
    *up = (bmsr & RTL8211FDI_BMSR_LINK_STATUS) != 0u;
    ...
    switch (spd_bits) {
    case 0x0u: *speed = RTL8211FDI_SPEED_10M;   break;
    case 0x1u: *speed = RTL8211FDI_SPEED_100M;  break;
    case 0x2u: *speed = RTL8211FDI_SPEED_1000M; break;
    default:   *speed = RTL8211FDI_SPEED_UNKNOWN; break;
    }
...
alp_status_t rtl8211fdi_wol_set_mac(rtl8211fdi_t *ctx, const uint8_t mac[6])
{
    ...
    s = mdio_write_raw(ctx, RTL8211FDI_WOL_MAC_REG0,
                       (uint16_t)mac[0] | ((uint16_t)mac[1] << 8));
    if (s != ALP_OK) goto out;
    s = mdio_write_raw(ctx, RTL8211FDI_WOL_MAC_REG1,
                       (uint16_t)mac[2] | ((uint16_t)mac[3] << 8));
    if (s != ALP_OK) goto out;
    s = mdio_write_raw(ctx, RTL8211FDI_WOL_MAC_REG2,
                       (uint16_t)mac[4] | ((uint16_t)mac[5] << 8));
out:
    (void)page_select(ctx, RTL8211FDI_PAGE_DEFAULT);
    return s;
}
...
alp_status_t rtl8211fdi_wol_enable(rtl8211fdi_t *ctx, bool enable)
{
    ...
    if (s != ALP_OK) goto out;
    if (enable) ctrl |= RTL8211FDI_WOL_CTRL_MAGIC_PKT_EN;
    else        ctrl &= (uint16_t)~RTL8211FDI_WOL_CTRL_MAGIC_PKT_EN;

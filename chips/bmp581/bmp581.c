...
static alp_status_t reg_read(bmp581_t *dev, uint8_t reg,
...
alp_status_t bmp581_set_sampling(bmp581_t *dev,
    ...
    uint8_t osr = (uint8_t)((1u << 6)
                          | (((uint8_t)press_osr & 0x07u) << 3)
                          |  ((uint8_t)temp_osr  & 0x07u));
    alp_status_t s = reg_write(dev, REG_OSR_CONF, osr);
    if (s != ALP_OK) return s;

    /* ODR_CONFIG: ODR[6:2] | PWR_MODE[1:0]. */
    uint8_t conf = (uint8_t)((((uint8_t)odr  & 0x1Fu) << 2)
                           |  ((uint8_t)mode & 0x03u));

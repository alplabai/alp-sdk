...
static alp_status_t imx219_read_reg(imx219_t *dev, uint16_t reg, uint8_t *val)
{
    uint8_t addr_be[2] = {(uint8_t)(reg >> 8), (uint8_t)(reg & 0xFF)};
    return alp_i2c_write_read(dev->bus, dev->addr, addr_be, sizeof(addr_be), val, 1);
}

static alp_status_t imx219_write_reg(imx219_t *dev, uint16_t reg, uint8_t val)
{
...
alp_status_t imx219_init(imx219_t *dev, alp_i2c_t *bus, uint8_t i2c_addr)
{
    ...
    uint8_t hi = 0, lo = 0;
    alp_status_t s = imx219_read_reg(dev, IMX219_REG_MODEL_ID_HI, &hi);
    if (s != ALP_OK) return s;
    s = imx219_read_reg(dev, IMX219_REG_MODEL_ID_LO, &lo);
    if (s != ALP_OK) return s;
    if ((((uint16_t)hi << 8) | lo) != IMX219_CHIP_ID) return ALP_ERR_IO;
...
{
    if (dev == NULL || !dev->initialised) return ALP_ERR_NOT_READY;
    return imx219_write_reg(dev, 0x0103, 0x01);
}

 ...
 * carried from the AE822 DFP CMSIS sys_ctrl_cdc.h / sys_ctrl_dphy.h.  Upstream's
 * header names neither, so the ALIF_##reg##_REG paste in ALIF_CLK_CFG() needs
 * these local definitions.  Same silicon, same offsets in both trees. */
#define ALIF_CDC200_PIXCLK_CTRL_REG 0x04U /* CDC200 pixel-clock control */
#define ALIF_MIPI_CKEN_REG          0x40U /* MIPI (TX-)D-PHY config-clock enable */
 ...
 * sys_ctrl_dphy.h) into the upstream 8-arg ALIF_CLK_CFG() encoding.  All four
 * gates live in CLKCTL_PER_MST (module 0x1, base 0x4903F000).  Per
 * [[reference_alif_clock_encoding_fork_vs_upstream]] the fork's 7-arg packed
 * values MUST NOT be copied (wrong module index/register block under the
 * upstream decode driver); only the silicon offsets + enable/select bits are
 * carried over.  vendor-ext, BENCH-UNVERIFIED (no panel wired on this batch).
 */
/* DPI (CDC) gate: PERIPH_CLK_ENA(0x4903F00C) bit1.  DFP sys_ctrl_cdc.h:29. */
#define ALIF_DPI_CLK \
	ALIF_CLK_CFG(CLKCTL_PER_MST, PERIPH_CLK_ENA, 1U, 1U, 0U, 0U, 0U, ALIF_PARENT_CLK_SYST_ACLK)
/* DSI host gate: PERIPH_CLK_ENA(0x4903F00C) bit28.  DFP sys_ctrl_dsi.h:28. */
#define ALIF_DSI_CLK \
	ALIF_CLK_CFG(CLKCTL_PER_MST, PERIPH_CLK_ENA, 28U, 1U, 0U, 0U, 0U, ALIF_PARENT_CLK_SYST_ACLK)
...
#define ALIF_CDC200_PIX_SYST_ACLK \
	ALIF_CLK_CFG(CLKCTL_PER_MST, CDC200_PIXCLK_CTRL, 0U, 1U, 0U, 1U, 4U, ALIF_PARENT_CLK_SYST_ACLK)

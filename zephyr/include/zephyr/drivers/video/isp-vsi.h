 ...
 * Vendored from the Apache-2.0 zephyr_alif fork
 * (alifsemi/zephyr_alif @ v2.3.0, commit 3a2b84d9), path
 * include/zephyr/drivers/video/isp-vsi.h.  ADR 0017 Tier-2 (INTERIM).
 * This was the last isp_pico.c compile blocker (isp_pico.h:9 #includes it;
 * hal_alif's own isp_wrapper/src/isp_api_wrapper.c:21 #includes it too) --
 * v2.3.0's 3-arg isp_vsi_bottom_half(dev, init_cfg, mi_mis) below already
 * matches isp_pico.c's call site.  Paired with the also-vendored
 * isp_ctrl_params.h (struct isp_params + ISP_PARAM_MASK_* it needs).
 *
 * NOT fully verbatim: ONE line is localized (see the note above
 * isp_vsi_register_ae_status_callback() near the bottom of this file) --
 * the upstream `__syscall` marker + its generated
 * `#include <zephyr/syscalls/isp-vsi.h>` were dropped for a plain prototype,
 * because Zephyr's syscall generator does not scan this vendored header in
 * the alp-sdk module context and the generated include 404s.  Everything
 * else is verbatim.
 */
/*
 ...
 * __syscall (Zephyr's gen_syscalls.py then emits
 * <zephyr/syscalls/isp-vsi.h> + the *_mrsh.c thunk from that marker).  In the
 * alp-sdk module context (a vendored header under zephyr/include/, not part
 * of the module's registered syscall header list) the generator does not
 * scan this file, so the include below 404s and the TU fails to compile --
 * this was confirmed on the AEN build.  Localized to a plain prototype;
 * isp_pico.c supplies the implementation directly under the
 * isp_vsi_register_ae_status_callback name (no z_impl_ prefix -- there is no
 * syscall marshalling without the generator).  Everything else in this file

 ...
 * The Alif Ensemble ADC (12-bit x3 + 24-bit x1) is driven by a vendored copy of
 * the Apache-2.0 zephyr_alif fork driver (drivers/adc/adc_alif.c, compatible
 * "alif,adc").  hal_alif ships no ADC class driver -- only the analog_ctrl VREF
 ...
 * ==================================================================
 *
 * Vendored from the fork with ONE mechanical (non-functional) edit beyond this
 * provenance header:
 *   - struct adc_data -> struct adc_alif_data.  Upstream Zephyr v4.4 added a
 *     public `struct adc_data` to <zephyr/drivers/adc.h> (the ADC RTIO/decoder
 *     data type), which the older fork driver's private device-data struct
 *     collides with ("redefinition of struct adc_data").  Renaming the driver's
 *     private struct resolves the fork-vs-upstream drift without touching logic.
 * (The driver's own `struct adc_config` does NOT collide -- upstream has none.)
 *
 * The driver's VREF path calls the
 * hal_alif analog_ctrl helpers (enable_adc_ref_voltage / enable_analog_periph*)
 * from "analog_ctrl.h", reached via the hal_alif drivers/analog include dir that
 * CONFIG_USE_ALIF_HAL_ANALOG puts on the build (ADC_ALIF selects it).  The

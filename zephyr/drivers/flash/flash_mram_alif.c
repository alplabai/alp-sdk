 ...
 * The Alif Ensemble on-die MRAM (the SoC's non-volatile NVM, base 0x80000000)
 * is driven by a vendored copy of the Apache-2.0 zephyr_alif fork driver
 * (drivers/flash/flash_mram_alif.c, compatible "alif,mram-flash-controller").
 * Upstream Zephyr v4.4 + hal_alif ship NO flash-class driver for the MRAM --
 * the upstream E8 SoC dtsi exposes MRAM only as a "zephyr,memory-region" /
 * "soc-nv-flash" memory region (XIP-readable, no write/erase path), which is
 * not a flash controller and so does not set FLASH_HAS_DRIVER_ENABLED.  MCUboot
 * (and its flash_map) need a real flash driver on the MRAM to read/write the
 * slot0/slot1 partitions, so this genuine fork-driver copy is carried in-tree
 * (so it survives a `west update`) and binds the fork-native compatible
 * "alif,mram-flash-controller".  The driver is pure CMSIS register/memory
 * access (no hal_alif library), so -- unlike ADC_ALIF -- it pulls in no

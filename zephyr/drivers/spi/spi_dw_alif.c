 ...
 * zephyr_alif fork (drivers/spi/spi_dw.c) so it survives a `west update`, then
 * carried on upstream Zephyr v4.4 + hal_alif (no fork). Only mechanical edits
 * vs the fork source:
 *   - DT_DRV_COMPAT snps_designware_spi -> alif_dwc_ssi_spi  (distinct compatible
 *     so this does NOT collide with upstream's DW-apb-SSI spi_dw, which owns
 *     "snps,designware-spi" and would double-instantiate);
 *   - #include "spi_dw.h"/"spi_dw_regs.h" -> the renamed vendored headers;
 *   - LOG module spi_dw -> spi_dw_alif;
 *   - CONFIG_SPI_DW_USE_DMA -> CONFIG_SPI_DW_ALIF_USE_DMA.
 * spi_context.h is the UNMODIFIED upstream header, reached via the CMake
 * include-path (zephyr/drivers/spi); every spi_context_* call it uses is
 * signature-stable in v4.4. vendor-ext, BENCH-UNVERIFIED (compiles + links on
 * the E8 he target; runtime CS scheme / DMA / clock-rate are bench follow-ups).
 */

#define DT_DRV_COMPAT alif_dwc_ssi_spi

/* spi_dw_alif.c - Designware DWC_ssi SPI driver implementation */
		      ...
		      void *userdata)
{
	...
	{
		/* ALWAYS program the frame size, both directions.
		 *
		 * reg_data is a read-modify-write of the LIVE CTRLR0, and
		 * spi_dw_configure() returns early when the spi_config is unchanged --
		 * which it is for every transfer on a bridge that reuses one config.
		 * So a DFS written for a packed transfer PERSISTS: the next small
		 * transfer takes the 8-bit polled loop while the macrocell is still
		 * clocking 16-bit frames, and the peer sees garbage.
		 *
		 * That is not a subtle degradation -- it is why enabling PACK32 gave
		 * zero successful PINGs from boot (#1677).  Restoring dfs*8 on the
		 * not-packed path is what makes the option usable at all. */
		const uint8_t bits = (spi->pack != 0u) ? (uint8_t)(spi->pack * 8u)
		                                       : (uint8_t)(spi->dfs * 8u);
...
#ifdef CONFIG_SPI_DW_ALIF_PACK32
		if (spi->pack != 0u) {
			static bool pack_announced;

			if (!pack_announced) {
				pack_announced = true;
				printk("PACK ENGAGED w=%u rx_len=%u\n",
				       (unsigned)spi->pack, (unsigned)spi->ctx.rx_len);
			}
...
#define SPI_DW_INIT(inst)                                                                   \
	...
	static const struct spi_dw_config spi_dw_config_##inst = {                             \
		...
		IF_ENABLED(CONFIG_SPI_DW_ALIF_USE_DMA,					    \

...
{
    /* RZ/V2N port: clock_source is always SCI_B_SPI_SOURCE_CLOCK_PCLK, so the
     * SCISPICLK else-branch below is dead; init to 0 so it carries no
     * -Wmaybe-uninitialized (the rzv BSP omits BSP_FEATURE_*_HAS_SCISPI_CLOCK,
     * which would otherwise leave this branch empty).  See r_sci_b_spi_cfg.h. */
    uint32_t peripheral_clock = 0U;
...
static void r_sci_b_spi_hw_config (sci_b_spi_instance_ctrl_t * const p_ctrl)
{
    ...
    /* RZ/V2N FIX: SCKSEL must stay 0 -- the RZ SCI master generates SCK internally.
     * The upstream RA r_sci_b_spi sets CCR4.SCKSEL = BSP_FEATURE_SCI_SPI_SCKSEL_MASK,
     * but on RZ/V2N that mis-routes the master clock so no SCK reaches the pin
     * (confirmed on silicon: SCI7 CCR4=0x02000000 with zero SCK on the scope).  The
     * Renesas RZ FSP r_sci_b_spi (rz-fsp v4.1.0) sets ccr4 = 0 for exactly this. */
    uint32_t ccr4  = 0U;
    uint32_t cfclr = 0U;
    uint32_t ffclr = 0U;
    ...
    /* RZ/V2N FIX: keep CCR3.BPEN = 0 -- do NOT bypass the PCLK<->TCLK synchronizer.
     * The upstream FSP derives BPEN from clock_source, and this port selects
     * SCI_B_SPI_SOURCE_CLOCK_PCLK (=1) only so R_SCI_B_SPI_CalculateBitrate() takes the
     * R_FSP_SystemClockHzGet(BSP_FEATURE_SCI_CLOCK) branch -- but that choice also sets
     * BPEN=1 as a side effect.  On RZ/V2N the SCI bus clock (RSCI7_PCLK) and operation
     * clock (RSCI7_TCLK / P5CLK) are SEPARATE CPG nets (distinct CLK_ON gates), so per
     * the HW manual (7.3.15 / CCR3.BPEN: "bypass permitted only when the same clock is
     * input to bus clock and operation clock") the synchronizer MUST NOT be bypassed.
     * With BPEN=1 the SCK/TCLK output domain is left unsynchronised and NO clock ever
     * reaches the SCK7 pin (confirmed on silicon: FSP transfer "completes" yet the scope
     * shows zero SCK and the GD32 receives nothing).  Force BPEN=0; clock_source still
     * selects only the bit-rate clock.  Set BPEN=1 only if PCLK==TCLK is proven on silicon. */
    ccr3 |= (uint32_t) (0U << R_SCI_B0_CCR3_BPEN_Pos);

 ...
 * (drivers/ipm/ipm_arm_mhuv2.h, alifsemi/zephyr_alif main).  Pure MHUv2
 * register-block layout + bit #defines for the same SoC IP -- no upstream-API
 * dependency here, so it is copied unchanged.  See ipm_arm_mhuv2.c for the
 * alp-sdk port note: the driver KEEPS its .poll_out/.poll_in tail, which the

...
/*
 * ARM MHUv2 mailbox, exposed through Zephyr's IPM API (compatible
 * "arm,mhuv2").  Vendored from the Apache-2.0 zephyr_alif fork
 * (drivers/ipm/ipm_arm_mhuv2.c) -- same SoC IP, register access unchanged.
 *
 * Why alp-sdk carries it: the Secure-Enclave service client lives in the
 * Apache-2.0 hal_alif module (se_services/zephyr/src/se_service.c) and talks to
 * the SE over MHUv2 via the Zephyr IPM API.  Upstream Zephyr v4.4 ships no
 * MHUv2 IPM driver (only ipm_mhu.c for MHUv1) and the fork SoC Kconfig that
 * would select it (SOC_SUPPORT_ARM_MHUV2 -> select IPM + select ARM_MHUV2) is
 * not in upstream -- the classic over-vendor-SDK gap.  This in-tree copy +
 * Kconfig + DT nodes close it so se_service.c links and the SE LCS/device-data
 * queries work on the upstream-Zephyr + hal_alif stack.
 *
 * PORT NOTE -- the IPM poll API:
 *   The fork extends Zephyr's ipm_driver_api with .poll_out/.poll_in and adds
 *   the matching ipm_poll_out()/ipm_poll_in() syscalls (fork
 *   include/zephyr/drivers/ipm.h).  hal_alif se_service.c calls those on its
 *   pre-scheduler (polling) path.  Upstream Zephyr v4.4 lacks them, so the
 *   alp-sdk carries a small, upstreamable Zephyr patch that adds exactly those
 *   two struct fields + syscalls (zephyr/patches.yml ->
 *   0002-ipm-add-poll-out-poll-in.patch).  With that patch applied the .poll_*
 *   assignments below compile; the interrupt-mode path (.send + callbacks),
		      ...
		      const void *pdata, int size)
{
	...
	/* CH_SET is a single 32-bit register: every transfer is exactly
	 * sizeof(uint32_t) bytes.  Reject anything else instead of always
	 * reading/writing 4 bytes regardless of what the caller declared --
	 * a short buffer read past its end is out-of-bounds. */
 ...
 * @brief   Report the max payload size of one MHUV2 transfer
 * @param[in] dev : pointer to Runtime device structure
			...
			const void *pdata, int size, k_timeout_t timeout)
{
	...
	/* CH_SET is a single 32-bit register -- see mhuv2_send(). */
	if (size != (int)sizeof(uint32_t)) {
		return -EINVAL;
	}
			...
			const void *pdata, int size, k_timeout_t timeout)
{
	...
	/* CH_ST is a single 32-bit register -- see mhuv2_send(). */
	if (size != (int)sizeof(uint32_t)) {
		return -EINVAL;
	}
...
static struct ipm_driver_api mhuv2_driver_api = { .send              = mhuv2_send,

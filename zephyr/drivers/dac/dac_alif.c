 ...
 * The Alif Ensemble DAC12 (2x 12-bit current-output DACs) is driven by a
 * vendored copy of the Apache-2.0 zephyr_alif fork driver
 * (drivers/dac/dac_alif.c, compatible "alif,dac").  hal_alif ships no DAC class
 ...
 * in hand it is structured exactly like adc_alif.c: a vendored copy of a fork
 * class driver that hal_alif does not provide, sitting over the hal_alif
 * analog_ctrl helper.  That is ADR 0017 Tier-2 (vendored fork copy), NOT
 * Tier-1.5 (in-tree-thin over an Apache hal/upstream library).  Declared Tier-2
 * here to match how it is actually carried, mirroring adc_alif.c.
 * ==================================================================
 *
 * Vendored VERBATIM from the fork: NO mechanical fork-vs-upstream edits beyond
 * this provenance header.  Unlike adc_alif.c (whose private `struct adc_data`
 * collided with upstream v4.4's new public `struct adc_data` in
 * <zephyr/drivers/adc.h>), upstream Zephyr v4.4 <zephyr/drivers/dac.h> declares
 * no `struct dac_data` / `struct dac_config`, only `struct dac_channel_cfg`, so
 * this driver's private `struct dac_data` / `struct dac_config` do NOT collide
 * and need no rename.
 *
 * The driver's VREF path calls the hal_alif analog_ctrl helpers
 * (enable_dac12_ref_voltage / enable_analog_peripherals / enable_analog_periph_clk)
 * from "analog_ctrl.h", reached via the hal_alif drivers/analog include dir that
 * CONFIG_USE_ALIF_HAL_ANALOG puts on the build (DAC_ALIF selects it).  The
 * register block layout and clock/VREF sequencing are the fork's.  The Alif DAC
 * has no IRQ (write-only IP), so -- unlike the ADC -- the nodes carry no
 * `interrupts`.
 * vendor-ext, BENCH-UNVERIFIED (compiles + links on the E8 he target; the
 * setpoint write / VREF / clock programming are bench follow-ups).
 */

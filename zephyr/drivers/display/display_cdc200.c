 ...
 * The TES CDC-2.1 (CDC200) DPI/RGB display controller -- the pixel-pump that
 * feeds the MIPI-DSI bridge on the Alif Ensemble E8 -- is driven by a vendored
 * copy of the Apache-2.0 zephyr_alif fork driver (drivers/display/
 * display_cdc200.c, compatible "tes,cdc-2.1").  Upstream Zephyr v4.4 + hal_alif
 * ship NO tes,cdc-2.1 (or any DPI display-controller) driver, so this is a
 * genuine fork-driver copy carried in-tree so it survives a `west update` (the
 * VIDEO_ALIF_CAM / ADC_ALIF / MHUv2 precedents).  Retire onto the opt-in
 * sdk-alif fork compatible once the cdc node is repointed AND bench-verified
 * (task #21).  See docs/adr/0017-alp-sdk-over-the-vendor-sdk.md.
 * ==================================================================
 *
 * Vendored from the fork, then PORTED to the upstream Zephyr v4.4 display API by
 * Alp Lab AB.  Unlike the fork video drivers, the fork's use of
 * `struct display_driver_api`, `enum display_pixel_format`,
 * `enum display_orientation`, `struct display_capabilities` and
 * `struct display_buffer_descriptor` is UNCHANGED in v4.4 -- the only port the
 * v4.4 surface forced is the missing public extended-API header.  The v4.4
 * deltas applied here (each marked "v4.4 display port (Alp Lab AB)" at its
 * site):
 *   - the fork driver #include'd <zephyr/drivers/display/cdc200.h>, a public
 *     device-specific extended-API header (struct cdc200_display_caps /
 *     cdc200_fb_desc + the cdc200_get_capabilities/_display_write/_display_read/
 *     _swap_fb/_set_enable/_get_framebuffer/_restore_fb prototypes) that the
 *     fork never published in its include/ tree.  Those declarations are folded
...
static int cdc200_setup_pixel_clock(const struct device *dev)
{
	...
	/*
	 * alp-sdk's upstream clock_control_alif provides NO .configure op
	 * (clock_control_configure() -> -ENOSYS) and its .set_rate returns
	 * -ENOTSUP for any non-I2S clock.  Both are non-fatal here: the pixel
	 * clock's source-select and divider are baked into the ALIF_CLK_CFG()
	 * encoding of pix_cid, so clock_control_on() below already brings the
	 * gate + source up at the right rate.  Tolerate ENOSYS/ENOTSUP (warn
	 * and continue); keep every other error fatal.
	 */
	ret = clock_control_configure(config->clk_dev, config->pix_cid, NULL);
	if (ret == -ENOSYS || ret == -ENOTSUP) {
		LOG_WRN("Pixel-CLK clock_control_configure() unsupported (ret %d); "
			"relying on the encoded source-select via clock_control_on()", ret);
	} else if (ret) {
	...
	if (ret == -ENOSYS || ret == -ENOTSUP) {
		LOG_WRN("Pixel-CLK clock_control_set_rate() unsupported (ret %d); "
			"relying on the encoded divider via clock_control_on()", ret);
	} else if (ret) {

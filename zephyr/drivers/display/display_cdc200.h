 ...
 *
 * Private register map + types for the vendored TES CDC-2.1 (CDC200) display
 * controller (ADR 0017 Tier-2; see display_cdc200.c for the full provenance
...
/*
 * ============================================================================
 * Device-specific extended display API.
 *
 * v4.4 display port (Alp Lab AB): folded in from the fork's missing public
 * <zephyr/drivers/display/cdc200.h> header.  This is the layer-aware extension
 * over the generic Zephyr display API -- the generic .write/.read/.get_*
 * callbacks act on layer 0 only; these device-specific entry points take an
 * explicit layer index and expose framebuffer swap / panel enable.
 * ============================================================================
 */

/** Per-layer capability descriptor. */
...
/** CDC200 display capabilities (panel-wide + per-layer). */
struct cdc200_display_caps {
	uint16_t x_panel_resolution;
	uint16_t y_panel_resolution;
	uint32_t supported_pixel_formats;
	enum display_orientation current_orientation;
	struct cdc200_layer_caps layer[CDC_LAYER_MAX];
};

/** Framebuffer descriptor used by the swap / get-framebuffer entry points. */
...
/**
 * @brief Get the current framebuffer of a given layer.
 *
 * @param dev CDC200 device.
 * @param idx Layer index (CDC_LAYER_1 or CDC_LAYER_2).
 * @param fb  Output framebuffer descriptor.
 */
void cdc200_get_framebuffer(const struct device *dev, uint8_t idx, struct cdc200_fb_desc *fb);

/**
 * @brief Write a buffer into the framebuffer of a given layer.
 *
 * @param dev  CDC200 device.
 * @param idx  Layer index (CDC_LAYER_1 or CDC_LAYER_2).
...
/**
 * @brief Read a buffer from the framebuffer of a given layer.
 *
 * @param dev  CDC200 device.
 * @param idx  Layer index (CDC_LAYER_1 or CDC_LAYER_2).
 * @param x    X coordinate of the upper-left corner.
...
/**
 * @brief Get the device-specific (layer-aware) capabilities.
 *
 * @param dev          CDC200 device.
 ...
 * @param dev CDC200 device.
 * @param idx Layer index (CDC_LAYER_1 or CDC_LAYER_2).
 * @param fb  New framebuffer descriptor (must match the layer's fb_size).
 */
void cdc200_swap_fb(const struct device *dev, uint8_t idx, struct cdc200_fb_desc *fb);

/**
 * @brief Restore both layers' framebuffers to their DT-configured defaults.
 *
 * @param dev CDC200 device.
 */

...
static int sdhc_dwc_get_host_props(const struct device *dev, struct sdhc_host_props *props)
{
	...
	/* alp-sdk: upstream Zephyr v4.4 carries bus_4_bit_support on struct
	 * sdhc_host_props (not on .host_caps as the fork's sdhc.h did). */
	props->bus_4_bit_support = (config->bus_width >= 4);

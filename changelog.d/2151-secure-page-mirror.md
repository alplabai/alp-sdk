...
- The mirror about to be sealed was never checked against the module's own
  128-byte array manifest at `0x50` — the two are generated by different
  scripts (`scripts/program_eeprom.py` takes the SKU as an explicit
  argument; `scripts/program_eeprom_secure_page.py` infers it from
  `board.yaml`) from independent inputs, so a self-consistent, CRC-valid
  mirror generated against the wrong `board.yaml` would lock forever
  undetected. Mode 3 now reads the array manifest fresh, integrity-checks

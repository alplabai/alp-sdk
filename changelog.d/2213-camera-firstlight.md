...
stacked sensor happens to use.

New example `examples/aen/aen-camera-firstlight` opens each of the three
RPi-style CSI-2 camera shields the E1M-EVK's J5 connector supports (IMX219,
OV5647, OV9281) through `<alp/camera.h>` only, starts the stream, and waits
for one frame with a 2 s timeout — printing CRC32 + a top-bits histogram +
...
whole CSI-2 -> CPI pipe has never run on real silicon; all three shield

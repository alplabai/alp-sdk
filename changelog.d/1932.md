...
(#1700)`) that landed on `dev` after this branch forked. At the time, the
gate (`scripts/check_changelog_fragments.py`) required exactly `<digits>.md`,
so no `1909-<slug>.md` alternative existed yet to fall back on; that gap was
closed by alp-sdk#1941, and enforcing that a fragment's leading digits match
the issue its own heading cites is now tracked separately as alp-sdk#1957.

...
whole chain by adding a shield and setting `CONFIG_ALP_SDK=y`:
...
- **D-PHY clocks.** The shared `&dphy` node's `pllref-clk` / `pllbypass-clk` /
  ...
  placeholder. This also changes what the CSI camera path's
  `dphy_dw_slave_setup()` turns on (`MIPI_CKEN` bit 12 is now real).
- **SoC glue.** `zephyr/soc-bridge/alif/mipi_display_e8.c` (ADR 0017
  Tier-1.5), built only on E8 with `CONFIG_MIPI_DSI_DW` or
  `CONFIG_DISPLAY_CDC200`, does what no driver does, each hook gated on its DT
  node: at `PRE_KERNEL_1`, when `dphy` is okay, it enables the D-PHY clock
  sources (CGU `CLK_ENA` +0x14 bits 21 and 23: HFOSC 38.4 MHz and the 100 MHz
  CFG clock) and D-PHY analog power (VBAT `PWR_CTRL` +0x08: clears bits
  0/1/4/5/8/9/12); at `POST_KERNEL` `CONFIG_KERNEL_INIT_PRIORITY_DEFAULT`, below
  `CONFIG_DISPLAY_INIT_PRIORITY`, it sets `CDC200_PIXCLK_CTRL` (`CLKCTL_PER_MST`
  +0x04) bits [24:16] to `SYST_ACLK / cdc200 clock-frequency`, rounded and
  clamped to 2..511, keeping the other bits; and at `PRE_KERNEL_1` it muxes the
  pad of each DSI panel's `bl-gpios` to GPIO with the input receiver on. The CGU,
  VBAT and `CLKCTL_PER_MST` bases come from the clock controller's `reg-names`,
  the 400 MHz from the `syst_aclk` fixed clock. The pad mux lives here because
  `display_cdc200.c` does not apply its `pinctrl-0` in a MIPI-DSI build.
- **Shield.** `e1m_evk_rk055hdmipi4ma0` (`zephyr/boards/shields/`, found through
  ...
  Its `Kconfig.defconfig` defaults on `DISPLAY`, `MIPI_DSI`, `HX8394`, `I2C`,
  `GPIO`, `GPIO_PCA_SERIES`, `REGULATOR`, `REGULATOR_FIXED`, and under
  `ALP_SDK` `MIPI_DPHY`, `MIPI_DPHY_DW`, `MIPI_DSI_DW` and `DISPLAY_CDC200`, and
  adds `CONFIG_HEAP_MEM_POOL_ADD_SIZE_E1M_EVK_RK055HDMIPI4MA0=4096` so the system
  heap is at least 4 KiB. The I2C2 pins, clock and `alp-i2c0` alias stay with
  the board devicetree.
...
`CONFIG_GPIO_PCA_SERIES_INIT_PRIORITY=60`: the defaults already order I2C (50),
the expander (50, after its bus by devicetree dependency), the regulator (75),
the CDC and DSI host (85) and the panel (90). The RAM-run `zephyr.bin` stays
106,448 B. `aen-dsi-regcheck` now enables the SoC nodes instead of declaring its
own copies. `aen-evk-demo` does not add the shield: with it, the TCAL9538 at
`0x73` becomes a GPIO-driver-owned device, which conflicts with its phase 4 raw
register writes.

Build-verified on E1M-AEN801 and E1M-AEN803; pixels on glass are not yet

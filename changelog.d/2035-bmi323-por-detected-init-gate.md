...
`ALP_ERR_IO` instead of proceeding to a `CHIP_ID` check that would have
falsely passed.
...
`ALP_ERR_IO`), but it is a behaviour change any caller relying on the old,
looser pass criterion needs to know about.

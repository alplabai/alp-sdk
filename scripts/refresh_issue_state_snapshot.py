...
Run on a schedule by `.github/workflows/refresh-issue-state-snapshot.yml`,
which opens a PR when the snapshot content actually changes. Not a
`scripts/check_*.py` gate -- it writes, it does not check -- so it is
exempt from `check_quality_registry.py`'s four-site rule.

...
#
# OS-topology + per-core OS-inference helpers for the board.yaml
# orchestrator.  Extracted from alp_orchestrate.py as a #285 modularization
# seam (one seam, no behaviour change): core-class -> runtime rules plus the
# os-topology dict/JSON an IDE/tool consumes -- a near-pure leaf whose only
# package dependency is the board-schema path (read at call time, kept
# single-source in the package __init__).  Public names are re-exported from
# alp_orchestrate, so `from alp_orchestrate import emit_os_topology` (and the
# internal callers + tests) keep resolving unchanged.
"""OS-topology + per-core OS-inference helpers for the orchestrator."""
...
def _core_os_choices() -> tuple[str, ...]:
    ...
    (no slice is built).
    """
    # BOARD_SCHEMA stays single-source in the package __init__; the local
    # import avoids a topology<->__init__ cycle (same pattern as models.py).
    from alp_orchestrate import BOARD_SCHEMA

...
The TSVs are the single source for each pad's e1m_pad/e1m_function/owner/
silicon fields (the `core` field is a separate, small policy source --
see core-ownership below).  Tools (the `alp` CLI / the IDE) want one
...
def _render(family: str, spec: dict, pads: list[dict[str, str]]) -> str:
    ...
    core_note = (
        [f"# `core` is sourced separately from {spec['core_ownership']!r}."]
        if "core_ownership" in spec else []
    )
    lines = [
        ...
        "# silicon owns it.  The TSVs are the source of those fields; this",
        "# file is gated byte-in-sync.  Pad exclusivity is gated by",
        "# scripts/check_pin_conflicts.py.",
        *core_note,

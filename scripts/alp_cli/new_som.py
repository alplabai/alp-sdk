...
def _render_preset(
    ...
    a("# placeholder backend; replace it with the real canonical key")
    a("# (ethos_u, drpai, deepx_dxm1, ...) before merging --")
    a("# scripts/check_inference_backend_parity.py cross-checks the value")
    a("# against the device dispatcher.")
    a("inference:")
...
@click.option("--vendor", default=None,
              help="Vendor display name for the SoC JSON "
                   "(default: the soc-ref vendor segment).")
@click.option("--display-name", default=None,
              help="Preset display_name (default derived from the SKU).")
@click.option("--inference-backend", default=None,
              type=click.Choice(INFERENCE_BACKENDS),
              help="Silicon-determined inference backend "
                   "(`tbd` placeholder when unknown; the default).")
...
@click.option("--cores", default=None, callback=_split_csv,
              help="Comma-separated canonical core ids "
                   "(default: the tbd_core0 placeholder).")
@click.option("--default-board", default=None,
              help=f"Stock carrier board (default {DEFAULT_BOARD}).")
@click.option("--default-hw-rev", default=None,
              help=f"Default hardware revision (default {DEFAULT_HW_REV}).")
@click.option("--output-root", default=None,
              type=click.Path(file_okay=False, path_type=Path),
              help="Root to generate metadata/ under "
                   "(default: the SDK checkout).")
...
def new_som_cmd(
    ...
    if not _SKU_RE.match(sku):
        click.echo(f"alp new-som: SKU '{sku}' must look like E1M-<UPPERCASE>")
        raise SystemExit(1)
    if not _SOC_REF_RE.match(soc_ref):
        click.echo(f"alp new-som: soc-ref '{soc_ref}' must be "
                   f"<vendor>:<family>:<part> (lowercase slugs)")
        raise SystemExit(1)
    if not _FAMILY_RE.match(family):
        click.echo(f"alp new-som: family '{family}' must be a lowercase slug")
        raise SystemExit(1)
    bad_cores = [c for c in cores if not _CORE_ID_RE.match(c)]
    if bad_cores:
        click.echo(f"alp new-som: core id(s) {bad_cores} must match "
                   f"^[a-z][a-z0-9_]+$")
        raise SystemExit(1)
    if inference_backend == "ethos_u" and ethos_u_variant is None:
        click.echo("alp new-som: --inference-backend ethos_u requires "
    ...
    if preset_path.exists() and not force:
        click.echo(f"alp new-som: {preset_path} already exists "
                   f"(pass --force to overwrite)")
        raise SystemExit(1)

    # a. SoM preset skeleton.
    ...
    # b. SoC spec skeleton -- only when the target SoC has no spec yet.
    soc_created = False
    if soc_path.exists():
        click.echo(f"SoC spec already present: {soc_path} (not touched)")
    else:
        soc_doc = _soc_skeleton(sku, soc_ref, vendor, cores)
    ...
    # Self-check: the skeletons must be schema-valid on arrival.  The
    # one sanctioned exception is a SKU outside the schema's current
    ...
    if preset_doc is not None:
        ...
        if errors:
            click.echo("alp new-som: INTERNAL ERROR -- generated preset "
                       "does not validate against som-preset-v1:")
            for e in errors:
                click.echo(f"  - {e}")
            raise SystemExit(1)
    ...
    if soc_created:
        errors = _schema_errors(
            json.loads(soc_path.read_text(encoding="utf-8")), SOC_SCHEMA_PATH)
        if errors:
            click.echo("alp new-som: INTERNAL ERROR -- generated SoC spec "
                       "does not validate against soc-spec-v1:")
            for e in errors:
                click.echo(f"  - {e}")
            raise SystemExit(1)
        click.echo("SoC spec skeleton validates against soc-spec-v1")

    # c. Numbered next-steps checklist.
    steps = [

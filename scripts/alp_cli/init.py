...
def _scaffold(dest: Path, som: str, preset: str, peripherals: list[str]) -> None:
    ...
    # newline="": `west alp-migrate --apply` later rewrites this same
    # board.yaml with newline="" (scripts/west_commands/alp_migrate.py); a
    # CRLF seed here means the first migrate flips the whole file to LF in
    # the customer's own git, for no reason.
    board_yaml.write_text(text, encoding="utf-8", newline="")

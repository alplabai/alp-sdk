...
def load_board_yaml(path: Path, *,
    ...
    # 9. `security.psa:` cross-field validation.  The schema is
    ...
    security = dict(project.get("security") or {})
    psa = dict(security.get("psa") or {})
    if psa:
        storage_entries = list(project.get("storage") or [])
        storage_names = {
            str(s.get("name")) for s in storage_entries
        ...
        valid_refs = storage_names | region_names

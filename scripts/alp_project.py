...
# downstream scaffolder) means the E1M pad ABI lives in exactly one place:
# _e1m_gpio_canonical().
...
def _emit_native_sim_overlay(project: dict[str, Any]) -> str:
    ...
    The pin-array is the full 52-entry positional map in e1m_pinout.h
    ...
    length).  gpio-emul caps at 32 pins, so the 52 pads span two
    controllers: gpio_emul0 backs canonical indices 0..31, gpio_emul1 backs
    32..51.  Every pad defaults to GPIO_ACTIVE_HIGH; an app that needs a
    ...
    do.  Like _emit_dts_overlay this is E1M-only (keyed on
    _e1m_gpio_canonical); an E1M-X variant would extend both together.
    """
    sku = project["som"]["sku"]
    canonical = _e1m_gpio_canonical()
    n = len(canonical)
    ...
    lines.append(" * Regenerate after changes to the e1m_pinout.h GPIO order.")
    lines.append(" *")
    ...
    lines.append(f" * resolves under `west build -b native_sim/native/64`.  All {n} E1M pads")
    ...
    lines.append("    alp_pins: alp-pins {")
    lines.append('        compatible = "alp,pin-array";')
    lines.append(f"        /* {n} entries in E1M canonical order (e1m_pinout.h).  Indices:")
    lines.append("         *   0..25  IO0..IO25       26..33 PWM0..PWM7")
    lines.append("         *   34..41 ENC0_X..ENC3_Y  42..49 ADC0..ADC7   50..51 DAC0..DAC1 */")
    lines.append("        gpios =")
    for i, suffix in enumerate(canonical):

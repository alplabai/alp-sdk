...
| `AEN_BOARD` | `alp_e1m_aen803_m55_he/ae822fa0e5597ls0/rtss_he` (alp-sdk#2094) | Qualified board target. `build.sh` REFUSES rather than silently dropping an overlay: if the app's `boards/` ships >=1 `alp_e1m_*`-qualified file but none matches this target's stem, it aborts (exit 3) — override `AEN_BOARD` (e.g. `alp_e1m_aen801_m55_he/ae822fa0e5597ls0/rtss_he`) for an app that has not yet grown an AEN803 overlay (alp-sdk#2101). |

...
| `aen-flowc-itcm.conf` | **C** | Flow-C-ONLY Kconfig fragment: `CONFIG_USE_DT_CODE_PARTITION=n` + `CONFIG_FLASH_LOAD_OFFSET=0x0`, forcing the link base back to ITCM `0x0` for a RAM-run. Layer on top of `aen-bench-shared.conf`; never use on a Flow A/D (production MRAM) build — it silently relocates a slot0-by-design app to `0x0`. |

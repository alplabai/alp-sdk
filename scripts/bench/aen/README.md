...
| `flash-jlink.sh <build-dir> [read-bytes]` | **D** | J-Link **direct MRAM flash** (no SE-UART). `app-gen-toc` builds the signed ATOC, the part-number device profile unlocks the built-in Alif MRAM loader, `loadbin`/`verifybin` write the package at its per-build start address (parsed from `app-package-map.txt`), `RSetType 2`/`r`/`g` pin-resets so the SE reloads it, then a generic-device RAM-console read-back. **Gates on the `verifybin` outcome (#1488): exit 3** when the transcript reports `verify failed`/`verification failed`/`mismatch`, and **exit 3** when it carries no `verify successful` line at all (the verify never ran) — either way the read-back is skipped and the board must not be treated as flashed. Also **exit 4** (DPIDR preflight says this is not the AEN E8), **exit 2** (the part-number device profile could not connect), **exit 7** (flash verified, but the post-boot console read never opened the probe). **No ATOC-replace guard (#2025):** `loadbin` here burns the same replacing (not merging) signed ATOC `app-write-mram -p` does, but this flow has no SE-UART dependency by design, so `bench_atoc_replace_guard()`'s `maintenance -opt gettoc` pre-write check does not run — a resident foreign entry (another core's app, an A32 Linux boot chain) is silently delisted with no warning. Tracked as a follow-up (#2027). |
| `flash-jlink-hp.sh <build-dir> [sram0_beacon_addr_hex]` | **D** | J-Link **direct MRAM flash for the M55-HP core** — the SES's second M55, held in reset at power-on. Authors an `M55_HP`/`loadAddress 0x50000000` signed ATOC, `loadbin`/`verifybin` write + verify it, `RSetType 2`/`r`/`g` pin-resets so the SES boots HP, then reads the `examples/aen/aen-hp-core-smoke` SRAM0 liveness beacon (magic/CPUID/VTOR/heartbeat) over the HE/system AP and re-reads the heartbeat word to prove it is advancing. Becomes the board's active boot image — reflash the canonical `person_detect` slot0 with `flash-jlink-mramxip.sh` afterwards. **No ATOC-replace guard (#2025):** same replacing (not merging) ATOC write as every other Flow D helper, same no-SE-UART-by-design gap — tracked as a follow-up (#2027). |
| `flash-jlink-mramxip.sh <build-dir> [read-bytes]` | **D** | J-Link **MRAM-XIP / slot0 two-blob** flash for an app linked into MRAM slot0 (a real NPU model that overflows ITCM). Writes the app → `0x80010000` + the signed ATOC → its parsed address; the slot0 link comes from the board `_defconfig` (`CONFIG_USE_DT_CODE_PARTITION=y`), so a plain build already qualifies — a `0x8000xxxx` vector means a Flow C fragment/overlay is still layered. See the script header for the gotcha on returning to ITCM apps afterwards. **No ATOC-replace guard (#2025):** same replacing (not merging) ATOC write as every other Flow D helper, same no-SE-UART-by-design gap — tracked as a follow-up (#2027). |

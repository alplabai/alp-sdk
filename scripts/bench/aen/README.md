...
| `flash-update-log-firewall-probe.sh [--package-only] <he-build-dir>` | **D** | Firmware-update-log HE direct-write MRAM firewall probe. Builds an app-only ATOC by default so any already-provisioned DEVICE/firewall policy is preserved; set `ALP_AEN_INCLUDE_DEVICE_CONFIG=yes` only for deliberate DEVICE replacement. |
...
| `read-update-log-proof.sh [--expect-hw\|--expect-firewall-probe]` | (B) | Re-read the firmware-update-log SRAM0 proof beacons without reflashing. Use this after the probe or dual-M55 run to prove what the silicon actually did. |

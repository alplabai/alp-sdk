...
| `AEN_OPENOCD_USB_LOCATION` | *(none, error-if-unset)* | Labgrid-pinned USB path for the AEN E8's J-Link (e.g. `3-4.4.3` on alplab-gw — resolve from `labgrid-client -p e1m-aen-evk-01 show`'s swd resource, never hardcode). Passed as `adapter usb location <value>`, prepended on the OpenOCD command line. `openocd-ram-run.sh` only. |

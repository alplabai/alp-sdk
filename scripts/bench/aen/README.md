...
| `FLOWD_DRY_RUN` | *(none)* | **Set to exercise the whole Flow D sector-pad pipeline (#2233) with NO probe ever opened** — `bench_flowd_read_sectors()`/`bench_flowd_proof()` print the CommandFile they would run and (for the read) synthesize an all-`0x00` sector image instead of a real `savebin`, so `bench_flowd_build()`/`bench_flowd_loadbin_lines()` downstream still run their REAL code path against real, inspectable files. Never set on a real bench host — every writer's OTHER JLinkExe touches (the DPIDR preflight, ATOC-guard queries) are unaffected by this variable and still need a real probe/`LG_PLACE`. |

...
| `BENCH_PLACE` | *(none, error-if-unset)* | `ram-run.sh` only (alp-sdk#2076). The labgrid-client place name whose probe to use (e.g. `e1m-aen-evk-02`) — you must already hold that place's reservation. Routed through `jlink-run.sh` (below) instead of `-SelectEmuBySN`: five probes on this bench share OEM serial `000603000869`, so a bare serial selector cannot tell them apart, and with two JLinkExe sessions now in one run an unselected probe could load one board and read a DIFFERENT one. |
| `AEN_JLINK_RUN` | `$HOME/board-farm/bin/jlink-run.sh` | `ram-run.sh` only. The board-farm's per-probe isolation wrapper: checks the `BENCH_PLACE` labgrid reservation, USB-namespace-masks every other probe, and picks a JLinkExe build whose bundled firmware matches the probe's running firmware. Takes `-CommandFile <path>`, not `-CommanderScript`. |

...
| `flash-jlink-hp.sh [--atoc-unqueryable] <build-dir> [sram0_beacon_addr_hex]` | **D** | J-Link **direct MRAM flash for the M55-HP core** (no SE-UART) — the SES's second M55, held in reset at power-on. Authors an `M55_HP`/`loadAddress 0x50000000` signed ATOC, `loadbin`/`verifybin` write + verify it, `RSetType 2`/`r`/`g` pin-resets so the SES boots HP, then reads the `examples/aen/aen-hp-core-smoke` SRAM0 liveness beacon (magic/CPUID/VTOR/heartbeat) over the HE/system AP and re-reads the heartbeat word to prove it is advancing. Becomes the board's active boot image — reflash the canonical `person_detect` slot0 with `flash-jlink-mramxip.sh` afterwards. **Same `--atoc-unqueryable` requirement, same exit 8, same distinction from `--replace-atoc` as `flash-jlink.sh` above (#2025)** — an HP ATOC replaces whatever was resident, including a slot0 image. |
| `flash-jlink-mramxip.sh [--atoc-unqueryable] <build-dir> [read-bytes]` | **D** | J-Link **MRAM-XIP / slot0 two-blob** flash for an app linked into MRAM slot0 (a real NPU model that overflows ITCM). Writes the app → `0x80010000` + the signed ATOC → its parsed address; the slot0 link comes from the board `_defconfig` (`CONFIG_USE_DT_CODE_PARTITION=y`), so a plain build already qualifies — a `0x8000xxxx` vector means a Flow C fragment/overlay is still layered. See the script header for the gotcha on returning to ITCM apps afterwards. **Same `--atoc-unqueryable` requirement, same exit 8, same distinction from `--replace-atoc` as `flash-jlink.sh` above (#2025).** Open question recorded in `changelog.d/`: whether `maintenance -c $SE_UART -opt gettoc` itself resets the target — if it does, this script's own pre-programming halt gate (exit 5, AP[3] absence after any reset) would fail regardless of SE-UART availability, so the query-based guard cannot front this script until that's verified on silicon. |

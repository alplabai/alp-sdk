...
| `flash-jlink.sh [--replace-atoc] [--atoc-unqueryable] <build-dir> [read-bytes]` | **D** | J-Link **direct MRAM flash** (no SE-UART *required*). `app-gen-toc` builds the signed ATOC, the part-number device profile unlocks the built-in Alif MRAM loader, a sector-padded `loadbin` (see #2233 below) writes the package at its per-build start address (parsed from `app-package-map.txt`), `RSetType 2`/`r`/`g` pin-resets so the SE reloads it, then a generic-device RAM-console read-back. **`app-write-mram -p`/`loadbin` REPLACE the whole ATOC (#2025).** As of **#2027**, `bench_flowd_atoc_guard()` decides how that gets checked: if `$SE_UART` is **exported**, it calls the SAME `bench_atoc_replace_guard()` Flow A uses — queries the resident ATOC first and refuses (exit 5) on a foreign entry unless `--replace-atoc` is also passed, exactly like Flow A. If `$SE_UART` is **unset** (Flow D's normal, no-serial-required case), the #2029 interim behaviour is unchanged: refuses (exit 8) unless `--atoc-unqueryable` is passed to acknowledge that any other resident boot entry (an A32 chain, an HP app) will be silently delisted — the abort message names both ways forward. `--atoc-unqueryable` and `--replace-atoc` are never merged or aliased: `--atoc-unqueryable` says "I cannot check (no SE-UART here)", `--replace-atoc` says "I checked and I still want to replace it" — a habitual `--atoc-unqueryable` on a no-SE-UART slot must never also silence the SE_UART-present guard on a board where the resident TOC genuinely can be read (and is in fact ignored there, since a real check ran). **If `$SE_UART` is exported but the query cannot complete** (a stale or wrong device path, or no SETOOLS `maintenance` binary), the refusal is unchanged — exit 5, nothing written — and since #2187 the remedy it prints is flow-aware: on Flow D it tells you to point `SE_UART` at this slot's real SE-UART (prefer `LG_PLACE`) or to `unset SE_UART` and pass `--atoc-unqueryable`, and says explicitly that `--replace-atoc` is NOT the way out of a check that never ran. Flow A's wording is unchanged there, because on Flow A the SE-UART *is* the write transport. **Sector-pads and proves the write (#2233):** before writing, reads the MRAM sector(s) the package touches and overlays the package on them (`flowd_sector_pad.py` via `bench_flowd_prepare_write()`), then `loadbin`s that PADDED image instead of the raw package — the built-in loader rewrites the WHOLE 16 KiB sector and never reads its prior contents, so an unpadded write would turn every neighbouring byte to `0xFF`. **Gates on a fresh-session read-back (`bench_flowd_proof()`), not `verifybin`** (which only ever compared against J-Link's own flash cache, never a real chip read): a NEW J-Link session `savebin`s the written range back and `cmp`s it against the padded image; **exit 3** on any mismatch. Also **exit 9** (the sector-pad plan/read/build step itself refused or failed), **exit 4** (DPIDR preflight says this is not the AEN E8), **exit 2** (the part-number device profile could not connect), **exit 7** (flash verified, but the post-boot console read never opened the probe). **exit 10** (`FLOWD_DRY_RUN` set — nothing written), **exit 11** (RACE DETECTED — the board HAS already been written and booted, restore from the printed paths). Not a persistence proof across a power cycle. |
| `flash-jlink-hp.sh [--replace-atoc] [--atoc-unqueryable] <build-dir> [sram0_beacon_addr]` | **D** | J-Link **direct MRAM flash for the M55-HP core** (no SE-UART *required*) — same shape as `flash-jlink.sh` but authors an `M55_HP` ATOC (`loadAddress 0x50000000`, entry name `HP-APP`) and reads back a liveness beacon from SRAM0 instead of the RAM console. **Same `bench_flowd_atoc_guard()` SE_UART-gated behaviour, same exit 5/8, same `--replace-atoc`/`--atoc-unqueryable` distinction as `flash-jlink.sh` above (#2025/#2027)** — an HP ATOC replaces whatever was resident, including a slot0 image. **Sector-pads and proves the write (#2233)** the same way as `flash-jlink.sh` above — padded `loadbin`, then a fresh-session `bench_flowd_proof()` read-back replaces the old `verifybin` gate. **exit 3** on any read-back mismatch, **exit 9** (sector-pad prepare step failed). Also **exit 4** (DPIDR gate), **exit 2** (part-number profile could not connect), **exit 7** (beacon read-back never opened the probe). **exit 10** (`FLOWD_DRY_RUN` set — nothing written), **exit 11** (RACE DETECTED, board already written/booted — restore from the printed paths). Not a persistence proof across a power cycle. |
| `flash-jlink-mramxip.sh [--replace-atoc] [--atoc-unqueryable] <build-dir> [read-bytes]` | **D** | J-Link **MRAM-XIP / slot0 two-blob** flash for an app linked into MRAM slot0 (a real NPU model that overflows ITCM). Writes the app → `0x80010000` + the signed ATOC → its parsed address; the slot0 link comes from the board `_defconfig` (`CONFIG_USE_DT_CODE_PARTITION=y`), so a plain build already qualifies — a `0x8000xxxx` vector means a Flow C fragment/overlay is still layered. See the script header for the gotcha on returning to ITCM apps afterwards. **Same `bench_flowd_atoc_guard()` SE_UART-gated behaviour, same exit 5/8, same `--replace-atoc`/`--atoc-unqueryable` distinction as `flash-jlink.sh` above (#2025/#2027).** The guard call is placed as early as possible — right after arg parsing, before this script's own first J-Link touch — because of an open question recorded in the script that #2027 did not resolve: whether `maintenance -c $SE_UART -opt gettoc` itself resets the target. If it does, `AP[3]` (APAddr `0x00300000`) could be gone by the time step 3's pre-programming halt runs; the documented, still-safe failure mode there is the existing `!! HALT FAILED before programming` abort (**exit 5**, nothing written) — not silent corruption — but it remains unverified on real silicon whether the SE_UART-present path ever actually reaches that state on this script. **Sector-pads BOTH blobs and proves them (#2233):** `bench_flowd_prepare_write()` reads every sector either blob touches, overlays both on them (merged into one image if they land in the same sector), and both `loadbin`s (still `, noreset` on each, #1902 unchanged) write the PADDED images. The old per-blob `verifybin` count is gone, replaced by a fresh-session `bench_flowd_proof()` read-back of both ranges — **exit 3** on any mismatch, **exit 9** if the sector-pad prepare step itself fails. The `#1902` loadbin-failure and pre-programming-halt gates are unchanged. **exit 10** (`FLOWD_DRY_RUN` set — nothing written), **exit 11** (RACE DETECTED, board already written/booted — restore from the printed paths). Not a persistence proof across a power cycle — the header's own cold-cycle readback requirement still stands. |
...
| `flash-update-log-firewall-probe.sh [--package-only] [--replace-atoc] <he-build-dir>` | **D** | Firmware-update-log HE direct-write MRAM firewall probe. Builds an app-only ATOC by default so any already-provisioned DEVICE/firewall policy is preserved; set `ALP_AEN_INCLUDE_DEVICE_CONFIG=yes` only for deliberate DEVICE replacement, and `ALP_AEN_DEVICE_CONFIG_JSON=<file>` to name a board-specific config already staged under SETOOLS `build/config`. **GUARD (#2025, exit 5):** before the write, queries the resident ATOC and refuses to proceed if any entry other than `DEVICE`/`HE-PROBE` is resident, or if the query can't be verified — the `loadbin` below writes the same replacing (not merging) ATOC structure `app-write-mram -p` does. `--replace-atoc` opts out. **Sector-pads and proves the write (#2233; exit 9 if that prepare step itself fails)** the same way as `flash-jlink.sh` above. **Gates on a fresh-session `bench_flowd_proof()` read-back, not `verifybin`: exit 3** on any mismatch — the MRAM write has already happened (that is what `loadbin` is), but (#1526) `RSetType 2`/`r`/`g` are a SEPARATE CommanderScript that only runs once this gate passes, so a failed proof keeps the `HE-PROBE` ATOC entry (`"flags": ["load", "boot"]`) from ever being pin-reset into and booted — `alp_ulog_partition` stays untouched at exit 3, not overwritten. Re-read the partition from a known-good state before trusting the `BASELINE_WORDS` captured earlier in the same run regardless. **exit 10** (`FLOWD_DRY_RUN` set — nothing written), **exit 11** (RACE DETECTED — the write session's own pre-load savebin differs from the pre-read; restore from the printed paths). Not a persistence proof across a power cycle. |
| `flash-update-log-dual.sh [--package-only] [--replace-atoc] <hp-build-dir> <he-build-dir>` | **D** | Firmware-update-log dual-M55 package: HP owner boots first and releases HE client. Builds an app-only ATOC by default for the same firewall-policy reason as the probe helper. **GUARD (#2025, exit 5):** before the write, queries the resident ATOC and refuses to proceed if any entry other than `DEVICE`/`HP-OWNER`/`HE-CLIENT` is resident, or if the query can't be verified. `--replace-atoc` opts out. **Sector-pads and proves the write (#2233; exit 9 if that prepare step itself fails)** the same way as `flash-jlink.sh` above. **Gates on a fresh-session `bench_flowd_proof()` read-back, not `verifybin`: exit 3** on any mismatch — same split-CommanderScript shape as the firewall probe (#1526): the write has already happened, but `RSetType 2`/`r`/`g` are a separate script gated on this proof passing, so a failed proof keeps the `HP-OWNER` ATOC entry (`"flags": ["load", "boot"]`) from being booted and releasing the HE client — `alp_ulog_partition` stays untouched at exit 3. Re-read the partition from a known-good state before trusting the update log a successful run produced regardless. **exit 10** (`FLOWD_DRY_RUN` set — nothing written), **exit 11** (RACE DETECTED — restore from the printed paths). Not a persistence proof across a power cycle. |
...
| `flowd_sector_pad.py plan\|build\|proof` | — | **Pure, host-side (#2233)** — no probe access from this file. `plan --write <blob>:<hex-addr> [--write ...]` prints the sector base(s) (e.g. `0x802E4000`) that must be pre-read before a write can be padded, refusing (exit 1, `REFUSE: ...` on stderr) a blob outside the `0x80000000`-`0x8057FFFF` MRAM window or two blobs overlapping with different bytes. `build --sector-dir <dir> --out-dir <dir> --write ...` overlays the blob(s) on `<sector-dir>`'s pre-read `<ADDR>.bin` sector images (one per merged, sector-aligned range — two writes sharing a sector merge into ONE padded image) and writes `<out-dir>/manifest.json` (`address`/`size`/`image`/`sha256`/`sectors`/`writes` per range) plus one `<out-dir>/<ADDR>.bin` per range. `proof --manifest <manifest.json> --read-dir <dir>` `cmp`s each range's padded image against a FRESH post-write read from `<read-dir>` and prints `PASS`/`FAIL 0x<addr>: ...` per range (exit 1 if any FAIL). `--sector-size`/`--window-lo`/`--window-hi` override the 16 KiB/`0x80000000`/`0x8057FFFF` defaults, and `--sector-size 0`/a non-power-of-two are refused, not a traceback. `plan`/`build` also compute a GUARD sector immediately before/after each padded range (where still inside the window) — pre-read and proved alongside the range itself, so collateral damage just outside the write is visible too, not just damage inside it. `proof` re-checks each padded image's own sha256/size against the manifest before comparing (a corrupted/hand-edited image between `build` and `proof` must not silently become the new "expected"), and refuses an empty manifest outright. Driven by `bench-env.sh`'s `bench_flowd_*` shell functions (see below), never invoked stand-alone by a Flow D writer. |
| `atoc_trailer.py resolve` | — | **Pure, host-side (#2233 review blocker 2)** — no probe access. Parses the 16-byte trailer SETOOLS leaves at the top of the `atoc` region (bench-measured on ONE real E1M-AEN803 board, serial 2026W36-0001, 2026-09-19 — NOT a documented SETOOLS spec) to resolve where the RESIDENT ATOC package actually starts. `resolve --sector-file <pre-read sector> --sector-base <hex> --window-lo <hex> --window-end <hex>` prints `NO_ATOC` for a blank (all-`0x00`/all-`0xFF`) trailer, or `PACKAGE_START=0x...`/`PACKAGE_SIZE=0x...`/`HEADER_ADDRESS=0x...` on success; refuses (exit 1, `REFUSE: ...`) if the header signature (`OEMTOC01`) is missing, `package_start + package_size != window_end`, or `package_start` is below `window_lo` — an inconsistent trailer is never guessed past. Used by `erase-storage.sh` before any write. |
...
| `AEN_BOARD` | `alp_e1m_aen803_m55_he/ae822fa0e5597ls0/rtss_he` | Qualified board target. |
...
| `AEN_OPENOCD_USB_LOCATION` | *(none, error-if-unset)* | Labgrid-pinned USB path for the AEN E8's J-Link — all three E8 boards on this bench answer the same SW-DP (`0x4c013477`), so the USB path is the only thing that selects which physical board you talk to; resolve it per-board from `labgrid-client -p <place> show`'s swd resource, e.g. `labgrid-client -p <your-bench-place> show`, never hardcode a path copied from another board. Passed as `adapter usb location <value>`, prepended on the OpenOCD command line. `openocd-ram-run.sh` only. |
| `TMPDIR` | `/tmp` | Where `bench_atoc_replace_guard()` (#2025) writes its pre-write `gettoc` transcript, `<tag>-atoc-before.<random>`, and where the Flow D sector-pad functions below (#2233) put their per-write scratch directories (padded images, pre-read sectors, post-write proof reads). **Retention is deliberate, not a leak:** every run leaves its file, whether the run passed or aborted — it is the record of what was resident immediately before the write. Never auto-cleaned; sweep `$TMPDIR` by hand periodically. |
| `FLOWD_DRY_RUN` | *(none)* | **Set to exercise the whole Flow D sector-pad pipeline (#2233) with no WRITE ever reaching a probe — a THREE-layer guarantee, not one flag with one effect** (review blocker 1: an earlier version of this fix only had layer 1 below, so the write session still ran for real underneath a "dry run" label). **1.** every writer script itself checks `FLOWD_DRY_RUN` right after building its write CommandFile and, if set, prints `--- DRY RUN: nothing written ---` plus the CommandFile and exits (**exit 10**) — it never calls `bench_jlink_run()` for that CommandFile at all. **2.** `bench_jlink_run()` independently REFUSES (**exit 13**, no probe opened) to run any CommandFile containing a `loadbin`/`erase` line while this is set — a backstop for #1, never the only thing standing between a script and a real write. **3.** `bench_flowd_proof()` never reports success in this mode — it returns **12** ("nothing proven: dry run") rather than 0, so a direct caller can never mistake "dry run" for "proof passed". `bench_flowd_read_sectors()` synthesizes an all-`0x00` sector image instead of a real `savebin` in this mode, so `bench_flowd_build()`/`bench_flowd_loadbin_lines()` downstream still run their REAL code path against real, inspectable files. Never set on a real bench host — every writer's OTHER JLinkExe touches (the DPIDR preflight, ATOC-guard queries, `erase-storage.sh`'s ATOC-trailer read) are unaffected by this variable and still need a real probe/`LG_PLACE`. |
| `FLOWD_SECTOR_SIZE` / `FLOWD_WINDOW_LO` / `FLOWD_WINDOW_HI` | `0x4000` / `0x80000000` / `0x8057FFFF` | Forwarded verbatim to `flowd_sector_pad.py` (#2233). Override only to point the sector-pad machinery at a different part's geometry. |
| `FLOWD_SECTOR_PAD_PY` | `<this dir>/flowd_sector_pad.py` | Path to the sector-pad helper (#2233); override only for a test double. |
...
The AEN801 and AEN803 M55-HE/HP board files
(`zephyr/boards/alp/e1m_aen801_m55_*/board.cmake` and
`e1m_aen803_m55_*/board.cmake`) wire it as the default

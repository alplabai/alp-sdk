...
export BENCH_PLACE=<your-labgrid-place>             # Flow C (ram-run.sh) only -- e.g. e1m-aen-evk-02;
                                                     # you must already hold this place's reservation
export AEN_JLINK_RUN=<board-farm>/bin/jlink-run.sh  # Flow C (ram-run.sh) only -- the per-probe isolation wrapper
...
| `ram-run.sh <build-dir> [sleep_ms] [size] [preload]` | **C** | **RAM-run** an ITCM image (no MRAM write): the load base is DERIVED from the LOAD segment with the lowest `p_paddr` among segments with nonzero `p_filesz` (`readelf -l`) — NOT just the first LOAD segment (an ITCM-retargeted link's first LOAD segment is often a zero-FileSiz `.bss` in DTCM) and not hard-coded `0x0` — an app that hard-codes a slot0 `CONFIG_FLASH_LOAD_OFFSET` still links at a non-zero ITCM address and is loaded there. Needs `arm-zephyr-eabi-nm`/`-readelf` on `PATH` (or `ZEPHYR_SDK_INSTALL_DIR` set) — on a fresh Zephyr SDK install these live under `<sdk-root>/arm-zephyr-eabi/bin`, not the default `PATH` — **and gawk (GNU awk)**, checked explicitly before use: the LOAD-segment derivation needs `strtonum()`, a gawk extension mawk/BSD awk lack. **TWO SEPARATE JLinkExe sessions (alp-sdk#2076), both routed through the board-farm's `jlink-run.sh` wrapper (`BENCH_PLACE` + `AEN_JLINK_RUN`, both required, no default — see below):** session 1 `loadbin`s to that base, `setpc <entry>` (thumb-bit cleared), `go`s, then `exit`s leaving the target RUNNING (no halt, no reset — **bench-verified** 2026-09-13 on evk-02/evk-03, `examples/peripheral-io/blink`, 6 of 6 clean); the host then sleeps `sleep_ms`; session 2 is a FRESH `connect` + `mem8` (no halt) that reads and ASCII-decodes the RAM console. The app is guaranteed to run for **at least** `sleep_ms`, not exactly it — routing each session through the wrapper (a labgrid reservation check + an OpenOCD probe-firmware read before every JLinkExe call) adds its own overhead on top, measured stretching a 1.5s gap to about 3.1s. The read is therefore no longer a frozen snapshot at `sleep_ms`, and unlike `dev`'s single-session shape (which left the core halted on exit) this leaves the core **running** on exit. An in-session `halt` after `go` measured an incoherent core and a failed `mem8` on real hardware (evk-02/-03, 2026-09-13) even though the app was healthy — the two-session, no-halt-before-read shape is the one measured to work. Session 1 is judged complete only once `loadbin`, `setpc`, and `go` have ALL been echoed and none rejected — a crashed or truncated JLinkExe session (killed mid-transcript, e.g. real bench evidence of JLinkExe segfaulting inside `jlink-run.sh`'s `unshare`) is reported as exactly that, not folded into a `loadbin` failure (review round 3, finding 1). **Exit codes:** **1** — toolchain (`arm-zephyr-eabi-*`) not found, the system `awk` has no `strtonum()` (requires gawk), the build dir's `zephyr.elf` doesn't exist, `mktemp` failed, or a `preload` file that doesn't exist (`set -e` abort); **2** — `BENCH_PLACE`/`AEN_JLINK_RUN` unset or the wrapper not executable, or `sleep_ms` is not a plain non-negative decimal integer (all checked before any probe access); **3** — the ELF has no `ram_console_buf` symbol (UART-console build; the ELF itself is confirmed to exist first — exit 1 otherwise, review round 3 finding 9); **4** — no LOAD segment with nonzero `FileSiz`, a non-hex parsed base, or the DPIDR preflight says this is not the AEN E8; **5** — derived base `>= 0x80000000` (slot0/MRAM-linked); **6** — derived base isn't `0x0`, the ITCM global alias, or SRAM; **7** — a connect failure in any of the three JLinkExe sessions (preflight/load/read), including a `jlink-run.sh` refusal (place not held, board unpowered, USB-mask failure) — the wrapper's own message is in the transcript, not just `bench_jlink_assert_connected`'s generic one; **8** — session 1's `loadbin` WAS echoed and ran but did not report `O.K.` (a stale image already resident in ITCM/DTCM could otherwise boot and be read back as this run's output) — judged on the LAST matching `loadbin $BIN $BASE` window, so a same-path preload's own successful `loadbin` ahead of it can't mask this; **9** — session 2's `mem8` itself failed (`Could not read memory.` or no dump line at all); **10** — session 1's JLinkExe transcript crashed (a literal `Segmentation fault`) or was truncated before `loadbin`/`setpc`/`go` were all echoed — this is a distinct failure from exit 8, not a `loadbin` result; **11** — `setpc`/`go` WAS echoed but rejected (a heuristic scan of its own window for rejection wording — no real rejected-transcript sample was available to anchor an exact string, tighten if one surfaces). All transcripts and generated CommandFiles live under one `mktemp -d` under `$TMPDIR`, created just before the first transcript write (so an early exit 1/2/3/4/5/6 leaves nothing behind, review round 3 finding 6) — removed on a **successful** exit only; a failing run leaves it behind (path printed) so a cited transcript is never already deleted. `SIGINT`/`SIGTERM` are trapped (exit 130/143) so a killed run also keeps its WORKDIR rather than the `EXIT` trap seeing a stale `$?=0` (review round 3, finding 3). Optional `preload` JLink file runs after halt / before loadbin in session 1 (e.g. clear a SoC integration reg). |
...
| `BENCH_PLACE` | *(none, error-if-unset)* | `ram-run.sh` only (alp-sdk#2076). The labgrid-client place name whose probe to use (e.g. `e1m-aen-evk-02`) — you must already hold that place's reservation. Routed through `jlink-run.sh` (below) instead of `-SelectEmuBySN`: several probes on this bench share one OEM serial, so a bare serial selector cannot tell them apart, and with two JLinkExe sessions now in one run an unselected probe could load one board and read a DIFFERENT one. |
| `AEN_JLINK_RUN` | *(none, error-if-unset)* | `ram-run.sh` only. The board-farm's per-probe isolation wrapper (e.g. `<board-farm>/bin/jlink-run.sh`): checks the `BENCH_PLACE` labgrid reservation, USB-namespace-masks every other probe, and picks a JLinkExe build whose bundled firmware matches the probe's running firmware. Takes `-CommandFile <path>`, not `-CommanderScript`. No default on purpose (matching `AEN_OPENOCD_CFG` below, not `JLINK_EXE`'s `$HOME/segger-latest` search) — on a real bench host the natural default IS the real wrapper, so a forgotten override would silently drive a real probe instead of failing closed. |

...
| `flash-jlink-a32.sh <bl32.bin> <dtb> <xipImage> <cramfs> <atoc-config.json>` | **D** | J-Link **A32 (Cortex-A32 Linux) five-blob** MRAM flash — the only helper here that writes more than two blobs. Writes `bl32.bin` (TF-A SP_MIN, XIP) → `0x80002000`, the carrier `.dtb` → `0x80010000`, `xipImage` → `0x80020000`, `rootfs.cramfs` → `0x80380000`, and `AppTocPackage.bin` → its parsed address, all in **one held session with no reset**. Only BOOTLOAD (`bl32`) and A32_APP (`xipImage`) are in the ATOC — the dtb reaches the kernel in `r2` via TF-A's `ARM_PRELOADED_DTB_BASE` and the cramfs is found through the MTD physmap, which is why Flow A cannot place this chain (`app-write-mram` writes only what the ATOC describes). Measured 2026-09-05: Linux 6.12.6 boots to a shell on two E1M-AEN803 modules with this layout (SE → TF-A BL32 → Linux as BL33; no BL1/BL2/BL31, no U-Boot). **No reset before programming:** with `RSetType 2` an `r` makes AP[3] (APAddr `0x00300000`) vanish from the CoreSight scan (8/8), after which nothing can be halted — so it is plain `connect` + `h`, and the halt is gated on the `PC = <8 hex>, CycleCnt = ` register dump, never on `Cortex-M55 identified.` (which prints at every connect). Sets `exec SetSkipProgOnCRCMatch = 0`. **Every blob is staged as a `.bin` copy** because `loadbin` refuses a `.dtb` (`File is of unknown / unsupported format.`), emits no `Flash download:` line and writes nothing while still exiting 0. Exits: **1** (usage, missing/empty input, ATOC guard or `app-gen-toc` failure, or a blob that would overrun the next one), **2** (part profile could not connect), **3** (verify failed, or fewer than 5 of 5 `verify successful`), **4** (DPIDR says this is not the AEN E8), **5** (a `loadbin` produced no `Flash download:` line — it wrote nothing), **6** (core never halted). **`verifybin` is not acceptance:** cold power-cycle and read back — there is precedent on this hardware of `Verify successful.` followed by a cold cycle reverting the write; the script keeps the staged files and prints the post-cycle `verifybin` lines to use. Builds none of its inputs (kernel `alifsemi/linux_alif` branch `v6.12-dev`; TF-A `alifsemi/trusted-firmware-a_alif` branch `alif_lts-v2.10.8`, `PLAT=devkit_e7` with `ALIF_SOC_E8=1`) — no bitbake/Yocto build has ever been run for this chain (#1968, #1971). |
